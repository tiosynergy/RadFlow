...
export function studiesLine(it: Pick<SearchResultItem, "studies">): string {
  return it.studies.length
...
export function exportColumnSpec(source: SearchSource, scope: ExportScope): Col[] {
  ...
  cols.push({ header: "Дослідження", width: 44, get: (it) => studiesLine(it) || null });
...
export function exportParamsRows(p: ExportParams): XlsxValue[][] {
  ...
  rows.push([
    "Повнота",
    p.truncated
      ? `НЕ ВСЕ: показано перші ${p.rows} записів — звузьте період або фільтри й вивантажте частинами`
      : "усі записи, що відповідають фільтрам",
  ]);
