...
export function completenessText(incomplete: ExportIncomplete, rows: number): string {
  ...
  if (incomplete === "cursor") return `МОЖЛИВО НЕ ВСЕ: у частини записів некоректний час прийому, і такі записи могли не потрапити у файл (${n}) — вивантажте вужчий період`;
