...
/* ⚠️ ІМЕНА НОРМАЛІЗУЮТЬСЯ НА ВХОДІ: trim + внутрішні пробіли до одного.
   Джерело правила — живий інцидент с31: у `profiles.full_name` жив подвійний
   пробіл («Заставська··Марія»), PatientEditModal зіставляв імена суворим
   рівнянням — і правка ПІБ ПАЦІЄНТА мовчки клала `referrer_id = null`
   (запис зникав із порталу направника). Модалку полагоджено (ключі замість
   імен), але поки імена десь порівнюються як рядки, друга така пара — бомба.
   Тут — ЄДИНА точка входу для всіх обовʼязкових ПІБ.
   ⚠️ НЕ переносити схлопування в zOptText: він і для приміток, де кратні
   пробіли й переноси рядків — легітимний вміст. Для необовʼязкових ІМЕН
   нижче є zOptName. */
const collapseSpaces = (s: string) => s.replace(/\s+/g, " ");
/* Порядок єдиний із zOptName: спершу схлопування, потім межі довжини. */
export const zName = z
  .string()
  .trim()
  .transform(collapseSpaces)
  .pipe(z.string().min(1, "Вкажіть ПІБ").max(200));
/** Необовʼязкове ІМʼЯ: семантика zOptText(200) ("" / null / undefined → null)
 *  плюс схлопування внутрішніх пробілів. ⚠️ Як і zOptText, перетворює
 *  ВІДСУТНІЙ ключ на null — у patch-обʼєктах («відсутній ключ = не чіпати»)
 *  НЕ використовувати: там потрібен union із .optional() (див. sPatientPatch). */
export const zOptName = z
  .union([z.string(), z.null(), z.undefined()])
  .transform((v) => (typeof v === "string" ? collapseSpaces(v.trim()) : ""))
