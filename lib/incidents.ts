...
//
// ⚠️ Ф4-8 СОЗНАТЕЛЬНО НЕ ТРОГАЕТ ЭТУ ФУНКЦИЮ — и это решение, а не забывчивость.
// Смещение часов измеряется АСИНХРОННО и приезжает через сотни миллисекунд
// после монтирования. Доски фиксируют `selectedDate = wallToday0()` ПЕРВЫМ
// рендером, а `today`/`isToday` пересчитывают каждый рендер. Если поправка
// пересекает полночь клиники (ПК отстаёт на 8 минут, реальное время 00:04),
// то через секунду после загрузки `today` уезжает на следующий день,
// `selectedDate` остаётся на прежнем, и доска МОЛЧА становится архивом:
// «Викликати» блокируется, звуки гаснут, записи нового дня не загружены
// (находка ревью А, HIGH). Это ровно тот класс «расхождение фреймов», который
// проект уже закрывал как баг M-4.
// Поэтому настенный канон правится ОТДЕЛЬНЫМ пакетом (U-70), где доски учатся
// следовать за «сегодня». Здесь исправлено ровно то, что в находке Ф4-8:
// сравнения с ИНСТАНТОМ, который поставил сервер (in_progress_at).
// Для тех мест, где нужен настенный момент по часам БАЗЫ, есть wallServerNow().
...
/* Настенный момент в зоне клиники, посчитанный по часам БАЗЫ (Ф4-8).
   Отличается от wallNow ровно одним: инстант берётся из serverNow(), то есть с
   поправкой на смещение часов браузера. Смещение применяется к ИНСТАНТУ, а
   зона раскладывается уже из исправленного момента — поэтому переход DST
   внутри поправки обрабатывает тот же Intl, что и всегда.

   Сейчас единственный потребитель — подпись «завершення о HH:MM» в
   components/StudyTimer.tsx, и это не случайность. Там остаток считается по
   serverNow(), и если бы подпись бралась из wallNow(), две ошибки перестали
   бы сокращаться: ДО пакета время завершения показывалось ВЕРНО именно
   потому, что и «сейчас», и остаток шли от одних (сломанных) часов.
   ⚠️ Не подменяйте этим wallNow() «за компанию»: см. предупреждение над
   wallNow — доски ловят «сегодня» первым рендером, до того как смещение
   измерено (U-70). */
export function wallServerNow(tz?: string): number {
  return wallNow(tz) + (serverNow() - Date.now());
}
