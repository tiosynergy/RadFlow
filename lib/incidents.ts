...
// Ф4-8: «сейчас» для настенного канона берётся из serverNow(), а не из
// Date.now(). Импорт односторонний — lib/serverClock.ts не знает ни о чём,
// кроме собственной арифметики, так что цикла тут быть не может.
...
/* ===== U-33 (с49): простій блокує ДОСЛІДЖЕННЯ, а не лише його старт =====
   ...
   `slotBlockedByFeed` питає «чи вільний ЦЕЙ момент» — і саме на ньому стояли
   `BookingModal` і `RescheduleModal`. Але серверний гард порівнює ДІАПАЗОНИ:

     tstzrange(started_at, coalesce(blocked_until,'infinity'))
       && tstzrange(scheduled_at, scheduled_at + duration_min)

   тож дослідження, яке ПОЧИНАЄТЬСЯ до простою і ЗАХОДИТЬ у нього, сервер
   відхиляє, а сітка малювала слот вільним. Планове ТО о 12:00 і запис на 11:45
   тривалістю 30 хв — зелений слот і відмова після натискання «Зберегти».
   ⚠️ Асиметрія була ВСЕРЕДИНІ однієї функції `slotState`: для чужих записів
   (`roomBusy`), графіка і перерв там рахувався ІНТЕРВАЛ, і лише для простоїв —
   один момент.

   ⚠️ Буфер прибирання сюди НЕ входить: гард рахує рівно `duration_min`.
   Додати буфер означало б блокувати слоти, які сервер приймає.

   ⚠️ ЧЕСНО ПРО МЕЖУ ДЗЕРКАЛА (ревʼю пакета). `durMin` очікується скінченним і
   додатним — так його передають усі три форми (`slotDur` / `dur`). Для
   `durMin <= 0` чи NaN дзеркало РОЗХОДИТЬСЯ із сервером, і саме в
   консервативний бік: сервер при `duration_min is null` пропускає запис
   (`if … then return new` у першому рядку гарда), а гілка `cap === 0` нижче
   блокує. Це свідомо: у формах така тривалість не виникає, а «старт усередині
   простою» — не той випадок, де варто послаблювати.
   Дві гілки нижче названі окремо навмисно — кожна відповідає на своє питання, і
   «невідомо» не має шляху стати «вільно» (те саме правило, що в усьому модулі). */
export function studyBlockedByFeed<T extends IncidentLike>(
  ...
  const cap = incidentDurCapMin(feed, roomId, startMs);
  if (cap === undefined) return true;   // простої не прочитані → «невідомо ≠ вільно»
  if (cap === 0) return true;           // старт уже всередині простою (стара відповідь)
  return cap < durMin;
}
...
//
// U-70: момент берётся из serverNow(), то есть с поправкой на смещение часов
// браузера. Это ОДНА правка на весь настенный канон: часы ПК, ушедшие на
// 8 минут, сдвигали разом «Запізнення», computeCallBlock, сетки слотов,
// LiveClock и выбор «сегодня» на досках.
//
// ⚠️ ПОЧЕМУ ЭТО НЕ СДЕЛАЛИ В Ф4-8 И ЧТО ПРИШЛОСЬ ДОБАВИТЬ ВМЕСТЕ С ЭТИМ.
// Смещение измеряется АСИНХРОННО и приезжает через сотни миллисекунд после
// монтирования. Доски фиксируют `selectedDate = wallToday0()` ПЕРВЫМ рендером,
// а `today`/`isToday` пересчитывают каждый рендер. Если поправка пересекает
// полночь клиники (ПК отстаёт на 8 минут, реальное время 00:04), `today`
// уезжает на следующий день, `selectedDate` остаётся — и доска МОЛЧА
// становится архивом: «Викликати» заблокировано, звуки выключены, записи
// нового дня в срез не загружены (находка ревью А пакета Ф4-8, HIGH).
// Поэтому вместе с этой строкой обе доски получили правило «показывала
// сегодня — за сегодня и следует» (`lib/useFollowToday.ts`, один экземпляр на
// обе). Оно будится сменой эпохи часов (`lib/useClockEpoch.ts`) и переносит
// дату, ТОЛЬКО если сутки перенесла сама поправка — сравниваются `wallDayKeyAt`
// от одного и того же `Date.now()` со старым и новым смещением. На настоящую
// полночь правило не реагирует. Без него эта правка — регрессия, а не улучшение.
//
// ⚠️ Смещение применяется к ИНСТАНТУ, а не к настенным частям: зона
// раскладывается уже из исправленного момента, поэтому переход DST внутри
// поправки обработан тем же Intl, что и всегда.
// Пока смещение не измерено, serverNow() === Date.now() — поведение прежнее.
...
/* U-70: `wallServerNow` СВЁРНУТА в `wallNow` — они стали одним и тем же.
   Она появилась в Ф4-8 ровно потому, что `wallNow` тогда осталась на часах
   браузера, и подписи «завершення о HH:MM» нужен был исправленный настенный
   момент (иначе две ошибки, которые до Ф4-8 сокращались, разъехались бы).
   Теперь исправлена сама `wallNow`, и второе имя для того же значения было бы
   ровно тем «двум копиям одного правила», против которых написан аудит.
   Потребитель (components/StudyTimer.tsx) переведён обратно на `wallNow()`. */
...
export function wallDayKey(tz?: string): string {
  return dayKeyOfWallMs(wallNow(tz));
}

// Ключ суток из УЖЕ настенных мс. Один форматтер на оба входа ниже — иначе
// «сегодня по текущим часам» и «сегодня по прежним часам» могли бы разойтись
// в формате и сравниваться строкой всегда неравно.
function dayKeyOfWallMs(wallMs: number): string {
...
/* U-70. Ключ суток клиники для ЗАДАННОГО реального инстанта, а не для «сейчас».
   Отвечает ровно на один вопрос, и он не риторический: «перенесла ли ПОПРАВКА
   часов календарные сутки?». Доски обязаны отличать «сутки сменились, потому
   что прошло время» (это не их дело — дёргать дату под руками оператора в
   полночь пакет не собирался) от «сутки сменились, потому что мы исправили
   часы» (тогда зафиксированное `selectedDate` протухло и доска молча стала
   архивом).
   Без этой функции вопрос задавался сравнением с прошлым запуском эффекта, и
   ответ зависел от того, успел ли сработать 20-секундный тикер (находка
   ревью А по U-70, HIGH). Теперь обе даты считаются от ОДНОГО `Date.now()`
   двумя смещениями, и разница может быть только поправочной.
   Нефинитный ms → ключ по текущим часам: безопасный статус-кво, не бросаем. */
export function wallDayKeyAt(ms: number, tz?: string): string {
  if (!Number.isFinite(ms)) return wallDayKey(tz);
