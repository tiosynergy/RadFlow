...
/* Невизначеність «зараз» у МЕЖАХ ОДНІЄЇ СЕКУНДИ — і рівно стільки, скільки
   ...
   ⚠️ УТОЧНЕНО ПІСЛЯ Ф4-8 (ревʼю А того пакета). Формулювання «залишки
   однонаправлені» стосується САМЕ цієї функції і сьогоднішнього стану:
   `lateCallClash` рахує від `wallNow()`, а Ф4-8 свідомо лишив `wallNow()` на
   годиннику браузера (правити настінний канон — окремий пакет, U-70). Коли
   U-70 переведе `wallNow()` на виміряний час, однонаправленість зникне:
   поправка може ПЕРЕСТРИБНУТИ сервер на половину RTT кращої проби, і клієнт
   стане подекуди «попереду». Тоді цю константу треба перерахувати як
   двосторонню (слак з обох боків або поріг за RTT), а не лишати як є.
   Обидва залишки СЬОГОДНІ однонаправлені (сервер суворіший) і закінчуються
   чесною помилкою `slot_unavailable`, а не зіпсованими даними: авторитет —
   гард.
