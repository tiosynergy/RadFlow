...
/**
 * Підпис позначки з поверхні ГРАФІКА (0184, RF-03b).
 *
 * ⚠️ ЧОМУ ТУТ Є ДАТА І КАБІНЕТ. Перша редакція повертала «графік роботи центру
 *    на день» — без дати і без кабінету. Ревʼю (лінза «досвід живої людини»)
 *    назвало це блокером, і воно має рацію: ack на цій поверхні ПОВЕРХНЕВИЙ,
 *    тобто відкриття будь-якого дня гасить усі крапки поверхні. Крапка, що
 *    гасне раніше, ніж людина встигла дізнатись ЩО саме змінилось, знищує
 *    новину назавжди — журналу в направника немає, а ретенція чистить лише
 *    прочитане. Дані для підпису ВЖЕ лежать у позначці (`subject_date`,
 *    `details.roomId`) — їх треба було просто вимовити.
 *
 * ⚠️ Кабінет називаємо ФАКТОМ («кабінету»), а не імʼям: назви кабінету на
 *    клієнті в цьому місці немає, а показувати uuid — гірше, ніж не показувати.
 * ⚠️ Формат дати — DD.MM без локалі: `Intl` тут дав би різний текст під різними
 *    середовищами, а підпис читає скрінрідер, і він мусить бути стабільним.
 */
export function scheduleSurfaceText(m: ChangeMarker): string {
  const d = m.subject_date && /^\d{4}-\d{2}-\d{2}$/.test(m.subject_date)
    ? ` на ${m.subject_date.slice(8, 10)}.${m.subject_date.slice(5, 7)}`
    : "";
  const scope = m.details && typeof m.details === "object" && "roomId" in m.details
    ? "кабінету"
    : "центру";
  return m.event_type === "schedule.override_cleared"
    ? `особливий графік ${scope}${d} знято`
    : `графік ${scope}${d}`;
}
...
export function markerWhat(m: ChangeMarker): string {
  /* ⚠️ 0184 (RF-03b): `field_scope='schedule'` НЕ однозначний. Для рядка ЧЕРГИ
     він означає «дата, час або кабінет» цього запису, а для позначки з
     поверхні графіка — «графік роботи центру на день», тобто зовсім інше.
     Розрізняє їх ПОВЕРХНЯ, і гілка стоїть ПЕРШОЮ саме тому: без неї направник
     читав би «змінено дату, час або кабінет» там, де жодного запису немає. */
  if (m.surface_key === "schedule") return scheduleSurfaceText(m);
...
export const SURFACE_BY_NAV: Record<string, SurfaceKey[]> = {
  ...
  ref: ["referrals"],
  centers: ["centers"],
  /* 0184 (RF-03b): «Нове направлення» — ЄДИНИЙ екран направника, де видно
     графік дня, і єдине місце з `useAckWhenVisible({surface:'schedule'})`.
     Ключ 'new' — це ключ вкладки в ReferrerSidebar, а не назва поверхні:
     карта веде surface → ПУНКТ МЕНЮ, і в персонала такого пункту немає, тож
     їхні сайдбари цей ключ просто не питають. */
  new: ["schedule"],
