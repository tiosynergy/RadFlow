...
/** Чи є ЩО показувати ЗАРАЗ. Дві умови, і жодна не косметична:
      • `fromKey !== toKey` — поправка «туди-назад» лишає накопичене
        повідомлення про добу, з якої нікуди не поїхали: банер «змінено з
        1 вересня на 1 вересня» (борг Г1-G, живий у чотирьох екранах);
      • `toKey === curKey` — повідомлення прив'язане до доби, ПРО ЯКУ воно
        говорить. Дошка, на відміну від модалки, живе довго, і оператор ходить
        по датах вільно: банер, що пережив перехід на іншу добу, бреше про
        поточний екран. Це ВЛАСТИВІСТЬ, а не перелік місць, зобовʼязаних його
        гасити, — новий спосіб змінити дату не розійдеться з нею мовчки.
    Явне гасіння там, де дату бере в руки ЛЮДИНА, лишається окремо і поруч:
    воно про НАМІР (банер прочитано), а ця умова — про БЕЗПЕКУ (банер не бреше
    про добу, якої вже немає на екрані). */
export function dayShiftNoticeVisible(n: DayShiftNotice | null, curKey: string): boolean {
