...
export const CONTRAST_SURCHARGE = 900; // доплата за контраст, грн (ТОЛЬКО статический справочник)
export const CONTRAST_DUR = 15; // +мин за контраст (ТОЛЬКО статический справочник)

/* ===== Контраст как СВОЙСТВО ПОЗИЦИИ ПРАЙСА (каталог центра) =====
   В настроенном каталоге контрастные исследования — ОТДЕЛЬНЫЕ строки прайса со
   своей ценой («МРТ головного мозку до та після в/в контрастування» 4900 ₴ против
   2200 ₴ у обычной). Поэтому там «Контраст» — ФИЛЬТР списка услуг по названию, а
   не модификатор с доплатой: доплата означала бы 4900 + 900.

   CONTRAST_SURCHARGE / CONTRAST_DUR остаются рабочими ТОЛЬКО для статического
   справочника ниже (центр без каталога): там области generic («Голова / мозок»),
   и контраст действительно модификатор. Границу режимов держит
   Catalog.isConfigured() — см. lib/catalog.ts. */

/* Слово «контраст» НЕ про контрастное вещество: «фазово-контрастна
   МР-венографія» — техника съёмки, «контрастність» — свойство изображения.
   Такие позиции контрастными не считаем. */
const NOT_AGENT_RE = /фазово[\s-]*контрастн\w*|контрастн[іо]ст\w*/gi;
/* «без контрасту», «безконтрастна», «без-контрастний» — вырезаем ЦЕЛИКОМ, а не
   отбрасываем всё название: у смешанного «без контрастування ТА З контрастуванням»
   после вырезания остаётся положительное упоминание → позиция контрастная.
   Дефолт смещён в безопасную сторону: пропустить контраст опаснее, чем показать
   лишнюю позицию (подготовка пациента, аллергия на гадолиний). */
const NO_CONTRAST_RE = /без[\s-]*контраст\w*/gi;
/** Корень «контраст» общий для укр./рус. форм: контрастування, контрастом,
    контрастирование, з контрастом. */
const CONTRAST_RE = /контраст/i;

/** Контрастная ли позиция прайса — по НАЗВАНИЮ услуги. Единственный источник
    правды для фильтра «Контраст» во всех формах записи и для флага
    studies[].contrast в режиме каталога. */
export function isContrastName(name?: string | null): boolean {
  const s = String(name ?? "");
  if (!s) return false;
  const stripped = s.replace(NOT_AGENT_RE, " ").replace(NO_CONTRAST_RE, " ");
  return CONTRAST_RE.test(stripped);
}
...
/* Название одного исследования для показа.
   Суффикс « з контрастом» НЕ добавляем, если контраст уже назван в самой позиции
   прайса («…до та після в/в контрастування») — иначе слово звучит дважды на всех
   досках, в колл-листе и в листе ожидания (ревью, L1). */
export function studyLabel(s: Study): string {
  const suffix = s.contrast && !isContrastName(s.region) ? " з контрастом" : "";
  return (s.type || "МРТ") + " · " + (s.region || "") + suffix;
}
...
export function studyText(s?: Study | null): string {
  ...
  return (
    (s.type || "") +
    (s.region ? " · " + s.region : "") +
    (s.contrast && !isContrastName(s.region) ? " · з контрастом" : "") +
    (s.dur ? " · " + s.dur + " хв" : "")
  );
}
