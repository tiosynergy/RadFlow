/* ===== RadFlow — довіра до ЧИТАННЯ графіка кабінету (U-13) =====
   ...
   валить тест на новому читачі — числа тут навмисно не дублюємо):
     ...
     • ⚠️ СЕРВЕРНИЙ гейт `app/queue/actions.ts` (scheduleGate + roomDayCtx) —
       найдорожче місце: він вирішує, чи запис узагалі створиться.
       ⚠️ Тут стояло «за коментарем у ньому графік у БД не enforce'иться, і для
       „після кінця дня“ цей гейт єдиний рубіж» — переказ чужого коментаря,
       який із 0084 (`check_room_schedule`) НЕПРАВДИВИЙ: графік є інваріантом
       БД, рубежів два. Виправлено в с63 разом із першоджерелом; переказувати
       сусідній файл замість заміру — і є той спосіб, яким неправда розійшлась
       по чотирьох місцях.
