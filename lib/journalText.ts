...
export function eventTitle(item: {
  ...
  switch (item.eventType) {
    ...
    case "patient_data.exported": {
      /* с77: перший реальний емітер — експорт пошуку в Excel. Скільки записів
         ЦЬОГО центру пішло у файл — сенс події для адміністратора; джерело й
         «обрізано» — у details, заголовок не перевантажуємо.
         с79 (Н-10): другий канал — CSV дашборда CEO (`source: "ceo_dashboard"`).
         Його в заголовку НАЗВАНО: інакше два різні файли (пошук і дашборд із
         доходом) читались би в журналі однаково, а інших слідів каналу екран
         журналу не показує. Пошук лишився без назви — як і був. */
      const rows = num(d, "rows");
      const tail = rows != null ? ` (записів: ${rows})` : "";
      ...
      /* с80 (Н-16): третій канал — CSV колл-листа (ПІБ, телефон, нотатка дзвінка
         за день). Названо з тієї ж причини, що й дашборд CEO. */
      if (str(d, "source") === "call_list") {
        return say("вивантажив дані пацієнтів з колл-листа", "вивантажено дані пацієнтів з колл-листа", tail);
      }
