...
**Git:** ветка `dev`, база — `91e324b` (на 1 коммит доков впереди `main` = `1f58405`).
Незакоммичено — **пакет сессии 14, 43 файла**: `PATCH /api/staff` + форма редактирования
карточки в `StaffManager`, перевод типографики на rem (684 конверсии), точечные
WCAG-блокеры, `lib/login.ts` + 2 теста, доки, `scripts/px-to-rem.py`,
`supabase/maintenance/2026-07-28_cleanup_test_data.sql`. **Коммитит владелец.**
...
0. **Владелец: закоммитить пакет сессии 14** (43 файла, 312/312) и **выполнить**
   `supabase/maintenance/2026-07-28_cleanup_test_data.sql` (dry-run пройден). Затем мердж
   `dev → main` (не забыть `git fetch` + `git pull --ff-only origin main` ДО мерджа и
   тулчейн НА СМЕРДЖЕННОМ дереве).
