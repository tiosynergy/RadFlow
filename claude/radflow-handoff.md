...
**Обновлено: 2026-08-26 ~11:05 UTC (14:05 Kiev), сессия 42 — ЗАКРЫТА.**

**Прод-БД на `0161`**, ledger 161, все md5, сторож `ok:true
**checked:13**` (в т.ч. новая проверка `gcal_sync_overdue` — молчит,
потому что зеркало живое). `dev` = **`2be8daf`**, `main` = **`7a4bf51`**
(PR #51) — пакет 6 влит, дерево чистое. Смоук 0160 (19 зон) и смоук
0161 (8 зон) прогнаны на проде; live-сверка периметра 0160 (RLS
deny-all, Vault-хелперы только service_role) чистая. Следующая
миграция — **0162**.
