    ...
    <div className="overlay">
      <div className="dialog fade-in" style={{ maxWidth: 560 }} ref={dialogRef} role="dialog" aria-modal="true" aria-label="Аварійна зупинка">
        ...
        <div className="dlg-body" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {stoppedRoomIds.length > 0 && (
            <div className="fld" style={{ marginBottom: 0 }}>
              <span className="fld-lab">Зараз зупинено{typeof affectedCount === "number" ? ` · постраждалих сьогодні: ${affectedCount}` : ""}</span>
              <div className="bd-rooms">
                {rooms.filter((r) => stopped.has(r.id)).map((r) => (
                  <div key={r.id} className="bd-room active" style={{ cursor: "default" }}>
        ...
        <div className="dlg-foot">
          {stoppedRoomIds.length > 0 && (
            <button className="btn btn-green" disabled={busy} onClick={() => onResume()} style={{ marginRight: "auto" }}>▶ Відновити роботу</button>
          )}
