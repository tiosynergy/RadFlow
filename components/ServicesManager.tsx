...
export default function ServicesManager({ clinicId, clinicTz, initialServices, roomOverrides, rooms, residualRoomIds, residualRoomCounts, clinicName, adminName }: Props) {
  ...
  /* Каталог приходить SSR-пропами і рендериться ЦІЛИМ — тож відкриття
     ...
     ⚠️ `refreezeKey` ТУТ НЕ ПОТРІБЕН, І ЦЕ РІШЕННЯ, А НЕ НЕДОГЛЯД (с63).
     Пункт «поверхня services без refreezeKey» стояв у техборгу з F4-10 і
     виглядав як механічний недоробок: мовляв, `QueueBoard` і
     `RadiologistBoard` ключ передають, а ці два — ні. Замір показав, що для
     ЦЬОГО екрана порада F4-10 хибна.

     Причина в тому, звідки береться список. У дошки простої читаються
     клієнтом і оновлюються realtime — тобто нова позначка приходить РАЗОМ із
     рядком, який людина бачить, і гасити її законно. Тут каталог — SSR-пропи,
     і на екрані НІКОЛИ не зʼявляється нічого нового: у файлі немає ні
     `useRealtimeRefetch`, ні `router.refresh` (заміряно, с63). Додати
     `refreezeKey` означало б гасити позначки про правки каталогу, ЯКИХ
     АДМІН НЕ БАЧИВ — його екран усе ще показує знімок на момент завантаження.
     Це рівно той дефект, проти якого існує `useAckWhenVisible`, тільки
     навиворіт: не «крапка висить на видимому», а «крапка згасла на
     невидимому».

     Порада F4-10 лишається чинною для `ReferralPortal` (`surface: "waitlist"`) —
     там список живий. Розійшлись не поверхні, а спосіб їх наповнення. */
