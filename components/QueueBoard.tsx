...
export default function QueueBoard({ clinicId, clinicTz, rooms, residualRoomIds, residualRoomCounts, services, roomOverrides, clinicName, adminName, adminRole, roleKey, initialDate = null, initialEntry = null }: QueueBoardProps) {
  ...
  /* ⚠️ F3 (с55): `pinnedKey` тут БІЛЬШЕ НЕМАЄ — рішення власника, підперте
     виміром. Дошки передавали `pinnedKey: initialDate`, тобто дату з дип-лінка.
     Пін у `derivedFromToday` — це `pinnedKey && curKey === pinnedKey`, збіг
     ЗНАЧЕНЬ, а не «оператор ще не йшов із цієї дати». Зонд с55, позиція P7:
     ПЕРША умова предиката вже відсікає будь-яку ЧУЖУ дату, тож пін міняє вислід
     РІВНО в одному стані — коли дип-лінк вказує на СЬОГОДНІ. А саме там він
     шкодить двічі: правило мовчить (дошка мовчки стає архівом — повернений
     дефект Г1-E), і `clockClaimOf` заявляє серверу `fromToday: false`, тобто
     відмова Г1-F на інлайн-переносах дошки не спрацьовує (P5a/P6a: вердикт
     `ok` замість `skew` при розходженні годинників 40 хв).
     ⚠️ НАЗВАНА ЦІНА, прийнята свідомо, і перша редакція цього абзацу її
     ЗАНИЗИЛА (знахідка ревʼю А): там стояло тільки «автоскролу вже не буде».
     Повна ціна ширша. Якщо поправка перенесла добу, запис, по який оператор
     прийшов дип-лінком, лишається на ІНШІЙ добі. А якщо поправка приземлилась
     РАНІШЕ, ніж догрузився список записів дип-лінкової доби, ефект переходу
     (`deepLinkDone` вище) відпрацює вже на списку НОВОЇ доби, не знайде там
     `initialEntry` і ЗГОРИТЬ назавжди — тобто рядок не розгорнеться взагалі,
     і повернення календарем на ту саму дату його вже не розгорне.
     Дошка при цьому не мовчить про головне: обидві доби називає банер Г1-E.
     Про сам запис вона не каже нічого — це названа межа пакета, не пропуск.
     ⚠️ ДРУГА НАЗВАНА ЦІНА (ревʼю А): коли вимірювач годинника МОВЧИТЬ (немає
     мережі, стара база, протухла сесія), зсув лишається 0, «сьогодні» дошки —
     це збитий годинник ПК, і тепер, без піна, інлайн-перенос на СЬОГОДНІШНЮ
     дату завжди возить `fromToday: true` → сервер відповідає відмовою Г1-F по
     колу. Вихід «обрати іншу добу» лишається (на чужій добі `fromToday` хибний),
     але для сьогоднішньої доби виходу немає. Це та сама ціна короткого тексту
     `CLOCK_SKEW_MSG`, яку власник прийняв свідомо в с54; F3 лише прибрала
     єдиний стан, у якому вона не наставала. */
  useFollowToday({ clinicTz, busy: anyModalOpen, value: selectedDate, setDate: setSelectedDate,
  ...
  /* Г1-F: заявка про годинник для ІНЛАЙН-переносів дошки (без модалки). Аргументи
     дослівно ті самі, що у виклику `useFollowToday` вище (зсув 0 і, з с55, БЕЗ
     `pinnedKey` — причина розписана там) — предикат «дата виведена з сьогодні»
     один і той самий, тож сервер судить рівно про те значення, яке дошка й
     підставила.
     ⚠️ Однаковість цих двох викликів — ВИМОГА, а не охайність: розійшовшись, вони
     дадуть різні відповіді на одне питання, і гард судитиме не про ту дату. Саме
     цим механізмом F3 і вимикала відмову Г1-F: пін стояв в ОБОХ, і обидва разом
     казали серверу «дата не виведена з сьогодні». */
  const boardClock = () => clockClaimOf({ clinicTz, curKey: dateKey(selectedDate) });
