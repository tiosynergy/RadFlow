...
describe("Ф4-2 — поведінка lateCallClash на секундних краях вікна", () => {
  ...
  it("слот за хвилину ПІСЛЯ кінця — вільно (слак не з'їдає цілу хвилину)", () => {
    ...
    expect(lateCallClash(P, slot("11:06"), at("10:30", 59))).toBeNull();
  });
...
describe("Ф4-2 — секунди у самому scheduled_time", () => {
  ...
  it("секунда виводить слот за межу вікна — і клієнт це бачить", () => {
    // Вікно 10:30:00 + 35 хв + слак 1 с = до 11:05:01. Слот 11:05:30 — поза ним.
  ...
  /* Дробова секунда — єдиний вхід, що РОЗРІЗНЯЄ слак і оператор порівняння
     ...
     11:05:01, і слот 11:05:00.5 всередині — а без слака з `<=` він поза.
...
describe("Ф4-2 — арифметика вікна не роздвоїлась", () => {
  ...
  /* Слак — саме СЕКУНДА розрядності wallNow(), а не «трохи про запас». Виріс
     би він до хвилини — клієнт почав би блокувати цілий слот, якого сервер не
     блокує, і жорстка кнопка «Викликати» гасла б без причини. */
  it("слак не більший за розрядність джерела часу", () => {
    expect(CALL_WINDOW_CLOCK_SLACK_MS).toBe(1000);
  });
