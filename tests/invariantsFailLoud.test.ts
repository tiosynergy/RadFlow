...
/* ⚠️ ВЛАСНІ обробники — ті, що НЕ належать обгортці 0174 (рядки без маркера).
   Їх рівно ТРИ, і кожен названий:
     • №18 — навколо живого виклику `server_now`;
     • №19 — обгорнута по-своєму ще з 0172;
     • оракул усередині №21 (0177) — падіння оракула (апстрим прибрав функцію,
       відкликав EXECUTE, змінив композитний тип) НЕ має ослiплювати перевірку
       публікації та replica identity: то НАШЕ хазяйство, воно перевірне завжди.
   Число тут КОНСТАНТА свідомо: додати четвертий власний обробник — це рішення,
   яке має вимагати правки сторожа і пояснення, а не проїхати мовчки. */
const OWN_HANDLERS = 3;
...
describe(`0174 — сторож не падає мовчки (${FILE})`, () => {
  ...
  it("обробників = обгорткам + власним (№18, №19 і оракул №21)", () => {
    expect((SRC.match(/exception when others then/g) || []).length)
      .toBe((CHECKS - 1) + OWN_HANDLERS);
  });
  ...
  it("зняття рядків обгортки лишає ТІЛЬКИ код перевірок — відступи не зсунуті", () => {
    ...
    expect((back.match(/exception when others then/g) || []).length).toBe(OWN_HANDLERS);
