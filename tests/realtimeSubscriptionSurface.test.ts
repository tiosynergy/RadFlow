...
const UNFILTERED_ALLOWED: Record<string, "accepted" | "no-pii"> = {
  ...
  /* ⚠️ СПИСОК ПОРОЖНІЙ — і це стан, а не забута заготовка (U-65, с57).
     Було пʼять записів: `lib/slotBusy.ts:queue_entries` і
     `components/Sidebar.tsx:waitlist_entries` («accepted»), плюс три таблиці
     каталогу в `components/ReferralPortal.tsx` («no-pii»). Усі пʼять закриті
     фільтром `clinic_id=eq.` — там, де центр один, прямо; там, де їх кілька
     (сайдбар керівника, портал направника), фан-аутом «по підписці на центр»,
     як давно зроблено в CeoDashboard: `postgres_changes` уміє рівно одну
     рівність на підписку.
     Прибрані звідси РАЗОМ із правкою коду. Список, який лишає закритий борг
     «прийнятим», нічим не кращий за документацію, що розійшлася з кодом.
     ⚠️ ЩО ЦЕ РОБИТЬ ІЗ ПЕРЕВІРКАМИ НИЖЧЕ, чесно: перевірка «підстава no-pii
     звіряється зі схемою» тепер ходить по ПОРОЖНЬОМУ словнику, тобто зелена
     тавтологічно. Вона лишається не як доказ, а як механізм: щойно сюди
     повернеться перший запис, вона знову щось перевіряє — і що вона справді
     жива, доводить стенд (мутація S6 кладе в словник PII-таблицю з підставою
     «no-pii» і чекає червоного).
     Доказ тримають дві інші: сканер («усе, що без фільтра, — у списку») і пін
     («список = ∅»). Разом вони дають те, заради чого пакет і робився, — жодної
     підписки без фільтра в проєкті. Третього тесту з такою назвою тут немає
     СВІДОМО: він був би зеленим за побудовою цих двох.
     Плюс перевірка, що `SENSITIVE` не зіпсувалась: вона не залежить від цього
     словника взагалі і тому не стає тавтологією разом із ним.
     ⚠️ ЩО ЛИШИЛОСЬ НЕОХОПЛЕНИМ і стане видимим, щойно словник наповниться:
     ЗВОРОТНИЙ напрямок «у списку немає мертвих записів» (борг закрили, а рядок
     лишили). Сьогодні його ловить пін — будь-який зайвий рядок робить словник
     непорожнім. Повернеться перший запис — і напрямок лишиться без сторожа;
     саме цю помилку я мало не зробив у цьому пакеті, полагодивши фільтри й
     лишивши записи. */
};
...
describe("U-61 — жодної підписки без фільтра, крім явно дозволених", () => {
  ...
  /* ⚠️ НАЗВУ ВИПРАВЛЕНО в с52 (U-80б). Було: «PII-таблиця не може прикинутись
     «no-pii», а сама SENSITIVE не могла зіпсуватись» — і перша половина цієї
     назви описувала НЕ цей тест, а попередній. Тіло сюди ніколи не заглядало в
     `UNFILTERED_ALLOWED`: воно перевіряє рівно те, що регулярка `SENSITIVE` і
     далі ловить очевидні PII-таблиці. Знайдено фальсифікацією: мутація S6
     («PII-таблицю переклеїли підставою no-pii») червонить перевірку ВИЩЕ, а цей
     тест лишається зеленим — тобто назва обіцяла сторожа, якого тут немає.
     Це той самий клас, що й решта пакета: підпис ширший за доказ. */
  it("регулярка SENSITIVE не зіпсувалась — інакше перевірка підстав стає тавтологією", () => {
    /* Якщо `SENSITIVE` перестане ловити явні PII-таблиці, тест вище («підстава
       «no-pii» звіряється зі СХЕМОЮ») знайде нуль чутливих колонок у будь-чому
       і зеленітиме завжди. Тримаємо чотири таблиці, у яких PII є напевно. */
  ...
  /* ⚠️ ПІНИМО І ЗНАЧЕННЯ, не лише ключі (с52, U-80б). Перша редакція звіряла
     `Object.keys(...)`, і цілий напрямок був невидимий: підставу можна було
     ОСЛАБИТИ, не чіпаючи ключів. `"no-pii"` перевіряється машиною (тест вище
     ходить у схему), `"accepted"` — це рішення людини, яке не перевіряє ніхто.
     Заміна `"no-pii"` → `"accepted"` тихо виводила таблицю з-під сверки зі
     схемою: зʼявиться завтра в `rooms` колонка з PII — і не почервоніє ніщо.
     Ключі не змінились, розмір не змінився, усі пʼять тестів зелені.
     Тепер словник звіряється цілком, і будь-яка зміна підстави гучна. */
  it("список підстав не роздувся мовчки і жодну підставу не ослаблено", () => {
    ...
    /* ⚠️ ПОРОЖНІЙ — і разом зі сканером вище це і є найсильніше твердження
       пакета: «код ⊆ список» + «список = ∅» дає «жодної підписки без фільтра».
       Окремого тесту з такою назвою тут СВІДОМО немає: він був би зеленим за
       побудовою цих двох, тобто підписом ширшим за доказ. */
    expect(UNFILTERED_ALLOWED).toEqual({});
  });
});
