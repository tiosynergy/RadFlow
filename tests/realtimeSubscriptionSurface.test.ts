 ...
 * колонок первинного ключа — але ТІЛЬКИ В ГІЛЦІ DELETE, щойно на таблиці
 * ввімкнено RLS:
 *
 *     and ( not is_rls_enabled or (c).is_pkey )
 *       -- if RLS enabled, we can't secure deletes so filter to pkey
 *
 * ⚠️ ЩО ТУТ СТОЯЛО НЕПРАВДИВОГО (виправлено в с57, U-66). Було: «у гілці
 * UPDATE такої умови немає — там `old_record` повний, але доставку UPDATE
 * політика перевіряє, тож отримувач і так має право бачити рядок». Друга
 * половина фрази — хибна, і саме через неї U-66 колись і завели: політика
 * рахується по НОВІЙ версії рядка, тож право бачити НОВУ вона доводить, а
 * право бачити СТАРУ — ні. UPDATE, який ВВОДИТЬ рядок у видимість підписника
 * (проставили `referrer_id`, змінили `room_id` радіолога), віддає йому повний
 * ПОПЕРЕДНІЙ стан — стан, якого він не мав права читати.
 * Розбір і чому наївне лікування робить гірше — `docs/audit/PR-U66-*.md`.
