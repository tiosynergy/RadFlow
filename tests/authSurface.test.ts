 ...
 * `app/api/**` і `/fhir/**` — але ⚠️ ЦЕ ВЖЕ НЕ МЕЖА ПРОЄКТУ, а лише межа
 * ЦЬОГО файлу: серверну поверхню закрив пізніший сторож
 * `tests/serverAuthSurface.test.ts`, який вимагає, щоб КОЖЕН роут під
 * `app/api/**` або мав гейт, або був НАЗВАНИЙ у `PRE_AUTH` разом із тим, що
 * його стереже замість ролі.
 *
 * ⚠️ ЧОМУ ЦЕЙ АБЗАЦ ПЕРЕПИСАНО (пакет 44, с59). Тут стояло «серверна поверхня
 * лишається поза цим сторожем і це НАЗВАНА межа» — і це стало НЕПРАВДОЮ в
 * момент появи `serverAuthSurface`, але проза лишилась. У пакеті 42 я прочитав
 * саме цей коментар, повірив йому і додав ПУБЛІЧНИЙ роут `/api/build`, не
 * знаючи про інвентар; спіймав мене не він, а сам сторож на першому повному
 * прогоні. Урок дослівно: сусідній файл — теж ДОКУМЕНТ, і застаріла межа в
 * ньому небезпечніша за відсутню, бо їй вірять.
 */
...
describe("поверхня авторизації — самі гейти", () => {
  ...
  /* ⚠️ Ф6-5 (с55). Знайдено, коли на новий екран-відмову додавали кнопку виходу:
     виявилось, що на `/setup` її НЕ БУЛО — а екран термінальний. Посилань із
     нього немає; `/queue` при ненастроєному центрі веде назад на `/setup`;
     middleware жене залогіненого з `/login` на `/queue`. Реєстратор у щойно
     зареєстрованому центрі (звичайний онбординг: центр створили, реєстратора
     завели, майстер не добігли) опинявся в петлі без жодного способу з неї
     вийти. Замір прода 03.09.2026: ненастроєних центрів 0, тобто сьогодні в
     петлі не сидить НІХТО — дефект живий за побудовою, а не за даними.

     Правило жило у ТРЬОХ місцях із чотирьох (`/radiologist`, `/referral`,
     `not-found`) і було забуте в четвертому — дослівно клас «дефект живе там,
     де правило є і про нього забули». Тому тут не перепис, а ВЛАСТИВІСТЬ, знята
     з дерева: повноекранна панель `minHeight: "100vh"` — це термінальний екран,
     і вона мусить давати вихід (розлогін або посилання). Новий такий екран без
     виходу червонить сторожа. */
  it("термінальні екрани дають вихід — інакше людина замкнена в петлі", () => {
    const files: string[] = [];
    ...
    const terminal = files.filter((f) => src(f).includes('minHeight: "100vh"')).sort();
    expect(terminal, "склад повноекранних панелей змінився — перечитайте, чи кожна дає вихід").toEqual([
      "app/not-found.tsx",
      "app/radiologist/page.tsx",
      "app/referral/page.tsx",
      "app/setup/page.tsx",
      "components/RoleNotice.tsx",
    ]);
    for (const f of terminal) {
      const s = src(f);
      expect(/<SignOutButton/.test(s) || /<Link\s+href=/.test(s),
        `${f}: повноекранний екран без виходу — ні розлогіну, ні посилання; людина на ньому замкнена`).toBe(true);
    }
  });
...
/** Те саме, що `codeOf`, але зі ЗБЕРЕЖЕНОЮ кількістю рядків.
 *
 * ⚠️ Знайдено в с57, під час правки піна нижче. `codeOf` схлопує блок
 * коментаря в ОДИН пробіл — для пошуку підрядка це байдуже, але два сторожі в
 * цьому файлі ходять по рядках і друкують `файл:рядок`. Після першого ж
 * блок-коментаря номер їхав на стільки рядків, скільки той займав, і читача
 * слало не туди: у цьому ж проєкті таку саму помилку вже ловило ревʼю U-61
 * (там було −283 рядки). Друга, тихіша шкода: схлопування СКЛЕЮЄ код до і
 * після коментаря в один рядок — і породжує збіги, яких у коді немає.
 * Тут замінюємо тіло коментаря на рівно стільки ж переносів. */
function codeLines(src: string): string[] {
  return src
    .replace(/\/\*[\s\S]*?\*\//g, (m) => " ".repeat(1) + "\n".repeat((m.match(/\n/g) || []).length))
    .replace(/(^|[^:])\/\/.*$/gm, "$1")
    .split(/\r?\n/);
}
...
describe("RF-4 — роль не підставляється замість відсутньої", () => {
  ...
  it("імʼя ролі ніколи не стоїть праворуч від `||` чи `??`", () => {
    ...
    for (const f of APP_SRC) {
      codeLines(readFileSync(resolve(process.cwd(), f), "utf8")).forEach((line, i) => {
  ...
  /* ⚠️ ЦЕ ПІН ПО СПИСКУ, а не сканер, і я не вдаю інакше. «Повнота через тип»
     застосовується до конкретних пропів свідомо; вивести цей список із коду
     властивістю не можна — «обовʼязковий проп» сам по собі не ознака ідіоми.
     Тому кожен рядок нижче — окреме рішення, і наступний, хто застосує прийом,
     має дописати сюди свій проп. Список вужчий за дерево за побудовою; ціна
     названа, щоб її не переплутали з доказом. */
  const TYPE_COMPLETE_PROPS = [
    /* с57, RF-4: без ролі екран отримував АДМІНСЬКЕ меню. */
    { prop: "roleKey", why: "компонент без ролі знову збирається — меню поїде за типовим значенням" },
    /* с57, U-65: без центрів підписка бейджа листа була БЕЗ фільтра, тобто
       крос-тенантним оракулом. Типове значення `[]` не поверне оракул (канал
       просто зникне), але поверне тихий екран без живого лічильника — і зробить
       це на кожному новому виклику, який забув проп. */
    { prop: "clinicIds", why: "виклик без центрів знову збирається — бейдж листа мовчки перестане оновлюватись" },
  ];

  it.each(TYPE_COMPLETE_PROPS)("$prop — обовʼязковий проп, без типового значення", ({ prop, why }) => {
    ...
    const reOpt = new RegExp(prop + "\\?\\s*:");
    /* Типове значення В ДЕСТРУКТУРИЗАЦІЇ: `prop = []`, `prop = "admin"`, `prop = {}`.
       ⚠️ Ширше, ніж було до с57. Там стояло `prop\s+=\s*["']` — лише РЯДКОВИЙ
       літерал, і проп-масив отримав би `= []` мовчки.
       ⚠️ І вужче, ніж моя перша спроба розширення (заміряно — 14 і 10 хибних
       спрацювань): вона ловила ще й JSX-атрибут `prop={…}` на кожному виклику
       та звичайне присвоєння `clinicIds = r.clinicIds.filter(…)` у
       lib/searchContract.ts. Розрізняємо трьома ознаками, кожна перевірена на
       реальних рядках цього дерева:
         • проп СТОЇТЬ НА ПОЧАТКУ рядка (у сигнатурі він на своєму рядку);
         • перед `=` є ПРОБІЛ — у JSX його не буває (`prop={x}`);
         • рядок НЕ закінчується `;` — це відрізняє елемент списку параметрів
           від оператора присвоєння. */
    const reDef = new RegExp("^\\s*" + prop + "\\s+=\\s*(?![=>])");
    for (const f of APP_SRC) {
      codeLines(readFileSync(resolve(process.cwd(), f), "utf8")).forEach((line, i) => {
        if (reOpt.test(line)) optional.push(`${f}:${i + 1}`);
        if (reDef.test(line) && !/;\s*$/.test(line)) defaulted.push(`${f}:${i + 1}`);
      });
    }
