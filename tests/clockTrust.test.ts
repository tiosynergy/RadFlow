...
describe("текст відмови", () => {
  it("називає причину і ОБИДВА виходи — інакше оператор замкнений", () => {
    expect(CLOCK_SKEW_MSG, "текст перестав називати причину — оператор читає відмову без пояснення").toContain("годинник");
    expect(CLOCK_SKEW_MSG, "зник перший вихід (повторити) — а він спрацьовує майже завжди").toContain("ще раз");
    expect(CLOCK_SKEW_MSG, "зник другий вихід (обрати дату вручну) — при мовчазному вимірювачі оператор замкнений").toContain("вручну");
  });
});
