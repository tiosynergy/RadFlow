...
describe("Г1-F — проводка гарда в шляхах запису", () => {
  ...
  it("портал направника будує заявку зоною ЦЕНТРУ, а не своєю", () => {
    ...
    /* с81: ДРУГА заявка порталу — перенос перетягуванням (`dropMoveReferral`).
       Звірено з правилом: доба ЦІЛІ — вибір людини (кидок на слот дня карти
       чи дока), зсув 0, зона ЦЕНТРУ запису; `pinnedKey` немає (F3). */
    expect(s, "заявка переносу перетягуванням будується не по добі цілі або не зоною центру")
      .toMatch(/clock: clockClaimOf\(\{ clinicTz: tz, curKey: target\.dateKey \}\)/);
    expect(s.split("clockClaimOf(").length - 1, "кількість заявок у порталі змінилась — нову точку ніхто не звіряв із правилом").toBe(2);
  });

  it("с81: перенос перетягуванням на дошці черги везе заявку по добі ЦІЛІ", () => {
    /* Док кидає на день дошки (та сама доба, що в `boardClock()`), карта дня — на
       день, обраний у календарі. Обидва шляхи йдуть через ОДИН `dropMove`, і
       заявка в ньому — по `target.dateKey` зі зсувом 0: предикат той самий, що
       в `useFollowToday` дошки, тож сервер судить рівно про ту дату, яку одержує. */
    const s = src("components/QueueBoard.tsx");
    expect(s, "заявка переносу перетягуванням будується не по добі цілі")
      .toMatch(/clock: clockClaimOf\(\{ clinicTz, curKey: target\.dateKey \}\)/);
    expect(s.split("clockClaimOf(").length - 1, "кількість заявок на дошці змінилась — нову точку ніхто не звіряв із правилом").toBe(2);
  });
...
/* ===== ВІДМОВА ГАРДА МУСИТЬ БУТИ ВИДИМОЮ (пакет 63, с63) ==================

   ⚠️ ЗАПЛАЧЕНО ЖИВИМ ПРОГОНОМ 11.09.2026. Гард відпрацював бездоганно —
   і оператор не побачив НІЧОГО: кнопка «Додати до листа» мовчала, запис не
   створювався, пояснення не було. Причина: батько показував відмову через
   `notify()`, тобто тостом на ДОШЦІ, а дошка лежить під оверлеєм модалки
   (z-index 100 проти 200). Текст `CLOCK_SKEW_MSG` писався так, щоб назвати
   причину і вихід, — і не доходив до людини взагалі.

   Це вже лагодили одного разу, сусідній модалці (`BookingModal.onSave`
   повертає ТЕКСТ рівно з цієї причини). Пін нижче тримає обидві половини:
   модалка вміє ПОКАЗАТИ, а батьки вміють ВІДДАТИ. Половина без половини
   безсила: покажчик без тексту мовчить, текст без покажчика тоне. */
describe("Г1-F — відмова видима в модалці листа, а не тоне під оверлеєм", () => {
  it("WaitlistModal показує помилку сервера У СЕБЕ", () => {
    const s = src(WM);
    expect(s, "модалка не тримає стан помилки — показувати нічого")
      .toMatch(/const \[saveErr, setSaveErr\] = useState<string \| null>\(null\)/);
    expect(s, "результат onSave нікуди не потрапляє — текст губиться мовчки")
      .toMatch(/const err = await onSave\(\{/);
    expect(s, "текст не доходить до стану — модалка й далі мовчатиме")
      .toMatch(/if \(err\) setSaveErr\(err\)/);
    /* Вузол і клас — ті самі, що в BookingModal: дві різні подачі одного класу
       відмов розходяться мовчки. */
    expect(s, "вузла помилки в розмітці немає — стан є, а на екрані нічого")
      .toMatch(/\{saveErr && \( <div className="dlg-err" role="alert">⚠ \{saveErr\}<\/div> \)\}/);
  });

  it("контракт onSave більше не `=> void`: інакше текст нема чим повернути", () => {
    expect(src(WM), "тип повернення звузили назад — батьки знову змушені кликати notify")
      .toMatch(/onSave: \(w: WaitlistFormOut\) => void \| Promise<void> \| Promise<string \| null>;/);
  });

  /* ⚠️ ДРУГА ПОЛОВИНА, і без неї перша нічого не варта: батько, що й далі
     кличе `notify` замість `return`, залишає ту саму невидиму відмову. */
  it("усі три батьки ВІДДАЮТЬ текст, а не топлять його в тості", () => {
    for (const [file, fn, label] of [
      ["components/WaitlistBoard.tsx", "onAdd", "додавання з дошки листа"],
      ["components/WaitlistBoard.tsx", "onEditSave", "правка з дошки листа"],
      ["components/ReferralPortal.tsx", "wlAdd", "додавання з порталу направника"],
      ["components/ReferralPortal.tsx", "wlEditSave", "правка з порталу направника"],
    ] as const) {
      const whole = src(file);
      const from = whole.indexOf(`async function ${fn}(`);
      expect(from, `${label}: обробника ${fn} не знайдено — пін тримає порожнечу`).toBeGreaterThan(-1);
      /* Тіло ріжемо до НАСТУПНОГО оголошення: файловий пошук знаходив би
         сусідній обробник і лишався зеленим — рівно пастка M18 вище. */
      const rest = whole.slice(from + 10);
      const next = rest.indexOf("async function ");
      const body = next > -1 ? rest.slice(0, next) : rest;
      expect(body, `${label}: відмова й далі йде в notify — тост потоне під оверлеєм`)
        .not.toMatch(/if \(!res\.ok\) \{ notify\(/);
      expect(body, `${label}: текст відмови не повертається модалці`)
        .toMatch(/if \(!res\.ok\) return res\.error;/);
    }
  });
});
