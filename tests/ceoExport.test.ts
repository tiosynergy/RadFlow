...
describe("CSV: захист від формульної інʼєкції", () => {
  ...
  /* Ревʼю с80 р2, L-1: `;` — НАШ роздільник; з очищеним обмежувачем тексту
     «Іванов;=2+3» у LibreOffice теж давав живу формулу. */
  it("L-1 с80 р2: знак формули одразу після `;` — теж з апострофом; `;` у тексті — без змін", () => {
    expect(csvCell("Іванов;=2+3")).toBe('"Іванов;\'=2+3"');
    expect(csvCell("a; +1;@x")).toBe('"a; \'+1;\'@x"');
    expect(csvCell("а;б")).toBe('"а;б"');
    expect(csvCell("після 14:00; уточнити")).toBe('"після 14:00; уточнити"');
  });
