 ...
 *  ⚠️ 0182 додала ОДИН (20 → 21): `zz_invite_issued_at`, штамп часу видачі
...
const GUARDS: ReadonlyArray<readonly [string, string]> = [
  ...
  /* 0182 (RF-02): штамп часу видачі запрошення. У списку — щоб зняття або
     вимкнення тригера червонило: без нього TTL мовчки перестав би діяти,
     а №17 бачить саме НАЯВНІСТЬ і `tgenabled`. Тіло стереже №19. */
  ["profiles", "zz_invite_issued_at"],
...
describe("№17 guard_triggers — інвентар гардів у сторожі", () => {
  ...
  it("усі 21 пара названа ПАРОЮ (таблиця, тригер) і з повним визначенням", () => {
