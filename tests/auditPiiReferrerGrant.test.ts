 ...
 *    до 0204 не пінив (з 0204 — гілка `order:`, див.
 *    `tests/guardTriggersInvariant.test.ts`), тому тут — статична симуляція
 *    всіх міграцій: жоден BEFORE-тригер цих таблиць не стоїть за абеткою після
 *    `zz_guard_read_keys`. Симуляція лишається і з 0204: ловить порушника ще в
 *    CI, до накату;
...
describe("0203 — сім тригерів і їхній порядок", () => {
  ...
  it("L4: `zz_guard_read_keys` — ОСТАННІЙ BEFORE-тригер кожної з трьох таблиць у сумі ВСІХ міграцій", () => {
    /* Порядок спрацювання — за імʼям (C). BEFORE-тригер, що стоїть за абеткою
       ПІСЛЯ гарда і править ключ, обійшов би його мовчки; №17 порядку до 0204
       не пінив (з 0204 — гілка `order:`, але вона бачить лише прод, а ця
       симуляція — ще міграцію в PR). */
