...
describe("сторож СЬОГОДНІ (останній передрук, а не заморожений 0166)", () => {
  ...
  it("priv_drift не зникла з живого сторожа", () => {
    /* ⚠️ ПІНИМО ФОРМУ ЗВІТУ, А НЕ ПРОСТО ІМʼЯ (0174). Доти мітка траплялась у
       тілі один раз, і шукати «'check', 'priv_drift'» було те саме, що шукати
       звіт. 0174 обгорнув перевірки обробниками винятків, і кожна мітка
       зустрічається ДВІЧІ — у звіті й у своєму обробнику. Пін по голому імені
       став хибно-зеленим: стенд `falsify-0166` показав це вимірюванням —
       мутація «перевірка тихо зникла з передруку» перейменовувала звіт, а пін
       далі знаходив ім'я в обробнику. Тому пін тримає САМЕ звітну форму
       (`'offenders', to_jsonb(v_tmp)`), якої в обробнику немає. */
    expect(latest.body, `${latest.file}: перевірка priv_drift зникла з передруку`)
      .toMatch(/'check',\s*'priv_drift',\s*'offenders',\s*to_jsonb\(v_tmp\)/);
  });

  it("не загублено жодної попередньої перевірки", () => {
    /* ⚠️ ПОВНЕ ІМʼЯ І ЗВІТНА ФОРМА, а не підрядок (0174). Список тримав куски
       імен (`security_invoker` замість `views_security_invoker`) і шукав їх
       голим `toContain` — поки мітка траплялась у тілі один раз, цього
       вистачало. 0174 обгорнув перевірки обробниками винятків: кожне імʼя тепер
       зустрічається ДВІЧІ (замір: гола форма 2, звітна 1). Мутація «перевірка
       тихо зникла з передруку» перейменовувала ЗВІТ, а пін далі знаходив імʼя
       в обробнику і мовчав — це виміряв стенд `falsify-0166` (позиція N16,
       «сторож дивиться не туди»). Тому пін тримає звітну форму
       `'offenders', to_jsonb(v_tmp)`, якої в обробнику немає. */
