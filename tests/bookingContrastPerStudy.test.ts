...
describe("с47: чекбокс контрасту — без тексту, у всіх формах", () => {
  ...
  it("CSS: поле «Контраст» у ряду має явний flex і не стискається", () => {
    const css = raw(CSS);
    /* ⚠️⚠️ Селектор мусить бути СИЛЬНІШИЙ за `.fld-row .fld { flex: 1 }`
       (0,2,0), яке живе нижче в цьому ж файлі. Проста форма `.bk-fld-contrast`
       (0,1,0) програє і за специфічністю, і за порядком — і мовчки не діє:
       на проді це виглядало рівно як відсутнє правило, поле 0px. */
    expect(css, "правило поля «Контраст» ослабили до .bk-fld-contrast — воно програє .fld-row .fld { flex: 1 }")
      .toMatch(/\.fld-row > \.fld\.bk-fld-contrast \{[^}]*flex: 0 0 auto/);
