...
/* ⚠️ ПʼЯТИЙ КАНАЛ (с72, пакет 0198) — і він про ІНШУ властивість, ніж чотири
 *    вище. Ті стережуть ЧИСЛО перевірок. Цей стереже, що передрук сторожа
 *    взагалі КЛАДЕ пін, який читає перевірка №25 у проді.
 *
 *    ⚠️ ТУТ НАВМИСНО НЕМАЄ ЗВІРКИ md5 ТІЛА З ПІНОМ, і причина заміряна, а не
 *       уявна. Перша редакція (с72) саме так і робила — рахувала md5 усього
 *       тіла з файлу і порівнювала з піном. Повна ревізія показала ціну:
 *       `falsify-0180` і `falsify-0181` стали ЧЕРВОНИМИ на своїх ПОЗИТИВНИХ
 *       контролях (T1 «переставлено два рядки списку», T2 «переписано
 *       коментар усередині блоку»). Ці мутації зобовʼязані лишатись ЗЕЛЕНИМИ:
 *       стенди ними доводять, що сторожі не стріляють на рефакторі. Сума по
 *       ВСЬОМУ тілу стріляє на будь-якій косметиці — тож у наборі, яким
 *       стенди міряють сторожів, їй не місце.
 *
 *    ДЕ ВОНА ТЕПЕР: у гейті міграцій (`planGuardPin` у
 *    `scripts/migration-gate-lib.mjs`), поруч зі звіркою «md5 файлу ↔ леджер» —
 *    тобто серед контрольних сум, де їй і місце. Гейт вшито в `npm run build`,
 *    на Vercel обходів не має, тож властивість тримається fail-closed на КОЖНІЙ
 *    прод-збірці, а не лише тоді, коли хтось запустив тести.
 *
 *    ЩО ЛИШИЛОСЬ ТУТ: найімовірніша помилка — передрукувати сторожа і ЗАБУТИ
 *    пін узагалі. Її видно без суми по тілу, вона стенди не чіпає, і про неї
 *    краще дізнатись на коміті, а не на збірці.
 */
describe("передрук сторожа зобовʼязаний класти пін (0198)", () => {
  ...
  it("найсвіжіший передрук несе пін у формі, яку читає №25", () => {
    ...
    expect(
      PIN_RE.test(txt),
      `${file} передруковує сторожа, але не кладе \`comment on function public.invariants_check(boolean)\` ` +
        `у формі guard_body_md5=<32 hex>;len=<N>. Це зобовʼязання з 0198: інакше перевірка №25 ` +
        `почервоніє одразу після накату. Пін рахує генератор передруку — перезберіть його.`,
    ).toBe(true);
  });

  it("форма піна тут — та сама, що читає перевірка №25 у проді", () => {
    /* ⚠️ Без цього рядка обидві форми могли б розійтись мовчки: тест приймав би
       пін, який прод вважає НЕЧИТАНИМ, і «пін є» означало б «пін є, але
       мертвий». Пін дослівний — по тілу сторожа, а не по переказу. */
    const txt = readFileSync(resolve(MIGDIR, latestReprintFile()), "utf8");
    expect(
      txt,
