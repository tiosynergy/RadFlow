...
describe.each(BOARDS)("%s — clash названий, а не «підтверджуваний»", (_name, src) => {
  it("inProgressBlockReason має гілку clash із причиною", () => {
    ...
    /* ⚠️ ДРУГА ПОЛОВИНА ІМЕНІ (с52, U-80б). Досі тут стояв ОДИН ассерт — на
       наявність літерала `r.code === "clash"`, — а назва обіцяла «із причиною».
       Різниця не схоластична: лишити гілку і повернути з неї `null` — це
       ДОСЛІВНО дефект, який стенд описує як «дошка знову віддає null на clash,
       кнопка мовчить», і жодна перевірка набору його не бачила. Знайдено
       ревʼю пакета U-80б: мутація стенда прибирала САМ літерал, тож сторож
       червонів «правильно» і виглядав достатнім.
       Пінимо саме те, що робить гілку осмисленою: вона ПОВЕРТАЄ рядок. Текст
       не пінимо навмисно — формулювання не контракт (див. T1 у стенді). */
    expect(src, "гілка clash є, але вона більше не повертає причини — кнопка знову мовчить")
      .toMatch(/r\.code === "clash"\)\s*\{?\s*\n?\s*return `/);
  });

  /* ⚠️ ПЛИТКА КАБІНЕТУ — ДРУГА ПОВЕРХНЯ ВИКЛИКУ, і до 10.09.2026 вона
     показувала ПРОТИЛЕЖНИЙ вердикт рядку черги. Заміряно живою перевіркою
     Ф4-2: о 13:44:58 рядок погас із причиною «о 14:15 наступний запис», а
     плитка о 13:46:03 усе ще світила активною синьою кнопкою. Дірки в даних
     не було (`callPatient` перечитує блок і віддає тост), але кнопка обіцяла
     дію, якої не існує — рівно клас U-67, уже чинений один раз.

     ⚠️ ЧОМУ СТОРОЖ САМЕ ТУТ І САМЕ ПО ОБИДВОХ ДОШКАХ. `RoomStatusCard` існує
     у ДВОХ копіях — у `QueueBoard` і в `RadiologistBoard`, — і правило вже
     один раз застосували в одній копії й забули в іншій. Правило, яке
     тримається на дисципліні, буде забуте втретє; тримати мусить набір. */
  it("плитка кабінету гасить кнопку виклику за callBlockReason", () => {
    expect(src, "у пропах картки немає callBlockReason — плитка знову не знає про блоки")
      .toMatch(/callBlockReason\?:\s*string \| null/);
    expect(src, "кнопка плитки не читає callBlockReason у disabled — вона знову обіцяє дію, якої немає")
      .toMatch(/disabled=\{[^}]*callBlockReason\}/);
    expect(src, "причина не доїхала в title — оператор не дізнається, ЧОМУ кнопка мертва")
      .toMatch(/title=\{callBlockReason \|\|/);
  });

  /* Проп можна оголосити, прочитати в кнопці — і не передати з дошки. Тоді всі
     три пінрядки вище зелені, а кнопка світиться як раніше: `undefined` не
     блокує. Саме так дефект і виглядав. */
  it("дошка ПЕРЕДАЄ причину в плитку, а не лише оголошує проп", () => {
    expect(src, "callBlockReason не передається у RoomStatusCard — проп оголошено й забуто")
      .toMatch(/callBlockReason=\{[^}]*inProgressBlockReason\(/);
  });
