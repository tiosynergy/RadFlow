...
const PRE_AUTH: Record<string, string> = {
  ...
  /* Пакет 42. ⚠️ Незахищеність тут — НЕ недогляд, а ВЛАСТИВІСТЬ приладу:
     він мусить відповідати тоді, коли зламано інше. Гейт `requireRole`
     потягнув би за собою і service-role ключ, і Auth, і `profiles` —
     відмова будь-чого з цього зробила б відповідь неоднозначною («стара
     збірка чи впав Auth?»), тобто вбила б рівно те, заради чого прилад
     існує. Читати нема чого: у БД не ходить, сесію не бере. */
  "/api/build": "штамп збірки sha256(commitSha)[:12]: у БД не ходить, сесію не читає, віддає 12 hex і слово production. Замість ролі його стереже те, що читати нічого — приватний SHA з штампа не відновлюється",
};
