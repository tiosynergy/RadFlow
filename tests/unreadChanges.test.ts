...
describe("U-37: мітла позначок покриває всі типи сутностей (0164)", () => {
  ...
  const guardSrc = (): { fn: string; file: string; prevChecks: number } => {
    let best = { fn: "", file: "", prevChecks: 0 };
    for (const f of files) {
      const txt = readFileSync(resolve(dir, f), "utf8");
      /* ⚠️ Якір на ПОЧАТОК РЯДКА (ревʼю Д, с51): міграції ЗГАДУЮТЬ сторожа в
         закоментованому відкаті — 0167 робить саме так. Гола підрядка колись
         навела б і цей тест, і стенд `falsify-u37` на файл без тіла сторожа.
         Відсутність терміналізатора тепер `continue`, а не ассерт: файл, у
         якому сторож лише згаданий, — не передрук, і валити на ньому весь
         статичний ярус було б хибним звинуваченням. */
      const at = txt.search(/^create or replace function public\.invariants_check/m);
      if (at < 0) continue;
      const end = txt.indexOf("\n$function$;", at);
      /* ⚠️ ГУЧНО, а не `continue` (ревʼю с56). Обґрунтування вище протухло:
         якір на ПОЧАТОК РЯДКА уже відсіює файли, де сторож лише ЗГАДАНИЙ, тож
         сюди доходить справжній передрук — і тиша означала б відкат на
         ПОПЕРЕДНІЙ. Міграція, що закриє тіло іншим доларовим тегом, лишила б
         увесь статичний ярус на старому числі: зелено в наборі, червоно в
         проді. ⚠️ БОРГ: `latestReprint()` у `scripts/falsify-u37.mjs` досі
         мовчить — правити стенд посеред пакета означало б протухлі якорі;
         поки що тести падають ПЕРШИМИ і не пускають коміт. */
      if (end < 0) throw new Error(`${f}: передрук invariants_check не закритий "$function$;"`);
      /* ⚠️ МАКСИМУМ ПО ВСІХ ПОПЕРЕДНІХ передруках — і кожне слово тут коштує
         окремо (с51).
         Пін нижче стояв на константі `>= 14`, замороженій у с49, тоді як живих
         перевірок уже 15: «монотонність» перетворилась на межу, під якою можна
         мовчки втратити перевірку. Мутація N10 стенда u37 (зняти один
         `v_n := v_n + 1;`) давала 14 — і сторож, названий її сторожем, лишався
         ЗЕЛЕНИМ.
         ⚠️ Перша редакція фіксу брала число ЛИШЕ з попереднього передруку, і
         ревʼю Д показало, що це храповик на один крок, а не храповик: міграція,
         яка впустила число до 14, робила поріг 14 для НАСТУПНОЇ — тобто втрату
         можна було «відмити» другим передруком. Максимум по всіх попередніх
         такої стелі не має: поріг тільки росте. */
      best = {
        fn: txt.slice(at, end),
        file: f,
        prevChecks: Math.max(best.prevChecks, (best.fn.match(/v_n := v_n \+ 1;/g) || []).length),
      };
    }
    return best;
  };
  ...
  it("сторож дізнався про мітлу: 14-та перевірка на місці і стереже ОБИДВІ половини", () => {
    ...
    const { fn, file, prevChecks } = guardSrc();
    ...
    /* ⚠️ МЕЖА РУХАЄТЬСЯ САМА (с51). Було `>= 14` — число, заморожене в с49; за
       два передруки живих стало 15, і «монотонність» перетворилась на межу, під
       якою перевірку можна втратити мовчки. Стенд u37 це й показав: мутація N10
       давала 14, і названий нею сторож лишався зеленим.
       Тепер порівнюємо з ПОПЕРЕДНІМ передруком; `>= 14` лишається нижньою
       страховкою на випадок, якщо передрук у проєкті колись стане першим. */
    /* ⚠️ Рахуємо по КОДУ без коментарів (ревʼю с56): `-- v_n := v_n + 1;`
       інакше зараховується як живий крок, і мутація «закоментувати шаг»
       лишає статично 18 при 17 у проді — зелений набір, девʼять червоних
       смоуків. Знімаємо лише для ЛІЧБИ: піни нижче дивляться на `fn` цілком,
       бо `--` трапляється і всередині SQL-літералів (`'--[^' || chr(10)`). */
    const fnCode = fn.replace(/\/\*[\s\S]*?\*\//g, " ").replace(/--[^\n]*/g, " ");
    const steps = (fnCode.match(/v_n := v_n \+ 1;/g) || []).length;
    expect(steps, `${file}: у передрукованому сторожі поменшало перевірок — щось загубили`)
      .toBeGreaterThanOrEqual(Math.max(14, prevChecks));
    /* ⚠️ СЛІПА ЗОНА МОНОТОННОСТІ (с55, знайдено ревізією після 0170). Поріг
       «не менше за попередній передрук» має дірку рівно на один крок у ТОМУ
       передруку, який додає перевірку: 0170 підняв 15 → 16, і мутація N10
       стенда u37 (зняти один `v_n := v_n + 1;`) давала 15 ≥ 15 — сторож,
       названий її сторожем, знову лишався зеленим, а ловив її сусід про піни.
       Це та сама поломка, що й у с51, тільки поверхом вище: межа рухається,
       але наздоганяє передрук на крок пізніше.
       Ліки — не число, а СТРУКТУРА: кожна названа перевірка (`'check', '…'`)
       мусить мати власний крок лічильника. «Перевірку додали, а лічильник не
       зріс» — це саме розрив між цими двома числами, і він видимий у будь-якому
       передруку без знання, скільки їх «має бути». */
    /* ⚠️ РАХУЄМО РІЗНІ ІМЕНА, А НЕ ВХОДЖЕННЯ (0174). Доти мітка перевірки
       ...
       рахувати. */
    const labels = new Set(fnCode.match(/'check',\s*'[a-z0-9_]+'/g) || []).size;
    expect(steps, `${file}: названих перевірок ${labels}, а кроків лічильника ${steps} — якась перевірка є, а в 'checked' не рахується`)
      .toBe(labels);
