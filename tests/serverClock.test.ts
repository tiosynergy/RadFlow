...
describe("межі правки названі й дотримані", () => {
  ...
  it.each([["components/QueueBoard.tsx"], ["components/RadiologistBoard.tsx"]])(
    ...
    (file) => {
      ...
      /* ⚠️ ПІН РОЗВЕРНУТО в с55 (F3), і це не послаблення сторожа, а виправлення
         його ЗМІСТУ. Він вимагав `pinnedKey: initialDate` під підписом «дата з
         deep-link захищена» — але вимір (зонд с55, P7) показав, що перша умова
         `derivedFromToday` і без піна не чіпає ЖОДНОЇ чужої дати, тобто пін
         «захищав» рівно один стан: дип-лінк на СЬОГОДНІ. А там він шкодив —
         глушив і правило (мовчазний архів), і відмову Г1-F на інлайн-переносах
         (P5a/P6a). Рішення власника: дошки слідують за поправкою. Тепер сторож
         вимагає ВІДСУТНОСТІ піна — повернення `pinnedKey` на дошку червонить
         саме цей рядок. */
      expect(s, "F3: `pinnedKey` повернувся на дошку — дип-лінк на сьогодні знову глушить правило і гард Г1-F")
...
/* ===== F6 (с55) — вартовий СИРОГО годинника =====

   КЛАС ДЕФЕКТУ, виміряний зондом с55 (позиції P13/P14). `serverNow()` рухають
   ДВІ величини — `Date.now()` і `_offsetMs`, — а `clockEpoch()` реагував лише
   на другу. Перевели годинник ПК на добу вперед: `wallDayKeyAt(serverNow())`
   змінився, епоха і слухачі — ні. Плюс прийнята оцінка не має терміну
   придатності: без нової проби вона живе скільки завгодно (`stale` вирішує
   лише, чи ПРИЙНЯТИ нову), а шапка модуля стверджувала протилежне. */
describe("observeWallStep — виявлення кроку сирого годинника", () => {
  it("ПЕРШЕ спостереження порівнювати нема з чим — завжди false", () => {
    expect(observeWallStep(1_000_000, 500),
      "перше спостереження почало щось стверджувати — вартовий смикав би пробу на кожному монтуванні").toBe(false);
  });

  it("рівний хід: стінний і монотонний ідуть разом — тиша", () => {
    observeWallStep(1_000_000, 500);
    expect(observeWallStep(1_020_000, 20_500)).toBe(false);
    expect(observeWallStep(1_040_000, 40_500)).toBe(false);
  });

  it("годинник крокнув УПЕРЕД на добу — вартовий помітив", () => {
    observeWallStep(1_000_000, 500);
    expect(observeWallStep(1_020_000 + 86_400_000, 20_500)).toBe(true);
  });

  it("годинник крокнув НАЗАД — теж помітив (це напрямок «ПК спішив»)", () => {
    observeWallStep(1_000_000, 500);
    expect(observeWallStep(1_020_000 - 3_600_000, 20_500)).toBe(true);
  });

  it("дрібна розбіжність нижче порога — тиша (інакше проба на кожному тіку)", () => {
    observeWallStep(1_000_000, 500);
    expect(observeWallStep(1_020_000 + (CLOCK_STEP_MIN_MS - 1), 20_500)).toBe(false);
    /* Рівно поріг — теж тиша: умова строга. Межа названа, щоб її не «поправили»
       на >= випадково. */
    observeWallStep(2_000_000, 100_000);
    expect(observeWallStep(2_020_000 + CLOCK_STEP_MIN_MS, 120_000)).toBe(false);
  });

  it("сама ВЕЛИЧИНА порога закріплена зверху — літералом, а не собою", () => {
    /* ⚠️ ЗНАХІДКА РЕВʼЮ Б, і це той самий клас, що допуск Г1-F у с54: обидва
       зонди межі вище виражені ЧЕРЕЗ САМУ КОНСТАНТУ, тож вони їдуть за нею
       куди завгодно, а решта блоку бʼє абсолютними числами далеко зверху
       (86 400 000 і 3 600 000). Будь-яке значення в проміжку 2 с…58 хв лишало б
       увесь блок ЗЕЛЕНИМ — тобто поріг можна було мовчки РОЗШИРИТИ, і вартовий
       перестав би бачити звичайну корекцію NTP на десятки секунд.
       Тому тут ЛІТЕРАЛ і окрема рамка на величину. */
    observeWallStep(3_000_000, 200_000);
    expect(observeWallStep(3_020_000 + 5_000, 220_000),
      "крок у 5 секунд не помічено — поріг вартового задерли").toBe(true);
    expect(CLOCK_STEP_MIN_MS,
      "поріг вартового виріс: типова корекція NTP помітиться лише через RESYNC_MS").toBeLessThanOrEqual(5_000);
  });

  it("markClockStale змушує ПРИЙНЯТИ свіжу пробу з гіршим RTT — і не скидає зсув", () => {
    /* ⚠️ ЗНАХІДКА РЕВʼЮ А по цьому ж пакету: без цього виклику вартовий —
       декорація. Він будить пробу, а `applyClockEstimate` її відкидає, бо чинна
       оцінка ще молодша за CLOCK_STALE_MS і має кращий RTT. Розбір і числа — у
       шапці `markClockStale`. */
    expect(applyClockEstimate({ offsetMs: 480_000, rttMs: 30 })).toBe(true);
    /* ЗЕЛЕНА БАЗА: правило «гірша проба не заміняє свіжу кращу» живе. */
    expect(applyClockEstimate({ offsetMs: 60_000, rttMs: 150 }),
      "гірша проба замінила свіжу оцінку — правило відбору за RTT зникло").toBe(false);
    expect(clockOffsetMs()).toBe(480_000);
    markClockStale();
    expect(clockOffsetMs(), "протухлення СКИНУЛО зсув — показ повернувся на годинник ПК, який ми й виправляємо").toBe(480_000);
    expect(clockOffsetKnown(), "протухлення оголосило годинник невиміряним").toBe(true);
    expect(applyClockEstimate({ offsetMs: 60_000, rttMs: 150 }),
      "після протухлення свіжа проба все ще відкидається — вартовий не міняє нічого").toBe(true);
    expect(clockOffsetMs()).toBe(60_000);
  });

  it("нефінітні відліки не приймаються і НЕ роблять вартового ГЛУХИМ", () => {
    /* ⚠️ Назва цього тесту — твердження, і перша редакція його НЕ ДОВОДИЛА:
       вона перевіряла лише «наступне рівне спостереження = false», а це
       залишалось правдою і з отруєною парою (будь-яка різниця з NaN дає NaN, а
       `NaN > поріг` — теж `false`). Тобто зняття гарда пройшло б як успіх.
       Доводить саме глухоту наступний рядок: після зіпсованого виклику вартовий
       ЗОБОВʼЯЗАНИЙ побачити справжній крок. */
    observeWallStep(1_000_000, 500);
    expect(observeWallStep(NaN, 20_500)).toBe(false);
    expect(observeWallStep(1_020_000, Infinity)).toBe(false);
    expect(observeWallStep(1_020_000 + 86_400_000, 20_500),
      "після нефінітного виклику вартовий перестав бачити крок годинника — пара отруєна NaN").toBe(true);
    observeWallStep(2_000_000, 100_000);
    expect(observeWallStep(2_020_000, 120_000), "рівний хід після відновлення читається як стрибок").toBe(false);
  });

  it("вартовий НЕ крутить епоху і НЕ будить слухачів — це робота applyClockEstimate", () => {
    /* ⚠️ Це РІШЕННЯ, а не забутий виклик: сон ноутбука і переведення годинника
       цими двома відліками не розрізняються, а наслідки в них протилежні.
       Крутити епоху на сон означало б смикати добу під руками оператора там,
       де пакет Г1 вирішив цього не робити. */
    let hits = 0;
    const off = subscribeClock(() => { hits++; });
    observeWallStep(1_000_000, 500);
    const step = observeWallStep(1_020_000 + 86_400_000, 20_500);
    expect(step).toBe(true);
    expect(clockEpoch(), "вартовий сам крутнув епоху — доба поїде і після звичайного сну").toBe(0);
    expect(hits, "вартовий сам розбудив слухачів").toBe(0);
    /* ЗЕЛЕНА БАЗА: шлях, який РЕАЛЬНО крутить епоху, працює. */
    expect(applyClockEstimate({ offsetMs: 480_000, rttMs: 50 })).toBe(true);
    expect(clockEpoch()).toBe(1);
    expect(hits).toBe(1);
    off();
  });

  it("resetServerClock скидає і пару вартового", () => {
    observeWallStep(1_000_000, 500);
    resetServerClock();
    /* Після скидання це знову ПЕРШЕ спостереження — інакше сусідній тест
       успадкував би чужу пару і зловив вигаданий стрибок. */
    expect(observeWallStep(9_000_000, 900_000)).toBe(false);
  });
});

/* Місце вживання вартового. Компонентних тестів немає за задумом, тож без цих
   пінів зняття інтервалу лишило б увесь набір зеленим — той самий клас, що в
   Ф4-8 закривався пінами на StudyTimer. */
describe("ServerClockSync — проводка вартового (F6, с55)", () => {
  const s = () => src("components/ServerClockSync.tsx");

  it("вартовий не просто викликається, а ВЕДЕ до свіжої проби", () => {
    /* ⚠️ Пін тримає НАСЛІДОК, а не рядок: `observeWallStep(...)` без `void
       sync()` лишив би тут зелене, а вартовий став би декорацією. Це той самий
       урок, що в с54 (пін тримав рядок, а не відмову: `return X` → `void X`
       лишалось зеленим).
       ⚠️ І ЩЕ ОДИН УРОК, знайдений СТЕНДОМ у другому проході цього ж пакета.
       Перша редакція піна мала вікно `[\s\S]{0,300}` між входом вартового і
       `void sync();` — і мутація «замінити `void sync()` на `void 0`» лишала
       його ЗЕЛЕНИМ, бо у вікно потрапляв `void sync();` СУСІДА (обробник
       `visibilitychange` двадцятьма рядками нижче). Той самий клас, що
       `role="status"` у вікні 200 символів у с52 і сканер, який крав гейт у
       сусіда. Тепер — сувора СУМІЖНІСТЬ від унікального рядка вартового. */
    expect(s(), "вартовий не веде до перезаміру — крок годинника лишиться невиправленим до RESYNC_MS")
      .toMatch(/lastWatchSync = mono;\s*markClockStale\(\);\s*void sync\(\);/);
  });

  it("вартовий живе на ОКРЕМОМУ інтервалі і прибирається за собою", () => {
    expect(s(), "вартовий втратив власний інтервал").toMatch(/const w = setInterval\(/);
    expect(s(), "інтервал вартового не прибирається при розмонтуванні — таймер переживе екран")
      .toMatch(/clearInterval\(w\);/);
    /* ⚠️ ЗНАЙДЕНО СТЕНДОМ (другий прохід). Полагодивши пін на ВЕЛИЧИНУ
       `WATCH_MS` (знахідка ревʼю Б), я зняв старий слабкий пін і не помітив, що
       разом із ним пішло єдине, що тримало МІСЦЕ ВЖИВАННЯ: мутація
       `}, WATCH_MS);` → `}, RESYNC_MS);` лишала весь набір зеленим — константа
       ж не мінялась. Рамка на величину і пін на місце вживання охороняють різне,
       і потрібні обидва. */
    expect(s(), "інтервал вартового заведено на періоді перезаміру — вартовий нічого не пришвидшує")
      .toMatch(/\}, WATCH_MS\);/);
  });

  /* ⚠️ ЗНАХІДКА РЕВʼЮ Б. Попередній пін тут тримав ІМʼЯ `RESYNC_MS` усередині
     виклику (`.not.toMatch(/const w = setInterval\([\s\S]{0,200}RESYNC_MS\)/)`)
     — а мутація `WATCH_MS = 10 * 60 * 1000` це імʼя туди не пише, тож пін
     лишався ЗЕЛЕНИМ, і вартовий існував би, нічого не пришвидшуючи. Пінувати
     треба ВІДНОШЕННЯ величин, а не написання. Числа читаємо з джерела
     арифметикою над множниками — без `eval` і без другої копії констант. */
  const constMs = (src: string, name: string) => {
    const m = src.match(new RegExp("const " + name + " = ([^;]+);"));
    expect(m, `${name} зник із джерела — пін відношення більше нічого не тримає`).toBeTruthy();
    return m![1].split("*").reduce((acc, part) => acc * Number(part.trim()), 1);
  };

  it("крок вартового МЕНШИЙ за перезамір і не повільніший за базову лінію звуків", () => {
    const watch = constMs(s(), "WATCH_MS");
    const resync = constMs(s(), "RESYNC_MS");
    const gap = constMs(s(), "WATCH_MIN_GAP_MS");
    const tick = constMs(src("lib/useQueueSounds.ts"), "OVERRUN_TICK_MS");
    expect(watch, "крок вартового зрівнявся з перезаміром — вартовий нічого не пришвидшує").toBeLessThan(resync / 4);
    /* ⚠️ Обґрунтування кроку міряється НАЙШВИДШИМ споживачем знімків, а не
       дошкою: перша редакція шапки посилалась на тік дошки (20 с) і не помітила,
       що звук тікає вдвічі частіше (знахідка ревʼю А). */
    expect(watch, "вартовий став повільнішим за базову лінію звуків — обґрунтування кроку в шапці більше не тримається")
      .toBeLessThanOrEqual(tick);
    expect(gap, "нижня межа між пробами вартового зникла — на машині з агресивною синхронізацією це сотні RPC на годину")
      .toBeGreaterThanOrEqual(30_000);
  });

  it("вартовий ПРОТУХЛЮЄ чинну оцінку перед пробою — інакше пробу відкинуть", () => {
    /* ⚠️ ЗНАХІДКА РЕВʼЮ А: без `markClockStale()` свіжа проба з гіршим RTT
       відкидається, поки чинна оцінка молодша за CLOCK_STALE_MS, — і вартовий
       не міняє нічого саме в найтиповішому випадку (корекція NTP на десятки
       секунд). Пін тримає і ПОРЯДОК: протухлення ДО проби. */
    expect(s(), "вартовий не протухлює чинну оцінку — його проба буде відкинута як «гірша за свіжу»")
      .toMatch(/markClockStale\(\);[\s\S]{0,40}void sync\(\);/);
    expect(s(), "нижня межа між пробами вартового знята")
      .toMatch(/if \(mono - lastWatchSync < WATCH_MIN_GAP_MS\) return;/);
  });

  it("відліки знімаються ПАРОЮ з двох різних годинників", () => {
    /* Обидва з `Date.now()` не викрили б нічого: розбіжність рахується саме між
       стінним і монотонним. */
    expect(s(), "вартовий міряє одним годинником — розбіжності не буде ніколи")
      .toMatch(/observeWallStep\(Date\.now\(\), performance\.now\(\)\)/);
  });
});
