...
beforeEach(() => {
  ...
  rls.tables = {
    ...
    queue_entries: [
      q("e3", C1, R1, DAY, "11:00", "waiting", "Уявна Марія", "+380670000003", KNEE, "to_recall", "після 14:00; уточнити"),
...
describe("файл: область, порядок, колонки", () => {
  ...
  it("колонки: дата, час, ПІБ, телефон, процедура, кабінет (і вимкнений), підпис статусу, нотатка", async () => {
    const r = await exportCsv({ date: DAY });
    const rows = dataRows(r.text);
    /* Телефон з «+» — з апострофом: для Excel провідний «+» — це формула. Номер
       користувач може ввести довільно (zPhone без шаблону, до 32 символів), тож
       «+HYPERLINK(…)» у полі телефону — реальний вектор. Без апострофа Excel ще й
       перетворював «+380670000001» на ЧИСЛО 3,8E+11 (так було до с80) — тепер
       цифри цілі, а видимий апостроф — названа ціна (PR-s80, §2). */
    ...
    // Без досліджень — примітка запису; без кабінету — порожньо; без часу — порожньо.
    expect(rows[3]).toEqual([DAY, "", "Без Часу", "", "Консультація", "", "Не відповідає", ""]);
  });
...
describe("сторінки під стелею db-max-rows (урок M-1 с79)", () => {
  ...
  it("2500 записів дня за стелі 1000 — у файлі всі 2500, жодного дубля", async () => {
    ...
    expect(exportEvents()[0].p_details).toEqual({ source: "call_list", rows: 2500, format: "csv" });
  });
...
describe("журнал — подія patient_data.exported, fail-OPEN, без ПДн", () => {
  it("одна подія в журнал центру з РЯДКІВ: число рядків, джерело call_list, формат csv, актор — сесія", async () => {
    ...
    expect(ev[0]).toMatchObject({
      ...
      p_entity_type: "staff", p_entity_id: ADMIN, p_details: { source: "call_list", rows: 4, format: "csv" },
    });
    noPiiAnywhere();
  });
...
describe("текст відмови для тосту (callListExportErrorText)", () => {
  it("429 — гальмо; 403/400 — безпечна фраза роуту; решта — загальна", () => {
