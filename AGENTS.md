...
You are a Senior Full-Stack Engineer continuing work on RadFlow — a multi-tenant
SaaS for radiology queue management. Before doing anything, read the memory files
(MEMORY.md and especially TODO.md) and docs/PRODUCT_OVERVIEW.md +
docs/audit/FULL_AUDIT_2026-06-25.md. Here is the context you must operate with.
...
- Maintain multi-tenant isolation (clinic_id / RLS) — this is critical and security-sensitive.
...
- Referrers and CEOs are GLOBAL accounts (profiles.clinic_id = NULL); their membership
  to clinics lives in access tables: referral_access (referrers) and ceo_access (CEOs),
  each a row per (user, clinic) with status. A user can hold a role AND extra grants
  (e.g. a radiologist who is also a CEO via ceo_access — role is not changed).
...
- RLS read policies for global users are added as separate PERMISSIVE policies that OR with
  the base clinic_id policy. profiles_referrer_linked_read / profiles_ceo_linked_read carry a
...
- Admin can reset/set passwords via /api/staff/password (authorizes radiologists by clinic_id,
  CEOs via active ceo_access). Known bug (still open): this route's clinic_id check 403s for
  global REFERRERS — see TODO.

## Migrations
- Applied to prod MANUALLY via Supabase SQL editor (no automated migration runner).
...
- Keep migrations idempotent (do $$ … exception when duplicate_object …$$, create … if not exists,
  drop policy if exists before create policy).

## What was built/changed in the previous session (verify before assuming current)
...
- Full CEO ("Керівник") management feature: migration 0040_ceo_global.sql (ceo_access table,
...
- CeoDashboard.reload hardened with try/catch/finally (may be UNCOMMITTED — verify git status).

## Open work — see TODO.md for the live list
- Commit pending changes; run `npm run typecheck` (== tsc --noEmit; script was added) and
  `npm run lint`. Note: bare `tsc` is NOT on PATH — use npx or the npm script. `next lint` is
  deprecated and ESLint isn't configured yet (separate task).
...
- For browser testing, use the Claude-in-Chrome connector against the local dev server
  (`npm run dev`, localhost:3000). I do NOT enter passwords to authenticate — ask the user to
  log in as the needed role, then inspect.
...
Start by reading TODO.md and telling me the current open items and what you'd tackle first.
