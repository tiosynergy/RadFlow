...
| `invariants` | `50 3 * * *` | `select public.invariants_check()` — сторож інваріантів (**15 перевірок** з `0166`: `0164` підняв 13 → 14 `ucm_orphan_markers`, `0166` — 14 → 15 `priv_drift`), слід у `maintenance_runs` | міграція `0154`–`0167` |
...
доки хтось не виконає запит вище руками. Тобто **15 перевірок сьогодні
