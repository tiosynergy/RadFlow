...
| `invariants` | `50 3 * * *` | `select public.invariants_check()` — сторож інваріантів (**26 перевірок** з `0201`), слід у `maintenance_runs` | міграція `0154`–`0201` |
...
⚠️ **І досі НІХТО не читає `maintenance_runs` автоматично** (борг, названий у
ревʼю пакета привілеїв, с49). Сторож чесно пише `ok:false` — і рядок лежить,
доки хтось не виконає запит вище руками. Тобто **усі перевірки сторожа
працюють рівно настільки, наскільки часто оператор сюди заглядає** (число
свідомо не пишемо: воно протухало тут уже двічі).
⚠️ Колишнє число в цьому рядку брехало з 0177 (казало «20» при 21) — жоден канал
`tests/invariantsCheckedPins.test.ts` цього рядка не бачив, бо в ньому немає
слова «сторож»; виправлено в 0180 руками. Поки
споживача немає, звірку `select public.invariants_check(false)` варто робити
першим кроком кожної сесії (задача №0).

> ⚠️ **Числа вище протухають.** Джерело істини — ОСТАННЯ міграція з
> передруком `invariants_check` (список перевірок, названі межі кожної і
> рецепт перезняття пінів — у ній). Звіряти замiром:
> `select public.invariants_check(false) -> 'checked'`.
>
> ✅ **Борг закрито в с57.** Доти тут стояло «цей файл сторожем не покритий»:
> піни в `tests/invariantsCheckedPins.test.ts` сканували ЛИШЕ `supabase/smoke`,
> і рядок вище відставав на чотири передруки. Тепер `docs/ops-cron.md` названий
> у `LIVE_DOCS` того ж тесту і стережеться нарівні зі смоуками — при бампі
> 19 → 20 сторож упав саме на ньому, ще до того, як я встиг його поправити.
> ⚠️ Межа: під наглядом РІВНО цей файл. Інший док, що почне тримати живе
> число, сам собою туди не потрапить — журнал аудиту і PR-доки навмисно
> зберігають старі числа, тому скан по всьому `docs/` неможливий.
