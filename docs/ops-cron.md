...
| `invariants` | `50 3 * * *` | `select public.invariants_check()` — сторож інваріантів (**20 перевірок** з `0175`), слід у `maintenance_runs` | міграція `0154`–`0175` |
...
доки хтось не виконає запит вище руками. Тобто **20 перевірок сьогодні
працюють рівно настільки, наскільки часто оператор сюди заглядає**. Поки
