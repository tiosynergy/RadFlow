...
| `invariants` | `50 3 * * *` | `select public.invariants_check()` — сторож інваріантів (**19 перевірок** з `0172`), слід у `maintenance_runs` | міграція `0154`–`0172` |
...
доки хтось не виконає запит вище руками. Тобто **19 перевірок сьогодні
...
> рецепт перезняття пінів — у ній). Цей файл сторожем не покритий: піни в
> `tests/invariantsCheckedPins.test.ts` сканують лише `supabase/smoke`, тож
> у с56 рядок тут відставав на чотири передруки. Звіряти замiром:
