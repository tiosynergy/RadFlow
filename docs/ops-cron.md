...
| `invariants` | `50 3 * * *` | `select public.invariants_check()` — сторож інваріантів (**22 перевірок** з `0180`), слід у `maintenance_runs` | міграція `0154`–`0180` |
...
доки хтось не виконає запит вище руками. Тобто **22 перевірки сьогодні
працюють рівно настільки, наскільки часто оператор сюди заглядає**.
⚠️ Це число брехало з 0177 (казало «20» при 21) — жоден канал
