...
### 🟡 M-3. Роли не разделены на уровне RLS: регистратор == админ *(✅ закрыто миграцией 0073)*

> **Фикс (решение владельца):** кабинеты (`rooms`) и профиль центра (`clinics`, включая `timezone`) и прайс (`services`) — пишет только админ; простои (`incidents`) и особый график дня (`schedule_overrides`) — админ + регистратор; `doctors` — **INSERT** админ+регистратор (кнопка «+ Додати» в форме записи), UPDATE/DELETE — админ; читают справочники все сотрудники (радиологу они нужны для доски). Тот же ролевой гейт продублирован внутри `submit_incident_rpc` и `emergency_stop_rpc` — они `SECURITY DEFINER` и RLS не применяют.
> **Ревью поймало:** политики обязаны быть `to authenticated` — иначе `anon` (истёкшая сессия в открытой вкладке) получал бы `42501: permission denied for function auth_is_desk` вместо пустого результата, и доска показывала бы ошибку загрузки простоев. Плюс хелперы вызываются как `(select fn())`, иначе `STABLE`-функция дёргается **на каждую строку**. И жёсткое предусловие: 0073 бессмысленна без 0064 — без гарда `profiles` любой пользователь переписывает себе `role` и обходит все новые гейты одним запросом.
...
#### 🟡 M-5. Запись из листа ожидания: две операции без атомарности и без CAS *(✅ закрыто миграцией 0100, 2026-07-17)*
...
> **Фикс (0100).** `markWaitlistScheduled` **удалена** (ещё 0089), а перенос сведён в ОДНУ транзакцию БД — `schedule_from_waitlist_rpc` (SECURITY DEFINER, staff-only): CAS `waiting→scheduled` (рядкове блокування сериализует конкурентов → проигравший получает `WAITLIST_STALE 55000`) → `insert queue_entry` (все booking-триггеры тут же) → запись `scheduled_entry_id` — всё атомарно; любой сбой откатывает и застолбление (кандидат остаётся `waiting`, сирот-записей нет). `WAITLIST_NOT_FOUND 42501` на несуществующего. Верифицировано вживую (rollback на `PAST_SLOT` → кандидат `waiting`; STALE/NOT_FOUND).
