...
| M-2 | `lateCallClash` слеп к следующему дню | подтверждено; проявляется только при вызове ≈ за длительность до полуночи, БД (0129) отбивает | ✅ с42, но НЕ так, как предлагал аудит: соседний день не грузим — код `next_day` (подтверждение, не блок) | сделано |
...
- ✅ M-2 закрыт в с42 (см. «Не делаем» о том, почему без соседнего дня).
...
- **Грузить соседний день ради M-2** — чужие записи в `entries` ломают счётчики
  дня, звук превышения и таймеры кабинетов (тот же принцип, что у «хвостов» в
  `lib/stuckStudy.ts`). Вместо этого — честное подтверждение `next_day`: окно
  вызова уходит за полночь, доска этого дня не видит, оператор решает сам. БД
  (`ACTUAL_OVERLAP`, 0129) остаётся authoritative.
