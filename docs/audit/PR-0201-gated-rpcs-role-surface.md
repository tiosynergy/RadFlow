...
* **№19 — 59 функцій.** Будь-яка правка будь-якої з них — тіло, `grant`/`revoke`,
  ...
  сторожа. Борг с74-Б2 закрито в с75: `tests/pinnedFnReprintOrder.test.ts`
  червонить міграцію, новішу за останній передрук, яка чіпає функцію зі списку
  (межа — динамічний SQL зі склеєним імʼям).
