...
**[HIGH] H-2. Нет оптимистичной блокировки — возможны потерянные обновления.**
...
**[MEDIUM] M-7. Сервис-роль обходит RLS — вся защита на ручной проверке каждого роута.** Задокументировано в `admin.ts`, и `ceo/grant` делает правильно. Риск — регресс в новом роуте.
Рекомендация: единый guard-хелпер (`requireRole(req, 'admin')`), покрытый тестом, обязателен во всех service-role роутах.
