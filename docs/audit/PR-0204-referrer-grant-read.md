...
недосяжні НЕПРОЧИТАНІ позначки зупиняють накат, а не видаляються). **Прод:** ✅
**накатано 25.09.2026 о 23:47:11 UTC** (с80) — сухий прогін, накат, смоуки,
фальсифікація, `db:gate` і штамп деплою — §17. До наката пакет звертався до
проду ЛИШЕ read-only запитами до каталогу й агрегатами `count(*)` (§3, §12).
...
**Раунд 3 — повна ревізія 40/40 зелені** (`EXPECTED_STANDS` = 40): чисте дерево,
коміт **`6408753`** (фінальний раунд; цей абзац дописано наступним, суто
документним комітом), `node scripts/falsify-all.mjs` без імен, `tsc --noEmit`
перед першим, 26.09 23:00–23:37 UTC (≈37 хв; найдовші — `falsify-u72` 326 с,
`falsify-0166` 201 с, `falsify-g1f` 116 с): 0166 60/60, 0180 15/15, 0181 21/21,
0182 13/13, 0183 7/7, 0184 13/13, 0185 12/12, 0192-tz 16/16, 0193-room-gate 19/19,
0194-gcal-blind 22/22, build-stamp 11/11, f346 32/32, f4-2 25/25, f4-8 17/17,
f4-affected 5/5, f4-incident-window 11/11, f4-portal 15/15, f6auth 36/36, f6srv
8/8, g1f 44/44, race-check 67/67, rf05 26/26, rf09 44/44, sched-refetch 10/10, u13
23/23, u15 31/31, u17-u18 25/25 (7 заморожено), u20 20/20, u30 15/15, u33 22/22,
u37 21/21, u55 24/24, u56 28/28, u57 10/10, u59-u60 31/31, u61 19/19, u66 6/6, u67
9/9, u70 19/19, u72 69/69. Протухлих якорів — 0; дерево після ревізії порожнє,
мітка `.falsify-all.running` знята.
...
## 17. Прод — результат (25.09.2026, с80)

Кроки §11 — по порядку, фрагменти база читала сама через `net.http_get` із
тимчасової гілки `s80-0204-frags` (лише фраги) зі звіркою sha256 до `execute`;
гілку видалено після наката.

| крок | що побачили |
|---|---|
| 1. Сухий прогін | помилка-маркер `DRYRUN_0204_ROLLBACK … checked=26 ok16=true ok17=true unreachable={} radius_rows(q/w/c)=2/0/0` — замір збігся з очікуваним (§11, §12): 2 записи черги одного направника з `pending_referrer` |
| 2. Накат | 23:47:11 UTC; читання назад: тіло сторожа `cf1a920d2052a6debaff8b46c450aeff` / 178 426, самопін збігся, леджер 204, останній — `0204_referrer_grant_read.sql` |
| 3. `invariants_check(false)` | `checked` 26; впала лише №13 `gcal_sync_overdue` (обидва дзеркала Google у `reauth_required` — наслідок П-3) |
| 4. Смоук 0204 | `SMOKE_OK … n/a=[]` — усі 30 міток, жодна не переїхала в n/a |
| 4а. Суміжні смоуки | `rls_initplan_smoke` і `search_roles_smoke` — `SMOKE_OK` (`rls_initplan` — `begin;`/`rollback;` рівно двома рядками знято перед `execute`, бо `EXECUTE` транзакційних команд не приймає) |
| 5. Фальсифікація | `FALSIFY_0204_ROLLBACK verdict=PASS probes_ok=21/24 probes_missed={}`; три n/a — проби «іншого центру», дозволені §11; після — `pg_proc` із «falsify 0204» 0, тригер `zzz_falsify_0204_late` 0 |
| 6. `npm run db:gate` | 204/204, неоштампованих 0; md5 рядка леджера 0204 = md5 файла `6c232e12ff5f3c3c9238795327ce9018` |
| 7. git і деплой | `main` = `5de402d`, штамп `/api/build` = `52f60d76a2cc` = `sha256(main)[:12]` (звірено з машини власника о 23:52 UTC); `db:gate:check` на `main` і `dev` — OK; червоне вікно закрито |

Контрольний замір після (read-only, 25.09 23:57 UTC): леджер 204/204, `ok:false,
checked:26`, упала лише №13; `SECURITY DEFINER` у `public` — 117 (EXECUTE
`authenticated` 44, `anon` 11, без `search_path` 0); політик 63; таблиць 33/33 з RLS;
аудит-тригерів `fn_audit` 10; мітла `trg_zzz_ref_entry_markers_prune` увімкнена,
EXECUTE функції — лише `postgres` і `service_role`.
