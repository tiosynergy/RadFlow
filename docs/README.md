...
> Стартовое сообщение для новой сессии — **[`../claude/NEXT_SESSION_PROMPT.md`](../claude/NEXT_SESSION_PROMPT.md)**
> (вложение: режим работы, ловушки, правила) плюс `../claude/session<N>-start-prompt.md` — сам промпт.
...
- **[`USER_GUIDE.html`](USER_GUIDE.html)** — инструкция конечного пользователя (украинский):
  регистрация центра, мастер настройки и отдельные разделы для администратора, регистратора,
  радиолога, врача-направителя и руководителя; плюс «что делать, если…» и словарь терминов.
  Файл автономный — открывается двойным кликом без сети, печатается, пересылается вложением.
  Макеты экранов внутри нарисованы настоящим CSS проекта на **вымышленных** пациентах (ПИИ нет).
  Пересобирается из `claude/guide/` (три части + `assemble.py`); **редактировать надо части,
  а не собранный HTML** — он перезаписывается при сборке.
...
- **[`ops-cron.md`](ops-cron.md)** — реестр фоновых задач (**10 штук**; дату сверки с продом
  смотреть в самом реестре — здесь её не дублируем, потому что копия даты и есть то, что
  протухает первым): расписания
