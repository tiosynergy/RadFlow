...
- **CEO отримує `access`-позначки, але не має де їх побачити й погасити** —
  у нього немає жодного екрана під операційні сутності. Це та сама «негасима
  позначка», що с28 закрила для адміна. Рішення (звузити аудиторію в 0131 чи
  дати CEO екран) — у серверному пакеті №4.
- **`actor_role = "system"` для всіх подій доступів** — чотири роути
  `referral_access` пишуть через service-role клієнт без JWT, тож
  `auth.uid()` порожній і правило «отримувачі мінус актор» не працює: адмін
  отримує крапку про власну дію. Теж пакет №4 (потрібен проброс актора в
  транзакцію + міграція).
- **Ретрай ack без backoff**: невдалий `mark_changes_seen` інкрементує
  `ackFailGen`, що миттєво перевзводить усі видимі хуки → при стійкій
  помилці (протухла сесія, 401) виходить безперервний цикл запитів.
  Пакет №4.
