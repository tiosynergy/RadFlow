...
- У Google Calendar створити ОКРЕМИЙ календар «RadFlow Backup — <Клініка>»
  (Settings → Add calendar → Create new). **ОСОБИСТИЙ календар RadFlow не
  прийме** (с43) — ні основний календар самого акаунта, ні чужий особистий,
  розшарений цьому акаунту з правом запису: у списку такі показані
  недоступними, а `/select` відхиляє їх кодом `calendar_is_primary`.
  Правило одне на всі точки: ціллю копії може бути лише **згенерований
  Google вторинний календар** (id у домені `@group.calendar.google.com`,
  `@group.v.…`, `@resource.…`, `@import.…`). Причина — у копії живуть ПІБ і
  телефони пацієнтів, а аварійне читання вимагає дати доступ персоналу:
  особистий календар віддав би разом із чергою все приватне.
...
⚠️ **Передумова перепідключення: спершу створіть ОКРЕМИЙ календар.**
`Відключити` і будь-який повторний OAuth обнуляють `calendar_id` — після них
клініка ОБОВʼЯЗКОВО проходить вибір календаря заново, а особистий календар
`/select` тепер відхиляє (с43). Клініка, яка сьогодні дзеркалить в особистий
календар, без нового календаря дзеркало вже не підніме. Порядок: створити
«RadFlow Backup — <Клініка>» у Google → «Відключити» → підключити → обрати
новий календар.
...
| Дзеркало саме вимкнулось рівно через 7 днів після підключення | токен видано в режимі Testing (7-денна стеля Google) | опублікувати застосунок; ПЕРЕД перепідключенням створити окремий календар — інакше вибір відхилить `calendar_is_primary` |
...
| Вибір відхилено: «Особистий календар не підходить» (`calendar_is_primary`) | обрано особистий календар (свій або чужий розшарений) — заборонено з с43 | створити окремий календар «RadFlow Backup — <Клініка>» і обрати його |
| Оранжеве «копія лежить в особистому календарі» | підключення зроблене до заборони | створити окремий календар, «Змінити календар», а події зі старого прибрати ВРУЧНУ |
...
- **Особистий календар заборонений як ціль копії** (с43). Підключення,
  зроблені ДО заборони, продовжують дзеркалити — /setup показує оранжеве
  попередження «копія лежить в особистому календарі». Але будь-яке
  перепідключення вимагатиме ОКРЕМОГО календаря (див. §«Статус застосунку»).
