...
**Дата:** 2026-08-06 (сессия 25 в проде) · **Ветка:** `dev`.

**`origin/main` = `fcc78a2`** (PR #18) — с25 выкачена в прод: система логирования
важных событий (миграция `0128`), страница «Журнал дій» и поиск записи по ID.
**`origin/dev` = `8fb7ecb`** — отстаёт ровно на мердж-коммит: выровнять
`git fetch && git merge --ff-only origin/main && git push origin dev`.
Тулчейн на свежем клоне `main`: tsc 0, lint 0, **vitest 782/782**,
`npm run build` успешно, `npm run audit:contrast` — 0.

**Прод-БД на `0128`** → следующая новая = **`0129`**. Миграцию 0128 владелец
применил вручную ДО выката клиента (порядок соблюдён).

**Данные прода (сверено 2026-08-06):** 147 записей очереди (69 сева
`[SEED s19]`), 2 клиники, 9 кабинетов, 11 строк листа ожидания, 6 кейсов,
`audit_log` 1113 строк, **`important_events` 13 строк** (2 типа событий,
PII — нет, проверено запросом).
