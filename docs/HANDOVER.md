...
**Дата:** 2026-07-18 (вечер) · **Ветка:** `dev` · **PROD:** БД на **`0107`** (0061–0107 применены владельцем; ledger `supabase_migrations` пуст — накатка ручная через SQL Editor). Следующая новая = **0108** (заготовлена под `service_room_durations`, фаза 2b). `/services` проверен вживую: сид «Заповнити з базового каталогу» — 45 позиций (Medicom), инлайн-правка цены сохраняется, `updated_at` тикает, фикс фокуса работает. ⚠️ Цены УЗД/Рентген/Мамографія в сиде = 0 — проставляет владелец.
