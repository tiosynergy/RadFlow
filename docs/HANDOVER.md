...
**Дата:** 2026-08-05 (сессия 22) · **Ветка:** `dev`.

**В `main` уже смёржено:** всё по с21 включительно (PR #16, `b9cc3a9`: звук
перевышения `0a42c3f` + доки `e3764f8`) — хвост «довести пакет звука до main» закрыт.
**Не закоммичено (пакет с22, тарбол `_s22_search.tar.gz`):** универсальный поиск —
см. блок ниже. **Миграций нет.**

**Прод-БД на `0127`** → следующая новая = **`0128`**.
Тулчейн на клоне `origin/dev` (`e3764f8`) + пакет с22: tsc 0, lint 0,
**vitest 509/509**, `npm run build` — успешно.
