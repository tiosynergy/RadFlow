...
**Дата:** 2026-07-12 · **Ветка:** `dev` · **PROD:** БД на `0064` (0061–0064 применены владельцем 2026-07-12); код `main` ещё старый — мердж `dev → main` разблокирован.
⚠️ Код в `dev` читает колонки `event_outbox.next_attempt_at/dead` — он совместим только с БД ≥ 0064 (миграция уже накачена, порядок соблюдён).
...
Остальное (P1/P2: CAS в остальных мутациях, `data || []` в 5 досках, `submit_incident_rpc`, CHECK `duration_min`, `ceo_kpi`, trgm-индексы, ретенция `audit_log`/`rate_limits`) — §5 отчёта.
