...
**Дата:** 2026-07-20 (сессия 4) · **Ветка:** `dev` · **PROD:** БД на **`0117`** (0061–0117 применены владельцем, каждая с её smoke; ledger `supabase_migrations` пуст — накатка ручная через SQL Editor). Код: PR #4 (0061–0117) смерджен `dev → main`, Vercel Production деплой Active. **`0118` (кейсы для направителя) НАПИСАНА и верифицирована в откате (SMOKE_OK) — НАКАТИТЬ; после неё следующая новая = 0119.** ⚠️ Цены УЗД/Рентген/Мамографія — проставляет владелец (импортом xlsx через /services или вручную).
