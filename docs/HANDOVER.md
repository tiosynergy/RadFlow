...
**Пакет сессии 16 НЕ закоммичен.** Владелец распаковал `_s16.tar.gz`, после чего пять файлов
доложены поверх точечно (фиксы части 3) — актуальный полный тарбол `_s16.tar.gz` перезаписан.
Коммитит владелец. Локальная `main` отстаёт (`1f58405`) — перед мерджем обязательно
...
>   иначе каскад от удаления клиники падал бы с сообщением про кабинет);
>   частичный индекс `rooms_clinic_active_idx`.
