...
**Дата:** 2026-07-28 (сессия 18) · **Ветка:** `dev` == **`4095c57`**; `main` ещё на
`837b778` — **PR первого пакета не смёржен**.

**Первый пакет** («кабінет з історією не видаляється») закоммичен и запушен в `dev`;
**миграция `0126` накачена на прод**, чистка 41 записи-сироты выполнена (очередь
114 → 73, без кабинета 44 → 3). **Второй пакет** (хвосты ревью с17 — только клиент,
миграций нет) подготовлен, но **НЕ закоммичен**. Коммитит и накатывает владелец.

**Прод-БД на `0126`** → следующая новая = **`0127`**.
Тулчейн (клон из GitHub + второй пакет): tsc 0, lint 0, **vitest 387/387**,
