...
**Дата:** 2026-07-17 · **Ветка:** `dev` · **PROD:** БД на **`0102`** (0061–0102 применены владельцем, ledger `supabase_migrations` пуст — накатка ручная через SQL Editor). ⚠️ **`0103` написана/закоммичена, применить вручную** (после 0102). Репо на **`0103`**, следующая новая = **0104**.
...
>
> **E. Находка Medium — info-leak модальности чужого кабінету → `0103`** (написана/закоммичена, **применить вручную** после 0102). `check_waitlist_consistency` (0090) и `check_studies_match_room` (0088) вибирали кабінет `where r.id = new.room_id` **без `clinic_id`**. Ці modality-тригери (`trg_c_*`) за алфавітом імені спрацьовують **раніше** clinic-match гардів (`trg_guard_waitlist_room`/`trg_guard_queue_room`), тож room_id ЧУЖОГО центру з невідповідною модальністю → `…_MODALITY_MISMATCH: room <модальність> …` розкриває існування+модальність чужого кабінету ще до нейтрального `ROOM_NOT_IN_CLINIC`. Підтверджено вживую (персонал Medicom + кабінет Odessa → «room MRI vs modality CT»). Фікс: `and r.clinic_id = new.clinic_id` в обидва SELECT (тіла відтворені 1-в-1, змінено лише SELECT) → чужий кабінет не знаходиться → `v_room_mod=null` → відмову дає clinic-match гард без модальності. Ревью — SHIP; скоуп повний (інші before-тригери до clinic-match кабінет не читають). Живу перевірку роблю після накатки. **Урок:** будь-який SELECT з `rooms`/tenant-таблиць у тригері/RPC — фільтруй за `clinic_id`, не лише за `id` (те саме стосується `queue_entries` — кожна нова колонка/перевірка).
