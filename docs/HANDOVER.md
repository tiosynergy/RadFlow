...
**Дата:** 2026-07-14 · **Ветка:** `dev` · **PROD:** БД на **`0080`** (0061–0080 применены владельцем). Следующая новая = **0081**.

> ⚠️ **БД ВПЕРЕДИ КОДА.** Идёт большая фича «политика очереди при задержке» (0078–0080): этапы 1–2 и половина 3-го. Прод в безопасности — статус `needs_reschedule` ставит только `queue_apply_delay_plan_rpc`, а вызывать её пока некому (Server Action не написан). Что осталось — в `NEXT_SESSION_PROMPT.md`, раздел «Этап 3b».
>
...
> ✅ Миграции **0061–0075 применены к проду**. Следующая новая = **0076**.
>
> **Что добавили 0074–0075 (2026-07-14):**
...
supabase/
  migrations/           → схема + RLS, последовательные .sql (прод и репо на 0075)
...
`supabase/migrations/` — прод и репо на **0075** → следующая новая = **0076**.
...
### 5.1. ✅ Миграции применены (по 0077, 2026-07-14) — мердж разблокирован

Прод-БД на **`0077`**. Осталось смерджить `dev → main` (деплой миграции **не** гоняет). Следующая новая = **0078**.

**Кандидат на 0078:** триггер `check_room_schedule` — инварианта «график кабинета» в БД нет вообще (ни до 0077, ни после), и «до открытия / выходной / потолок +2 ч» держит только сервер. Не срочно: тот же уровень доверия к персоналу был и раньше.

**Осталось из блока конкурентности (следующий шаг):**

- **`submit_incident_rpc` — та же непочиненная гонка.** Ветка `p_id is null` делает голый `insert into incidents` → при параллельной аварийке/втором «Поломка» на тот же кабинет ловит `23505` и откатывает **всю** транзакцию, включая свой `not_held`. Лечится так же: `on conflict (room_id) where status='active' do nothing returning id into v_id;` и, если `v_id is null` — вернуть существующий активный инцидент либо честную доменную ошибку `'INCIDENT: кабінет уже у простої'` вместо сырого 23505.
- **Инциденты не сериализованы с бронированием** (пункт 2 бэклога) — бронь может лечь внутрь только что созданного простоя. Порядок блокировок обязателен: сначала строки `queue_entries`, потом `incidents`; advisory-lock кабинета в начале incident-RPC = дедлок со статусными RPC.
...
Прод и репо на **0075**; следующая новая = **0076**. Применяются **вручную** через Supabase SQL Editor (автораннера нет). Держать идемпотентными (`create or replace`, `drop … if exists`, `do $$ … exception when duplicate_object …$$`). Обновлять `supabase/types.ts` при смене схемы. **SQL применять к проду ДО мерджа `dev → main`.**
