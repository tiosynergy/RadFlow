...
- ⏸️ **Ротация `SUPABASE_SERVICE_ROLE_KEY` — отложена владельцем** (решение с26, 2026-08-06):
  ключ засветился в скриншоте, но ротацию делаем к готовности продукта. Готовый план замены
  без простоя и проверки после — `docs/setup/SERVICE_ROLE_KEY_ROTATION_2026-08-06.md`.
  Не поднимать как P0; напомнить один раз перед выходом на реальных клиентов.
