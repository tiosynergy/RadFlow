...
--  сторож рахує 26 перевірок (0180)».
--
-- ДВА РЕЖИМИ ЗАПУСКУ:
...
--   (j) invariants_check(false): checked = 26 (0180), room_busy_service_role мовчить;
...
begin
  ...
  -- (j) сторож: 26 перевірок (0180), room_busy_service_role мовчить.
  ...
  if (v_res ->> 'checked')::int is distinct from 26 then
    raise exception 'SMOKE_FAIL j: checked=% (очікував 26)', v_res ->> 'checked';
  end if;
