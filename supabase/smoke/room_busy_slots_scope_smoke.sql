...
--  сторож рахує 22 перевірок (0180)».
--
-- ДВА РЕЖИМИ ЗАПУСКУ:
...
--   (j) invariants_check(false): checked = 22 (0180), room_busy_service_role мовчить;
...
begin
  ...
  -- (j) сторож: 22 перевірок (0180), room_busy_service_role мовчить.
  ...
  if (v_res ->> 'checked')::int is distinct from 22 then
    raise exception 'SMOKE_FAIL j: checked=% (очікував 22)', v_res ->> 'checked';
  end if;
