...
--  сторож рахує 20 перевірок (0175)».
--
-- ДВА РЕЖИМИ ЗАПУСКУ:
...
--   (j) invariants_check(false): checked = 20 (0175), room_busy_service_role мовчить;
...
begin
  ...
  -- (j) сторож: 20 перевірок (0175), room_busy_service_role мовчить.
  ...
  if (v_res ->> 'checked')::int is distinct from 20 then
    raise exception 'SMOKE_FAIL j: checked=% (очікував 20)', v_res ->> 'checked';
  end if;
