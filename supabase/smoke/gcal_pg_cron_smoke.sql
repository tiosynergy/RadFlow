...
begin
  ...
  --    ⚠️ Піни перезнято під 0181 (RF-01, джерело кейса під гардом; №16 —
  --    дві політики журналів, №19 — гард + дві case-RPC, 23 → 25; `checked`
  --    НЕ мінявся, 22). Попередні: 0180 g c7670d89…, g2 caecded8…, 94298;
  ...
  if v_txt is distinct from 'f157d9de47d29abdf24a96ce89b04b6e' then
    raise exception 'SMOKE_FAIL g: md5 тіла invariants_check = %, очікував f157d9de… (передрук розійшовся)', v_txt;
  end if;
  ...
  select md5(replace(prosrc, chr(13), '')) into v_txt
   ...
   where proname = 'invariants_check'
     and pronamespace = 'public'::regnamespace;
  if v_txt is distinct from '19b47366fc60ce602f0642a1924ec779' then
    raise exception 'SMOKE_FAIL g2: тіло прода != тіло файлу 0181 (md5 без CR = %)', v_txt;
  end if;
  v_done := v_done || ' g g2';
