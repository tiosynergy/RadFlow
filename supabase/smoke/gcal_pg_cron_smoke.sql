...
begin
  ...
  -- ⚠️ Пін перезнято після 0172 (0161: 935bdd06…, 0164: d8d22ff4…, 0165: f422cce0…,
  ...
  --    навмання.
  --    Значення 5af876d2… звірено двічі: живим запитом до прода І незалежним
  --    розбором тіла з файлу 0172 (між `as $function$` і `$function$;`) з тією
  ...
  --      • 0172 накатано DO-блоком через MCP — заміряно `cr_count = 0`, і
  ...
  if v_txt is distinct from '5af876d25b063b3c44ba6ae46c28ad74' then
    raise exception 'SMOKE_FAIL g: md5 тіла invariants_check = %, очікував 5af876d2… (передрук розійшовся)', v_txt;
  end if;
  ...
  select md5(replace(prosrc, chr(13), '')) into v_txt
   ...
   where proname = 'invariants_check'
     and pronamespace = 'public'::regnamespace;
  if v_txt is distinct from '8b3d332d25d9fa838113e2f29b84cdd3' then
    raise exception 'SMOKE_FAIL g2: тіло прода != тіло файлу 0172 (md5 без CR = %)', v_txt;
  end if;
  v_done := v_done || ' g g2';
