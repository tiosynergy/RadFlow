...
begin
  ...
  -- ⚠️ ВИПРАВЛЕНО 14.09 (с69). Раніше тут стояв `raise exception 'немає
  --    клініки без підключення'`, і саме на ньому смоук падав ПЕРШИМ ділом:
  --    заміряно — клінік без рядка підключення у проді **0 із 2**. Тобто весь
  --    файл не міг завершитись зеленим, і ніхто цього не бачив.
  --    Смоук не має права залежати від того, чи лишилась у проді «зручна»
  --    ненастроєна клініка: якщо такої немає, він створює СВОЮ. Транзакція
  --    все одно відкочується (`rollback` наприкінці), а `clinics` вимагає
  --    лише `name` — решта колонок має дефолти (`timezone` = 'UTC', що
  --    проходить CHECK із 0192).
  if v_clinic is null then
    insert into public.clinics (name) values ('__smoke_gcal_fixture__')
  ...
  if (v_res ->> 'checked')::int is distinct from 26 then
    raise exception 'SMOKE_FAIL e: checked = %, очікував 26', v_res ->> 'checked';
  end if;
  v_done := v_done || ' e';
  ...
  --    навмання.
  --    ⚠️ Піни перезнято ще раз під 0178 (RF-09; попереднє значення для 0177
  --    було b8f17ba1…). ⚠️ І тут ЧЕСНО про походження числа, бо перша
  --    редакція цього коментаря стверджувала замір, якого не було (спіймано
  --    ревʼю): 10b3204c… узято РОЗБОРОМ ТІЛА З ФАЙЛУ 0178 (між `as $function$`
  --    і `$function$;`, плюс хвостовий перевід рядка) з тією самою
  --    нормалізацією, що вище. Прилад перевірено: той САМИЙ розбір на файлі
  --    0177 відтворює b8f17ba1… і 51abbdb1… — тобто прилад перевірено на
  ...
  --    ⚠️ Піни перезнято під 0183 (RF-03, графік кабінетів направнику; №16
  --    втратив рядок знятої політики `sched_referrer_read` 64 → 63, №19
  --    отримав `sched_override_read` 26 → 27; `checked` НЕ мінявся, 22).
  --    Обидва числа зняті ДВІЧІ: розбором ФАЙЛА 0183 (прилад звірено на 0182:
  --    той самий розбір відтворює 498f86f7…/c936ff4a… і довжину 95662) і живим
  --    запитом до прода ПІСЛЯ накату 08.09.2026:
  --    g = 00d9ad82…, g2 = 3ac1aa3c…, length(prosrc) = 96322, cr = 0.
  --    Попередні: 0182 g 498f86f7…, g2 c936ff4a…, 95662;
  --    0181 g f157d9de…, g2 19b47366…, 95038; 0180 g c7670d89…, g2 caecded8…, 94298;
  --    0179 g 71e552c2…, g2 5e468b5e…, 80871; 0178 g 10b3204c…, g2 6ff5dd3d…,
  --    77313. Обидва числа зняті ДВІЧІ: розбором ФАЙЛА 0181 (прилад звірено
  --    на 0180: той самий розбір відтворює c7670d89…/caecded8… і довжину
  --    94298) і живим запитом до прода ПІСЛЯ накату 07.09.2026:
  --    g = f157d9de…, g2 = 19b47366…, length(prosrc) = 95038, cr = 0 —
  --    збіглось до біта.
  ...
  -- ⚠️ ПІН ЗНЯТО 14.09 (с69), і це не послаблення, а прибирання мертвого
  --    сторожа. Значення `6eac0bb1…` — це тіло 0185; у проді сьогодні інше
  --    (`af390d6f…` після 0194). Пін, який треба переписувати РУКОЮ після
  --    кожного передруку, протухає по побудові — і протух: із 0186 цей файл
  --    не міг завершитись зеленим НІ РАЗУ. Перевірка, що завжди червона,
  --    вчить не читати смоук.
  --    ДЕ ЦЯ ВЛАСТИВІСТЬ ЖИВЕ НАСПРАВДІ: кожен пакет передруку звіряє тіло
  --    ДВІЧІ (предстан і результат) у своєму фрагменті накату, а md5 ФАЙЛА
  --    міграції тримає №7 `ledger_md5` + `npm run db:gate`.
  --    ⚠️ Чого не тримає ніхто — див. межу М-4 пакета 0194: тіло сторожа не
  --    пінить жодна з 23 перевірок. Це окремий пакет, і він у
  --    `docs/audit/ToDo_Production.md` (Н-1).
  raise notice 'SMOKE_INFO g: md5 нормалізованого тіла invariants_check = %', v_txt;
  ...
  select md5(replace(prosrc, chr(13), '')) into v_txt
   ...
   where proname = 'invariants_check'
     and pronamespace = 'public'::regnamespace;
  -- ⚠️ Знято разом із `g` і з тієї самої причини (значення 0185 при
  --    `af390d6f…` у проді). Число друкуємо — воно корисне очима, — але
  --    сторожем воно більше не прикидається.
  raise notice 'SMOKE_INFO g2: md5 тіла без CR = %', v_txt;
  v_done := v_done || ' g-info g2-info';
