...
begin
  ...
  --    ⚠️ Піни перезнято під 0182 (RF-02, TTL запрошення; №15(f2) виняток на
  --    нову колонку, №17 новий тригер-штамп, №19 25 → 26; `checked` НЕ
  --    мінявся, 22). Обидва числа зняті ДВІЧІ: розбором ФАЙЛА 0182 (прилад
  --    звірено на 0181: той самий розбір відтворює f157d9de…/19b47366… і
  --    довжину 95038) і живим запитом до прода ПІСЛЯ накату 08.09.2026:
  --    g = 498f86f7…, g2 = c936ff4a…, length(prosrc) = 95662, cr = 0.
  --    Попередні: 0181 g f157d9de…, g2 19b47366…, 95038; 0180 g c7670d89…, g2 caecded8…, 94298;
  ...
  if v_txt is distinct from '498f86f71c5f6a0cf6196bb384494d3b' then
    raise exception 'SMOKE_FAIL g: md5 тіла invariants_check = %, очікував 498f86f7… (передрук розійшовся)', v_txt;
  end if;
  ...
  select md5(replace(prosrc, chr(13), '')) into v_txt
   ...
   where proname = 'invariants_check'
     and pronamespace = 'public'::regnamespace;
  if v_txt is distinct from 'c936ff4a209b852c7dc766448f99b4e7' then
    raise exception 'SMOKE_FAIL g2: тіло прода != тіло файлу 0182 (md5 без CR = %)', v_txt;
  end if;
  v_done := v_done || ' g g2';
