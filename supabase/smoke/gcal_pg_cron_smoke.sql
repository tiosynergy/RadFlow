...
begin
  ...
  -- ⚠️ Пін перезнято після 0174 (0161: 935bdd06…, 0164: d8d22ff4…, 0165: f422cce0…,
  --    0166: bc10f4e5…, 0167: 12cf23fe…, 0170: d754ee12…, 0171: c61de84b…,
  --    0172: 5af876d2…, 0173: 07a4e102…).
  --    Кожен передрук сторожа
  --    міняє це число — знімати ЖИВИМ запитом після накату, а не переписувати
  --    навмання.
  --    Значення a26cb538… звірено двічі: живим запитом до прода І незалежним
  --    розбором тіла з файлу 0174 (між `as $function$` і `$function$;`) з тією
  ...
  --      • 0172–0174 накатано DO-блоком через MCP — заміряно `cr_count = 0`, і
  ...
  if v_txt is distinct from 'a26cb538b1b85dab918511af09bfcd4a' then
    raise exception 'SMOKE_FAIL g: md5 тіла invariants_check = %, очікував a26cb538… (передрук розійшовся)', v_txt;
  end if;
  ...
  select md5(replace(prosrc, chr(13), '')) into v_txt
   ...
   where proname = 'invariants_check'
     and pronamespace = 'public'::regnamespace;
  if v_txt is distinct from 'f1d65f4fc2ebc58e4a8027a887942f71' then
    raise exception 'SMOKE_FAIL g2: тіло прода != тіло файлу 0174 (md5 без CR = %)', v_txt;
  end if;
  v_done := v_done || ' g g2';
