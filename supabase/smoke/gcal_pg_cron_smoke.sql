...
begin
  ...
  if v_txt is distinct from '10b3204c97781909c3e53a3f901056ec' then
    raise exception 'SMOKE_FAIL g: md5 тіла invariants_check = %, очікував 10b3204c… (передрук розійшовся)', v_txt;
  end if;
  ...
  select md5(replace(prosrc, chr(13), '')) into v_txt
   ...
   where proname = 'invariants_check'
     and pronamespace = 'public'::regnamespace;
  if v_txt is distinct from '6ff5dd3db1681620ff6ccef18904e7f2' then
    raise exception 'SMOKE_FAIL g2: тіло прода != тіло файлу 0178 (md5 без CR = %)', v_txt;
  end if;
  v_done := v_done || ' g g2';
