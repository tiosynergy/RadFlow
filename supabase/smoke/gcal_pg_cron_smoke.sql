...
begin
  ...
  if (v_res ->> 'checked')::int is distinct from 18 then
    raise exception 'SMOKE_FAIL e: checked = %, очікував 18', v_res ->> 'checked';
  end if;
  v_done := v_done || ' e';
  ...
  -- ⚠️ Пін перезнято після 0171 (0161: 935bdd06…, 0164: d8d22ff4…, 0165: f422cce0…,
  --    0166: bc10f4e5…, 0167: 12cf23fe…, 0170: d754ee12…). Кожен передрук сторожа
  --    міняє це число — знімати ЖИВИМ запитом після накату, а не переписувати
  --    навмання.
  --    Значення c61de84b… звірено двічі: живим запитом до прода І незалежним
  --    розбором тіла з файлу 0171 (між `as $function$` і `$function$;`) з тією
  --    самою нормалізацією, що вище.
  -- ⚠️ НОРМАЛІЗАЦІЯ ТУТ — НЕ КОСМЕТИКА, і це заміряно (с56). Накат через
  --    SQL Editor із Windows приносить CRLF: у проді тіло 0171 має 53579
  --    байтів при 811 CR і 811 LF, тоді як у файлі 52768 байтів і самі LF.
  --    Прибрати CR — і `md5(prosrc)` дає c8473081c6159c868a8a70ad0d47428b,
  --    ДОСЛІВНО як у файлі. Тобто «прод == артефакт міграції» доводиться лише
  --    ПІСЛЯ зведення кінців рядків; побайтне твердження через SQL Editor
  --    хибне ЗА ПОБУДОВОЮ, хоч поведінка тотожна.
  ...
  if v_txt is distinct from 'c61de84b0d38381c391b02ee4f4aab99' then
    raise exception 'SMOKE_FAIL g: md5 тіла invariants_check = %, очікував c61de84b… (передрук розійшовся)', v_txt;
  end if;
  ...
  select md5(replace(prosrc, chr(13), '')) into v_txt
   ...
   where proname = 'invariants_check'
     and pronamespace = 'public'::regnamespace;
  if v_txt is distinct from 'c8473081c6159c868a8a70ad0d47428b' then
    raise exception 'SMOKE_FAIL g2: тіло прода != тіло файлу 0171 (md5 без CR = %)', v_txt;
  end if;
  v_done := v_done || ' g g2';
