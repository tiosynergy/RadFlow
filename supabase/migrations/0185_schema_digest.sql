-- ============================================================================
-- 0185_schema_digest.sql
-- ============================================================================
--  RF-05, ОСТАННІЙ ШМАТОК: schema-contract manifest, якого просив аудит.
--
--  ЩО ВЖЕ СТЕРЕГЛОСЬ ДО ЦІЄЇ МІГРАЦІЇ
--    №16 policy_digest    — політики RLS
--    №17 guard_triggers   — тригери
--    №19 guard_fn_bodies  — тіла функцій
--    №22 grant_digest     — гранти (0180, RF-04)
--  ЧОГО НЕ СТЕРЕГЛОСЬ: ФОРМА ТАБЛИЦІ. Ані колонки, ані constraint-и.
--
--  ⚠️ ЦЕ НЕ ТЕОРЕТИЧНА ДІРКА. `db:gate` звіряє md5 ФАЙЛІВ міграцій, а не стан
--     БД; тести ганяються проти двійника; жоден із 22 інваріантів форму не
--     читає. Тобто `alter table queue_entries alter column note drop not null`
--     або тихо доданий `check`, що пускає раніше заборонений статус, не
--     побачив би НІХТО — рівно так само, як до 0180 не бачився жоден GRANT
--     з UI Supabase.
--
--  ЩО РОБИТЬ ЦЯ МІГРАЦІЯ
--    Одну річ: додає перевірку №23 `schema_digest` у `invariants_check`.
--    Жодних змін даних, жодних нових обʼєктів, жодних грантів.
--    `checked` 22 → 23.
--
--  ⚠️ ЧОМУ ПАКЕТ ВУЗЬКИЙ. План с60 радив класти в один передрук кілька
--     дешевих гілок, щоб платити ревізію один раз, і назвав кандидата —
--     розширення (g2) на `to_jsonb(p)`. Свідомо НЕ кладемо: множник «пакет
--     виходить ширшим за док» спрацював у проєкті десять разів, а тут і без
--     того нова гілка, 84 піни, стенд і передрук. Заплатити ревізію двічі
--     дешевше, ніж розширити blast radius прод-міграції.
--
--  ⚠️ ЧОМУ ДАЙДЖЕСТ НА ОБʼЄКТ, А НЕ РЯДОК НА КОЛОНКУ — ЗАМІР 09.09.2026:
--     367 колонок + 161 constraint; плоский список нормалізованих рядків
--     важить 32 531 байт, і тіло сторожа виросло б з 97 025 до ~134 000.
--     Тут 84 ключі ≈ 5 КБ. Форма не вигадана — це той самий канон
--     «кількість:md5(список)», що вже стоїть у гілці `c:` перевірки №22.
--     ЦІНА, названа вголос: червоний скаже «таблиця X змінилась», а не
--     «колонка X.note стала nullable». Префікс КІЛЬКОСТІ рятує половину.
--     84 = 33 t: + 1 v: + 33 k: + 10 e: + 7 u:.
--
--  ⚠️ ЩО ЦЕ КОШТУЄ НАДАЛІ — читати ОБОВʼЯЗКОВО, перш ніж лаятись на червоне.
--     Будь-яка наступна міграція, що додасть колонку, constraint, мітку enum
--     або частковий унікальний індекс, ЗОБОВʼЯЗАНА оновити відповідний ключ
--     у `expd`, тобто зробити передрук сторожа. Раніше «додати колонку»
--     коштувало нуль, тепер — повний ритуал пакета.
--     ⚠️ ЧЕРВОНИЙ `schema_digest` ПІСЛЯ ВАШОЇ DDL-МІГРАЦІЇ — ЦЕ НЕ БАГ
--        СТОРОЖА, А ЗАБУТИЙ КРОК РИТУАЛУ. Вимикати перевірку НЕ МОЖНА: урок
--        0141 каже, що постійно червоний сторож — це видалений сторож, і саме
--        тому тут є ПРИЛАД, а не інструкція: `node scripts/print-schema-digest.mjs`.
--        ⚠️ Він друкує ГОТОВИЙ ЗАПИТ, а не результат, і це названа межа, не
--        лінощі: скрипти проєкту ходять у БД через `createClient`
--        (PostgREST), а той виконує лише RPC і таблиці, не довільний
--        каталожний SQL; заводити definer-функцію «виконай будь-що» заради
--        зручності — це нова діра, більша за задачу. Запит лишається
--        виконати в SQL Editor або через MCP: без аргументу він віддає всі
--        84 рядки одразу у форматі `('key','n:md5'),` — просто вставити в
--        `expd`; з аргументом-іменем таблиці — ПОРЯДКОВИЙ список її колонок,
--        її constraint-и, індекси та enum-и, щоб прочитати
--        `changed:t:queue_entries:40:…->40:…` за хвилину, а не за пів години.
--
--  ЗОНДИ ДО НАКАТУ (усі в транзакції з маркером відкоту, 09.09.2026):
--    A0 без правок ................. 0 offenders          ✔ зелений базис
--    A1 + колонка .................. changed:t: 4→5       ✔
--    A2 drop not null (applied_at) . changed:t: 4:c4785f→4:ff101b  ✔
--    A2b зміна типу (md5) .......... changed:t: 4:f3b402  ✔
--    A3 + constraint ............... changed:k: 1→2       ✔
--    A4 нова таблиця ............... new:t:, БЕЗ new:k:   ✔ (межа підтверджена)
--    E1 enum add value ............. changed:e:queue_status 8→9      ✔
--    U1 drop unique index .......... changed:u:queue_entries 2→1     ✔
--    I1 identity by default ........ changed:t:audit_log 9:e7c9→9:e4d7 ✔
--    A5 після відкотів ............. 0 offenders          ✔ слідів немає
--
--  ⚠️ ЗНАХІДКА ПРО САМ ІНСТРУМЕНТ, названа вголос. Перша редакція зонда мала
--     `exception when others then null` — і коли A2 впав на `alter table`,
--     обробник зʼїв помилку МОВЧКИ: провалений зонд виглядав як пройдений.
--     Причина була в даних, не в стороже: `migration_ledger.name` входить у
--     первинний ключ, зняти з нього NOT NULL не можна (42P16). Зонд
--     переписано із захопленням `sqlerrm`. Це той самий клас, що вже стоїть
--     у боргах під назвою «`fn_audit` глотає БУДЬ-ЯКУ помилку».
-- ============================================================================
...
begin
  if not exists (select 1 from public.migration_ledger
    ...
    raise exception '0185 потребує 0184 (накатуйте по порядку)';
  end if;
  if exists (select 1 from public.migration_ledger
              where name = '0185_schema_digest.sql') then
    raise exception '0185 вже накатана';
  end if;

  /* ⚠️ ПЕРЕДРУК НАОСЛІП ЗАБОРОНЕНО. Леджер каже лише «0184 накатана», а
     `create or replace` нижче перезапише ТЕ, ЩО СТОЇТЬ У ПРОДІ, яким би воно
     не було. Якщо хтось правив тіло руками через SQL Editor, 0185 затерла б
     це мовчки, а пост-асерт лишився б зеленим — бо він звіряє результат із
     числом, порахованим ІЗ ФАЙЛА, тобто обидві сторони походили б з дерева.
     Тому вимагаємо, щоб прод ДО накату побайтно дорівнював 0184. */
  if (select md5(replace(p.prosrc, chr(13), ''))
        from pg_proc p
        join pg_namespace n on n.oid = p.pronamespace
       where n.nspname = 'public' and p.proname = 'invariants_check'
         and pg_get_function_identity_arguments(p.oid) = 'p_write boolean')
     is distinct from '11a297318da068f53b113c3d9120e6b8' then
...
-- ============================================================================
-- 1. Передрук invariants_check: НОВА перевірка №23 schema_digest
-- ============================================================================
-- ⚠️ Блок нижче ГЕНЕРУЄТЬСЯ: `node scripts/build-0185-reprint.mjs`.
--    Він бере тіло з 0184 ДОСЛІВНО і робить ОДНУ заякорену вставку, яка мусить
--    збігтися рівно один раз. Руками сюди не писати.
-- <<<INVARIANTS_REPRINT>>>
...
-- ============================================================================
-- 2. Асерт ПІСЛЯ: сторож зелений ТУТ, у цій же транзакції
-- ============================================================================
do $post$
declare
  -- ⚠️ ЦІ ДВА ЧИСЛА ВПИСУЄ ГЕНЕРАТОР, а не рука. У 0184 така константа була
  --    вписана руками — і один правлений коментар усередині функції зробив її
  --    протухлою МОВЧКИ. Тут генератор перезаписує обидва рядки при кожному
  --    прогоні, тож розійтись їм ніде.
  c_md5 constant text := '8871cad05dbbee6c4cef4f4f9a3fb515';
  c_len constant int  := 111592;
  v      jsonb;
  v_md5  text;
  v_len  int;
begin
  ...
  if coalesce((v ->> 'ok')::boolean, false) is not true then
    raise exception '0185: сторож червоний одразу після передруку: %', v ->> 'failed';
  end if;
  if (v ->> 'checked')::int <> 23 then
    raise exception '0185: checked = %, а мусить стати 23', v ->> 'checked';
  end if;
  ...
  /* ⚠️ «Зелений сторож» — ЩЕ НЕ ДОКАЗ, що передрук той самий: тіло могло
     ...
     ВІДБИТОК тіла з тим, що порахував генератор ІЗ ФАЙЛА. Два незалежні
     обчислювачі, один md5 — інакше прод і дерево розійшлись би мовчки. */
  ...
  if v_md5 is distinct from c_md5 or v_len is distinct from c_len then
    raise exception '0185: тіло сторожа після передруку % / % — очікували % / %',
      v_md5, v_len, c_md5, c_len;
  end if;

  /* Стверджуємо саме те, ЩО додали, а не «щось не зламалось»: нова гілка
     мусить бути присутня в тілі і мусить бути ЗЕЛЕНОЮ (перевірено вище). */
  if position('schema_digest' in (select p.prosrc from pg_proc p
       join pg_namespace n on n.oid = p.pronamespace
      where n.nspname = 'public' and p.proname = 'invariants_check')) = 0 then
    raise exception '0185: у тілі сторожа немає гілки schema_digest';
  end if;
end
$post$;
...
-- === ПІСЛЯ НАКАТУ ===
-- ============================================================================
-- ⚠️ КРОК 0 — ДО НАКАТУ, інакше пост-асерт не доводить нічого. `node
--    scripts/build-0185-reprint.mjs` мусить дати ПОРОЖНІЙ `git diff`. Бо і
--    встановлене тіло, і `c_md5` походять з ОДНОГО файла: правка руками плюс
--    повторний прогін генератора лишили б усе зеленим.
--
-- 1. `npm run db:gate` — ОДРАЗУ після накату. Без цього 0185 є на диску, але
--    без md5 у леджері, і БУДЬ-ЯКИЙ наступний деплой стане на «НЕ НАКАТАНО».
--    Це єдиний крок, що блокує прод.
-- 2. Перезняти піни смоуку `supabase/smoke/gcal_pg_cron_smoke.sql`.
--    ⚠️ ЧИСЕЛ ТУТ НЕМАЄ СВІДОМО. Перша редакція цього хвоста їх переписувала
--    рукою — і вони протухли рівно від однієї правки коментаря всередині
--    гілки (третій раз за пакет той самий клас: константа md5 у 0184,
--    `trim()` у нормалізації, ці рядки). Джерело істини ОДНЕ:
--      • `g2` і довжина — рядки `c_md5` / `c_len` у `do $post$` вище, їх
--        вписує генератор;
--      • `g` (нормалізований) друкує той самий генератор,
--        `node scripts/build-0185-reprint.mjs`, і прилад для нього звірено на
--        ВІДОМІЙ відповіді: та сама формула на тілі 0184 відтворює 673861da…
--    Дописати рядок провенансу з ПОПЕРЕДНІМИ значеннями — історія не мусить
--    обриватись на найцікавішому місці.
-- 3. `node scripts/bump-checked-pins.mjs 22 23`.
--    ⚠️ ВІН ХОДИТЬ ЛИШЕ ПО `supabase/smoke/*.sql`. Руками, бо він туди не
--    заходить: `docs/ops-cron.md` рядки 38 і 97 («22 перевірок», «22
--    перевірки сьогодні»). Їх пінить `tests/invariantsCheckedPins.test.ts`
--    (`LIVE_DOCS`), і число він бере з ОСТАННЬОГО передруку В ДЕРЕВІ, тобто
--    червоніє щойно файл 0185 зʼявився в дереві — ще ДО накату.
--    ⚠️ ЗВІДСИ ПОРЯДОК КОМІТУ: файл міграції, бамп пінів і правка
--    `docs/ops-cron.md` мусять бути в ОДНОМУ коміті, а смоуки в цьому вікні
--    не ганяти — вони читають ПРОД і до накату законно вимагають 22.
-- 4. `npm test` — ловить `migrationGate.test.ts` і `invariantsCheckedPins`.
-- 5. Прогнати смоуки, що пінять `checked` (їх девʼять, усі в `supabase/smoke`).
-- 6. Повна ревізія стендів: `node scripts/falsify-all.mjs`.
--    ⚠️ `EXPECTED_STANDS` піднімати з 34 до 35 ЛИШЕ разом із появою
--    `scripts/falsify-0185.mjs`. Підняти число без стенда = вічний
--    `countMismatch` і exit 1.
-- 7. `docs/audit/PR-0185-schema-digest.md` — у кожної міграції з 0166 такий є.
--
-- ВІДКІТ:
--   1. повернути тіло сторожа з 0184 і перевірити ЗАПИТОМ, що md5 без CR
--      знову `11a297318da068f53b113c3d9120e6b8`, довжина `97025`, checked 22;
--   2. `delete from public.migration_ledger where name = '0185_schema_digest.sql';`
--   3. ⚠️ ПРИБРАТИ файл 0185 з диска АБО лишити його разом із рядком леджера —
--      інакше `db:gate` дасть «НЕ НАКАТАНО» (файл є, рядка немає) і закриє
--      деплой;
--   4. повернути піни смоуків і `docs/ops-cron.md` на 22.
-- ============================================================================
