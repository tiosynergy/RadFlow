-- ============================================================================
-- 0179_rf09_definer_and_audit.sql
-- ============================================================================
--  RF-09 (High), канали b і c — те, що 0178 НЕ закрила і чесно про це сказала.
--  Плюс новий канал d, знайдений с59 при читанні роута (закритий КОДОМ у тому
--  самому пакеті 38: /api/ceo/grant більше не віддає й не видає токен
--  наявному акаунту — див. tests/ceoGrantRoute.test.ts).
--
--  RF-09b. `ceo_list_for_clinic(uuid)` — SECURITY DEFINER, EXECUTE у
--  `authenticated`, віддавала `invite_token` для `role='ceo'`. Колонковий
--  грант 0178 на definer-функцію НЕ поширюється (вона виконується правами
--  власника). Ескалація: CEO з активним грантом на дві клініки — адмін
--  клініки B читає його ЖИВИЙ токен і захоплює акаунт, що бачить клініку A.
--  Тихо: без скидання пароля, без сліду. Заміряно на проді 07.09: активних
--  CEO-грантів 1, CEO з двома клініками 0, живих токенів 0 — не горить, але
--  кожне нове «Скинути пароль» відкриває вікно.
--
--  RF-09c. `fn_audit()` пише рядок `profiles` ЦІЛКОМ (`to_jsonb(old/new)`),
--  разом із токеном. Заміряно 07.09: 74 записи по profiles, у 4 токен не NULL,
--  у 2 з них проставлено clinic_id → їх бачить адмін клініки і будь-який CEO
--  з активним грантом на неї (audit_read_admin / audit_read_ceo). Ті 4
--  значення мертві (усі 9 профілів мають password_set=true, а set-password
--  вимагає false) — але кожне НОВЕ запрошення писало б туди живий токен.
--
--  ЩО РОБИТЬ ЦЯ МІГРАЦІЯ
--    1. `ceo_list_for_clinic` — сигнатура та сама (RPC-контракт і типи не
--       рвемо), але колонка `invite_token` тепер ЗАВЖДИ NULL. Екран
--       (CeoManager) уже переведений на карту токенів з відповідей роутів
--       (lib/inviteLink.ts) — крок 1 виката, до цієї міграції.
--       ACL перевидано явно + assert у ТІЙ САМІЙ транзакції (пастка 0122).
--    2. `fn_audit()` — `before`/`after` пишуться БЕЗ ключа `invite_token`
--       (`to_jsonb(x) - 'invite_token'`; на таблицях без такої колонки це
--       no-op, тож п'ять інших аудит-тригерів працюють як і раніше).
--    3. Зачистка ЧОТИРЬОХ історичних рядків audit_log — за явним переліком id
--       (before-images у docs/audit/PR-0179-rf09-definer-audit.md); smoke
--       перевіряє ДЕЛЬТУ через get diagnostics row_count = 4.
--    4. Передрук `invariants_check`:
--       • №15 priv_drift, дві нові гілки: (g) у audit_log немає жодного рядка з
--         НЕПОРОЖНІМ invite_token у before/after (значення, не ключ — ключ із
--         null стоїть у ~70 старих записах); (g2) жодна SECURITY DEFINER
--         функція public з EXECUTE у клієнтської ролі не торкається
--         `invite_token` — ні в сигнатурі, ні в аргументах, ні в тексті тіла —
--         і не повертає рядок `profiles` цілком; окрім названого винятку
--         `ceo_list_for_clinic`, чиє ТІЛО пінить №19 (тобто «колонка є, але
--         завжди NULL» — під наглядом дайджесту, а не чесного слова).
--         Названа межа: `returns jsonb` із `to_jsonb(p)` без слова
--         invite_token у тілі не ловиться;
--       • №19 guard_fn_bodies: новий дайджест `fn_audit()` і НОВИЙ рядок
--         `ceo_list_for_clinic(p_clinic uuid)` — зняти гейт `auth_is_admin()`
--         або повернути токен у select тепер червонить сторожа.
--       ⚠️ `checked` НЕ змінюється: 21 → 21 (розширено наявні перевірки).
...
--    • `fn_audit` і далі глотає БУДЬ-ЯКУ помилку (`exception when others then
--      null`) — «аудит мовчки не пишеться» не лікується тут; це Р2.
--    • `waitlist_entries.claim_token` НЕ секрет (транзієнтна мітка CAS з
--      0089, ніхто її не читає, жодна RPC не приймає її параметром; 0 рядків
--      із значенням) — заміряно с59, у пакет не входить.
--    • Логи (Vercel/Supabase access-логи з токеном в URL) — не дивились.
-- ============================================================================
...
begin
  if not exists (select 1 from public.migration_ledger
                  where name = '0178_rf09_invite_token_grant.sql') then
    raise exception '0179 потребує 0178 (накатуйте по порядку)';
  end if;
end
$ledger$;

-- ============================================================================
-- 1. RF-09b: ceo_list_for_clinic більше не віддає токен НІКОМУ
-- ============================================================================
...
begin
  ...
  -- ⚠️ Тіло цієї функції пінить invariants_check №19 (0179): зняти цей гейт —
  -- сторож червоніє тієї ж ночі.
  ...
  return query
    select
      ...
      p.note,
      p.password_set,
      -- RF-09b (0179): одноразовий токен НЕ віддаємо НІКОМУ. Definer-функція
      -- обходить колонковий грант 0178, а «лише для role='ceo'» (0044) означало
      -- «будь-якому адміну будь-якого з центрів цього CEO» — тихе захоплення
      -- акаунта, що бачить інші центри. Колонка лишається в сигнатурі заради
      -- RPC-контракту; екран бере токен з відповіді роута видачі.
...
-- Пастка 0122: після create or replace ACL перевидаємо ЯВНО і перевіряємо в
-- цій самій транзакції.
...
do $acl$
begin
  if has_function_privilege('anon', 'public.ceo_list_for_clinic(uuid)', 'EXECUTE') then
    raise exception '0179: anon має EXECUTE на ceo_list_for_clinic';
  end if;
  if not has_function_privilege('authenticated', 'public.ceo_list_for_clinic(uuid)', 'EXECUTE') then
    raise exception '0179: authenticated ВТРАТИВ EXECUTE на ceo_list_for_clinic — екран CEO порожній';
  end if;
  if position('invite_token' in (select prosrc from pg_proc
                                   where oid = 'public.ceo_list_for_clinic(uuid)'::regprocedure)) = 0 then
    raise exception '0179: тіло ceo_list_for_clinic без згадки invite_token — не та редакція';
  end if;
  if (select prosrc from pg_proc where oid = 'public.ceo_list_for_clinic(uuid)'::regprocedure)
       !~ 'null::text\s+as\s+invite_token' then
    raise exception '0179: ceo_list_for_clinic усе ще віддає p.invite_token';
  end if;
end
$acl$;

-- ============================================================================
-- 2. RF-09c: fn_audit пише рядок БЕЗ invite_token
-- ============================================================================
...
begin
  begin
    ...
    -- RF-09c (0179): одноразовий токен запрошення в аудит НЕ пишемо. `- 'key'`
    -- на jsonb без такого ключа — no-op, тож для п'яти інших таблиць із цим
    -- тригером поведінка не міняється ні на біт. Сам факт зміни рядка
    -- profiles (хто, коли, які ІНШІ поля) лишається в журналі.
    ...
    values (
      ...
      case when tg_op in ('UPDATE', 'DELETE') then to_jsonb(old) - 'invite_token' end,
      case when tg_op in ('INSERT', 'UPDATE') then to_jsonb(new) - 'invite_token' end
    );
...
-- ============================================================================
-- 3. Зачистка ІСТОРИЧНИХ рядків audit_log із токеном — за явним переліком id
-- ============================================================================
-- Перелік знято запитом 07.09.2026 (before-images — у PR-доці). Оновлюємо ЛИШЕ
-- ці id і ЛИШЕ ключ invite_token; smoke нижче звіряє дельту.
do $cleanup$
declare
  v_n int;
begin
  update public.audit_log
     set before = case when before ? 'invite_token' then before - 'invite_token' else before end,
         after  = case when after  ? 'invite_token' then after  - 'invite_token' else after  end
   where id in (2700, 2702, 3238, 3239)
     and table_name = 'profiles'
     and (coalesce(before->>'invite_token', '') <> '' or coalesce(after->>'invite_token', '') <> '');
  get diagnostics v_n = row_count;
  if v_n is distinct from 4 then
    raise exception '0179: зачистка audit_log зачепила % рядків, очікувалось 4', v_n;
  end if;
  if (select string_agg(id::text || ':' || md5(coalesce(before::text, '')) || ':' || md5(coalesce(after::text, '')), ',' order by id)
        from public.audit_log where id in (2700, 2702, 3238, 3239))
     is distinct from
     '2700:d41d8cd98f00b204e9800998ecf8427e:7bc199a895a64245afd49a3e9e9001fd,'
     '2702:7bc199a895a64245afd49a3e9e9001fd:c5ed01a2e4bdea2ad7a76cc4d9feb98b,'
     '3238:d41d8cd98f00b204e9800998ecf8427e:ab846c6843ddb78e5d2dcea8e2c84181,'
     '3239:ab846c6843ddb78e5d2dcea8e2c84181:69c91f9f1b4d2059db032c2239048fb4' then
    raise exception '0179: after-images зачистки не збігаються з заміряними 07.09 (див. PR-док)';
  end if;
  if exists (select 1 from public.audit_log
              where coalesce(before->>'invite_token', '') <> ''
                 or coalesce(after->>'invite_token', '') <> '') then
    raise exception '0179: у audit_log лишились рядки з токеном поза переліком';
  end if;
end
$cleanup$;

-- ============================================================================
-- 4. Передрук invariants_check: №15 (g)/(g2), №19 fn_audit + ceo_list_for_clinic
-- ============================================================================
...
values ('0179_rf09_definer_and_audit.sql')
...
-- === ПІСЛЯ НАКАТУ ===
-- ============================================================================
--   select public.invariants_check(false);   -- ok:true, checked = 21 (БЕЗ змін)
--   npm run db:gate                          -- 179/179
--   ⚠️ bump-checked-pins.mjs НЕ запускати: число перевірок не змінилось.
--   ⚠️ перезняти ОБИДВА md5-піни тіла invariants_check у
--      supabase/smoke/gcal_pg_cron_smoke.sql — передрук змінює md5 і довжину.
--   ⚠️ ПОВНА ревізія стендів обовʼязкова: передрук ламає стенди, прибиті до
--      попередньої редакції, і робить це МОВЧКИ.
--   select md5(btrim(regexp_replace(prosrc || coalesce(pg_get_function_sqlbody(oid)::text, ''), '\s+', ' ', 'g')))
--     from pg_proc where oid = 'public.ceo_list_for_clinic(uuid)'::regprocedure;  -- 4f3ee1ff…
--   … і те саме для 'public.fn_audit()'::regprocedure;                              -- ed1ddd90…
--
-- ============================================================================
-- === ЯК ПЕРЕВІРИТИ, ЩО ВОНО ПРАЦЮЄ (зонди з відкотом) ===
-- ============================================================================
-- Зонд 1. RPC не віддає токен навіть адміну свого центру (під його сесією):
--   select invite_token from public.ceo_list_for_clinic('<clinic>');  -- усі NULL
--
-- Зонд 2. Сторож ловить повернення токена в RPC (ВІДКОТИТИ ОДРАЗУ):
--   begin;
--     create or replace function public.ceo_list_for_clinic(p_clinic uuid) … p.invite_token …
--     select public.invariants_check(false);
--     -- очікуємо guard_fn_bodies → ['body:ceo_list_for_clinic(p_clinic uuid)->…']
--   rollback;
--
-- Зонд 3. Сторож ловить токен у аудиті (ВІДКОТИТИ ОДРАЗУ):
--   begin;
--     insert into public.audit_log(actor, clinic_id, table_name, row_id, action, before, after)
--     values (null, null, 'profiles', gen_random_uuid(), 'insert', null, '{"invite_token":"zz"}');
--     select public.invariants_check(false);
--     -- очікуємо priv_drift → ['audit_log_invite_token:<id>']
--   rollback;
--
-- Зонд 4. Сторож ловить НОВУ definer-RPC із токеном у результаті (ВІДКОТИТИ):
--   begin;
--     create function public.zz_probe() returns table(invite_token text)
--       language sql security definer set search_path = public as $$ select null::text $$;
--     grant execute on function public.zz_probe() to authenticated;
--     select public.invariants_check(false);
--     -- очікуємо priv_drift → ['definer_returns_invite_token:authenticated:zz_probe()']
--   rollback;
--
-- Зонд 5. Живий тригер: UPDATE profiles → новий рядок audit_log без ключа
--   invite_token (у транзакції з відкотом):
--   begin;
--     update public.profiles set note = coalesce(note, '') where id = (select id from public.profiles limit 1);
--     select (after ? 'invite_token') from public.audit_log order by id desc limit 1;  -- false
--   rollback;
--
-- ============================================================================
-- === ВІДКАТ ===
-- ============================================================================
-- ⚠️ Це відкат ЗАКРИТИХ КАНАЛІВ ДІРИ БЕЗПЕКИ (RF-09b/c). Історичні 4 рядки
--    audit_log відкоту НЕ підлягають — їх значення знищено навмисно.
--
-- begin;
--   -- ceo_list_for_clinic: редакція 0044 (з `case when p.role = 'ceo' then p.invite_token …`)
--   -- fn_audit: редакція 0140 (`to_jsonb(old)` / `to_jsonb(new)` без `- 'invite_token'`)
--   -- invariants_check: редакція 0178 (передрук цілком)
--   -- delete from public.migration_ledger where name = '0179_rf09_definer_and_audit.sql';
