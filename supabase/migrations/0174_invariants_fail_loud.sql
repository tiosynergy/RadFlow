-- ---------------------------------------------------------------------------
--  0174 — сторож більше не падає МОВЧКИ (закриття названої межі 0172)
--
--  ЩО. Передрук `public.invariants_check` БЕЗ зміни числа перевірок: кожна з
--  вісімнадцяти перевірок, які досі не мали обробника, обгорнута
--  `begin … exception when others then … end;`. Виняток тепер стає ЧЕРВОНИМ
--  порушником `raised:<sqlstate>:<текст>` під іменем СВОЄЇ перевірки, а не
--  валить увесь прогін.
--
--  ЧОМУ. Це найдорожча дірка з тих, що лишались, і 0172 назвав її сам: у тілі
--  був рівно ОДИН `exception when` (у №18, навколо живого виклику
--  `server_now`) плюс той, що 0172 дав №19. Виняток у будь-якій іншій
--  перевірці відкочував транзакцію ЦІЛКОМ:
--    • `select public.invariants_check()` повертав помилку;
--    • рядок у `public.maintenance_runs` не зʼявлявся ВЗАГАЛІ;
--    • у `cron.job_run_details` лишався `failed`, якого ніхто не читає.
--  Тобто зламати сторожа було ВИГІДНІШЕ, ніж покраснити його: червоне лежить
--  у `result->failed` і видно, а відсутність рядка не видно нікому. Тиша
--  читається як здоровʼя — це той самий клас, що «порожній журнал = сторож не
--  крутиться», записаний у самому тілі ще з 0154.
--
--  ⚠️ ЛІЧИЛЬНИК ЛИШАЄТЬСЯ ЗЗОВНІ обгортки. `v_n := v_n + 1;` стоїть ПЕРЕД
--     `begin`, тому перевірка, яка впала, все одно порахована: `checked` = 19
--     і при червоному. Інакше «19 → 18» довелося б відрізняти від «перевірку
--     прибрали», а це два різні дефекти з однаковим слідом.
--
--  ⚠️ ВІДСТУПИ НЕ ЗСУНУТІ — і це не недбалість, а умова перевірності.
--     Обгортка вставлена ОКРЕМИМИ рядками з маркером `/* 0174 */`, тіло
--     перевірок лишилось на своїх позиціях побайтно. Це дозволило поставити
--     найсильнішу постумову збірки: **прибрати всі рядки з маркером — і
--     виходить тіло 0173 ДОСЛІВНО**. Якби код поїхав на два пробіли вправо,
--     твердження «змінились рівно вісімнадцять обгорток» довелося б приймати
--     на віру: різниця була б у кожному рядку функції.
--
--  ⚠️ ЗАМІРЯНО ПЕРЕД ПРАВКОЮ:
--     • обробників у тілі 0173 — рівно 2 (№18 і №19), решта 18 голі;
--     • жодна з вісімнадцяти не містить власного `raise` — обгортка не міняє
--       семантику (перевірено збіркою, вона падає, якщо знайде);
--     • останній рядок коду кожного сегмента — `end if;`, тобто межа обгортки
--       однозначна (теж перевіряє збірка).
--
--  ⚠️ ФОРМА ОБРОБНИКА ЄДИНА НА ВСІ ВІСІМНАДЦЯТЬ, і вона трохи інша, ніж у №19
--     (0172): там обробник кладе текст у `v_tmp`, а звітує код НИЖЧЕ, поза
--     блоком; тут блок накриває і сам звіт, тож обробник дописує в `v_fail`
--     напряму. Причина не стилістична: єдина форма дозволила накрити ОДНИМ
--     правилом і №9, яка звітує з-під `if exists (…)` і `v_tmp` не має зовсім.
--     Переписувати №19 під нову форму не став: поведінка тотожна, а дифф виріс
--     би без жодної користі.
--
--  ⚠️ НАЗВАНА МЕЖА, яку цей передрук НЕ закриває: якщо впаде щось ПОЗА
--     перевірками — сама шапка, збирання `v_res` або `insert` у
--     `maintenance_runs`, — тиша повертається. Зовнішній сторож свіжості
--     (алерт «рядка job='invariants' немає понад 26 годин») лишається
--     потрібним і залежить від планувальника, а не від цієї функції.
--
...
begin
  if not exists (select 1 from public.migration_ledger
                  where name = '0173_audit_triggers_named.sql') then
    raise exception '0174 потребує 0173 (накатуйте по порядку)';
  end if;
end
$ledger$;
...
values ('0174_invariants_fail_loud.sql')
...
-- === ПІСЛЯ НАКАТУ ===
--
--   select public.invariants_check();   -- checked = 19 (НЕ 20)
--   npm run db:gate
...
--   ⚠️ Число перевірок НЕ мінялось — девʼять смоуків і пін-тест не чіпати.
--   ⚠️ ПОВНА ревізія стендів ОБОВʼЯЗКОВА (передрук міняє md5 тіла).
--
-- === ЯК ПЕРЕВІРИТИ, ЩО ВОНО ПРАЦЮЄ (зонд із відкотом) ===
--
--   Поставити в транзакції завідомо зламану редакцію однієї перевірки
--   (наприклад `1 / 0` всередині її запиту), викликати `invariants_check(false)`
--   і відкотити. МАЄ бути: функція ПОВЕРНУЛАСЬ, `checked = 19`, а в `failed`
--   лежить `{"check":"<імʼя>","offenders":["raised:22012:division by zero"]}`.
--   До 0174 той самий зонд давав ПОМИЛКУ виклику і жодного рядка в журналі.
--
-- === ВІДКАТ ===
--
-- ⚠️ Повернути тіло 0173 і зняти рядок леджера ЛИШЕ разом із видаленням файлу
--    з чекауту — інакше гейт збірки завалить «файл є, запису немає».
--
-- begin;
-- -- передрук invariants_check у редакції 0173 (без обгорток)
-- -- delete from public.migration_ledger where name = '0174_invariants_fail_loud.sql';
-- commit;
-- ---------------------------------------------------------------------------
