...
--
--  ⚠️ ⚠️ ЦЕЙ ФАЙЛ НЕ НАКАТУВАТИ. Передрук і самопін сторожа (перевірка №25) тут
--     — ДВА верхньорівневі стейтменти, тобто ДВІ транзакції. Обрив між ними
--     лишає нове тіло зі СТАРИМ піном, і №25 червоніє. Атомарний шлях один —
--     `scripts/frag/0200_apply.sql`, весь одним запитом.
--
--  ПОРЯДОК:
--   1. `scripts/frag/0200_dryrun.sql` → DRYRUN_0200_ROLLBACK, checked=25,
--      №19 і №25 зелені (`ledger_md5` червона ОЧІКУВАНО — md5 штампує гейт).
--   2. `scripts/frag/0200_apply.sql` → нотис APPLY_0200_OK.
--   3. ОКРЕМИМ запитом `select public.invariants_check(false);` — checked 25;
--      до `npm run db:gate` єдиний очікуваний порушник — `ledger_md5`.
--      ⚠️ Сім перевірок залежать від ДАНИХ і ЧАСУ (cron_*, outbox_*, gcal_*,
--         ucm_orphan_markers) і можуть почервоніти не від цього пакета.
--   4. `scripts/frag/0200_falsify.sql` → verdict=PASS: кожна з трьох мутацій
--      названа в №19 своїм підписом. Потім предстан — окремим запитом.
--   5. `npm run db:gate` (ЛИШЕ з машини власника), `npm test`, `tsc --noEmit`.
--   6. git ОДНИМ заходом: гілка → dev → main → push → штамп деплою. До пушу
--      в `main` кожна прод-збірка падає (рядок у леджері є, файла немає).
--   7. ⚠️ ПІСЛЯ КРОКУ 5 генератор НЕ ЗАПУСКАТИ: він перезаписує файл, чий md5
...
begin
  ...
  --
  --     ЩО ПІНИМО (сьогодні 43 підписи; ключ — імʼя РАЗОМ із типами
  ...
  --        0193 → 40, 0200 → 43), а заголовок ніхто не перечитував: 0192 оновила
...
comment on function public.invariants_check(boolean) is 'guard_body_md5=793ebcc08997fc54d36472cc3fd2ff9b;len=140125';
...
--     (d7f87fff05d4ec8cad62ae2f7df30d19 / 137081), САМОПІН повертається до 0198 (не
