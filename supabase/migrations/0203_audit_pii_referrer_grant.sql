...
--  `checked` 26 -> 26. Список №17: 23 -> 30; №19: 59 -> 60 (`guard_record_read_keys()`).
...
--      DRYRUN_0203_ROLLBACK з checked=26, ok17=true, zz_last=true, failed_before_ddl
--      = лише gcal_sync_overdue і guard_triggers із сімома `missing:`.
...
--      guard_md5 = 8c8e6403db7653949e03d026320c6099, guard_len = 170446, ledger_rows = 203,
...
--   5. `scripts/frag/0203_falsify.sql` → verdict=PASS (probes_ok+na = 36, n17=4,
...
begin
  ...
  if (select max(name) from public.migration_ledger) not in
     ('0202_tz_kyiv_no_catalog_scan.sql', '0203_audit_pii_referrer_grant.sql') then
  ...
  if md5(v_src) not in ('e1f1fdcfcea99906f02b0af193b814fa', '8c8e6403db7653949e03d026320c6099') then
  ...
  if to_regprocedure('public.guard_record_read_keys()') is not null
     and (select md5(replace(p.prosrc, chr(13), '')) from pg_proc p where p.oid = to_regprocedure('public.guard_record_read_keys()'))
         is distinct from '55f111f5d83e52b1c7970f06746d7ff6' then
    raise exception '0203: guard_record_read_keys() уже є з ІНШИМ тілом — чиясь чернетка? спершу розібратись';
  end if;
...
comment on function public.invariants_check(boolean) is 'guard_body_md5=8c8e6403db7653949e03d026320c6099;len=170446';
  ...
  select array_agg(e.value->>'check' order by e.value->>'check') into v_failed
   ...
   where e.value->>'check' not in ('gcal_sync_overdue', 'guard_triggers')
     and not (e.value->>'check' = 'ledger_md5'
              and e.value->'offenders' = jsonb_build_array('0203_audit_pii_referrer_grant.sql'));
...
begin
  ...
  select replace(p.prosrc, chr(13), '') into v_src
   ...
   where n.nspname = 'public' and p.proname = 'invariants_check'
     and pg_get_function_identity_arguments(p.oid) = 'p_write boolean';
  if md5(v_src) is distinct from '8c8e6403db7653949e03d026320c6099' or length(v_src) <> 170446
  ...
  if md5(v_q) is distinct from 'a477984fa5d7184f444433f309b8976f' then
  ...
  if not exists (
    select 1 from pg_proc p join pg_language l on l.oid = p.prolang
     where p.oid = 'public.guard_record_read_keys()'::regprocedure
       ...
       and md5(replace(p.prosrc, chr(13), '')) = '55f111f5d83e52b1c7970f06746d7ff6'
  ) then
    raise exception '0203: guard_record_read_keys() після створення не той (атрибути або сирий md5 тіла)';
  end if;
...
--     `guard_record_read_keys()`, тіло сторожа до 0202 (e1f1fdcfcea99906f02b0af193b814fa / 165537),
