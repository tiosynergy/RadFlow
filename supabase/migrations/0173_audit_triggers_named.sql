-- ---------------------------------------------------------------------------
--  0173 — аудит-тригери під іменем (закриття названої межі 0172)
--
--  ЩО. Передрук `public.invariants_check` БЕЗ зміни числа перевірок: у списку
--  №17 `guard_triggers` було 14 пар, стало 20 — додано шість аудит-тригерів
--  (`trg_audit_*`), усі шість викликають `fn_audit()`.
--
--  ⚠️ `checked` ЛИШАЄТЬСЯ 19. Це не косметика для звіту: піни числа перевірок
--     у ДЕВʼЯТИ смоуках і `tests/invariantsCheckedPins.test.ts` не рухаються
--     взагалі. Перезняти треба лише два md5-піни тіла в `gcal_pg_cron_smoke`
--     (кроки g і g2) — і `tests/guardTriggersInvariant.test.ts`, де інвентар
--     пар 14 → 20.
--
--  ЧОМУ. 0172 назвав цю межу сам: тіло `fn_audit` він пінить (№19), але тіло
--  не каже, що функція до чогось ПРИЦЕПЛЕНА, а в списку №17 аудит-тригерів не
--  було. Отже аудит-слід знімався однією командою.
--
--  ⚠️ ЗАМІРЯНО, А НЕ ПРИПУЩЕНО — зонд із відкотом на живому проді:
--     у транзакції знято `drop trigger trg_audit_profiles on public.profiles`
--     (тригерів 1 → 0), викликано `invariants_check(false)` →
--     **`ok:true, checked:19, failed:[]`**, і транзакцію відкочено `raise`.
--     Після відкоту: аудит-тригерів знову 6, тригерів `public` 76, сторож
--     зелений. Тобто дірка була справжня, а не теоретична.
--
--  ⚠️ ЩЕ ОДИН ЗАМІР, який виправив мою ж стару цифру: у 0172 записано
--     «підрядка `trg_audit` у тілі сторожа 0 разів». Сьогодні в СИРОМУ тілі її
--     ТРИ рази — це власний коментар 0172 про цю саму межу. У тілі БЕЗ
--     коментарів — 0. Числа з коментаря і числа з коду міряються по-різному, і
--     саме тому тут названо обидва.
--
--  ⚠️ ЩО ЗАЛИШАЄТЬСЯ МЕЖЕЮ (не пропуск):
--     • тригери ПОЗА цими двадцятьма — рішення власника про іменний список
--       лишається чинним (безсписковий інвентар усіх 76 зробив би кожну правку
--       ритуалом «допиши в список»);
--     • НОВА таблиця з PII, яка не отримає аудит-тригера, ніким не помічається:
--       список каже «ці шість мають бути», а не «аудит має бути скрізь, де
--       треба». Правило «де треба» — продуктове, і його ніхто не формулював;
--     • `set session_replication_role = 'replica'` гасить усі тригери, не
--       торкаючись каталогу — каталожна перевірка цього не бачить у принципі.
--
...
begin
  if not exists (select 1 from public.migration_ledger
                  where name = '0172_guard_fn_bodies_invariant.sql') then
    raise exception '0173 потребує 0172 (накатуйте по порядку)';
  end if;
end
$ledger$;
...
values ('0173_audit_triggers_named.sql')
...
-- === ПІСЛЯ НАКАТУ ===
--
--   select public.invariants_check();   -- checked = 19 (НЕ 20), guard_triggers
--                                       -- зелена, у списку 20 пар
--   npm run db:gate
...
--   ⚠️ ПОВНА ревізія стендів ОБОВʼЯЗКОВА: передрук міняє md5 тіла, а частина
--      стендів прибита до тексту сторожа (урок с55).
--
-- === ЯК ПЕРЕЗНЯТИ СПИСОК (коли тригер міняють СВІДОМО) ===
--
--   select string_agg(
--            format('      (%L,%L,%L)', c.relname, t.tgname,
--                   regexp_replace(pg_get_triggerdef(t.oid), '\s+', ' ', 'g')),
--            E',' || chr(10) order by c.relname, t.tgname)
--     from pg_trigger t
--     join pg_class c on c.oid = t.tgrelid
--     join pg_namespace n on n.oid = c.relnamespace
--    where n.nspname = 'public' and not t.tgisinternal
--      and (c.relname, t.tgname) in ( /* пари зі списку перевірки №17 */ );
--
--   ⚠️ Запит БЛАГОСЛОВЛЯЄ те, що зараз у проді. Перезнімати лише прочитавши
--      дифф очима. Новий список — у НОВУ міграцію: `db:gate` тримає md5.
--
-- === ВІДКАТ ===
--
-- ⚠️ Повернути тіло 0172 і зняти рядок леджера ЛИШЕ разом із видаленням файлу
--    з чекауту — інакше гейт збірки завалить «файл є, запису немає».
--
-- begin;
-- -- передрук invariants_check у редакції 0172 (список №17 — 14 пар)
-- -- delete from public.migration_ledger where name = '0173_audit_triggers_named.sql';
-- commit;
-- ---------------------------------------------------------------------------
