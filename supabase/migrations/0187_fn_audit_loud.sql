-- 0187_fn_audit_loud.sql
-- ============================================================================
--  RadFlow — Міграція 0187: `fn_audit` більше не ковтає помилки МОВЧКИ.
--  Запускати ПІСЛЯ 0186. Даних не змінює. Схеми не змінює.
--  Змінює ДВІ речі: тіло `fn_audit()` (один `raise warning`) і пін цього тіла
--  у списку №19 всередині `invariants_check`.
-- ============================================================================
--
--  ЩО БУЛО
--  -------
--  `fn_audit` — AFTER-тригер аудит-сліду на ШЕСТИ таблицях (`queue_entries`,
--  `waitlist_entries`, `incidents`, `profiles`, `referral_access`,
--  `ceo_access`). Вставка в `audit_log` була обгорнута в
--  `begin … exception when others then null; end` — тобто БУДЬ-ЯКА помилка
--  запису в журнал ковталась, не лишаючи сліду НІДЕ: ні в базі, ні в логах.
--  «Аудит мовчки не пишеться» неможливо було помітити інакше як ручною
--  звіркою рядків бойових таблиць із журналом.
--
--  Сама 0179 це назвала і відклала: «fn_audit і далі глотає БУДЬ-ЯКУ помилку
--  … не лікується тут; це Р2».
--
--  ЗАМІР 11.09.2026 (запитом, а не пам'яттю)
--  -----------------------------------------
--  Слідів спрацювання дірки НЕМАЄ. Живих рядків без жодної події в журналі —
--  чотири (`profiles` ×2, `referral_access`, `ceo_access`), і ВСІ чотири
--  створені 26.06–13.07, тобто ДО того, як журнал узагалі почався
--  (найраніша подія — 15.07.2026 18:30). Усе, що створене після появи
--  тригера, покрите повністю: `queue_entries` 329 живих / 0 без аудиту.
--  ⚠️ Це НЕ доказ, що дірка ніколи не спрацює: `audit_log` має ретеншен, тож
--  відсутність старої події — очікувана і не є ознакою збою. Доказ рівно
--  такий: за час життя тригера жодної НЕПОЯСНЕНОЇ дірки не знайдено.
--
--  ЩО ЗМІНЕНО І ЩО НЕ ЗМІНЕНО
--  --------------------------
--  ⚠️ ПОВЕДІНКА НЕ МІНЯЄТЬСЯ НІ НА БІТ. Помилка запису в журнал і далі
--  КОВТАЄТЬСЯ — рішення «цілісність робочого запису важливіша за рядок логу»
--  лишається чинним, і воно правильне: переповнений або зламаний `audit_log`
--  не має зупиняти запис пацієнтів у клініці. `raise warning` не перериває ні
--  транзакцію, ні тригер.
--  Змінюється РІВНО одне: дірка перестає бути невидимою. У серверний лог іде
--  рядок `AUDIT_FAILED table=… op=… row=… sqlstate=…`.
--
--  ⚠️ SQLERRM СВІДОМО НЕ ЛОГУЄМО — знахідка ДРУГОГО раунду ревʼю.
--  Повідомлення СУБД про невдалу вставку може нести фрагмент самого рядка:
--  `before`/`after` тут — це `to_jsonb` рядка `queue_entries`, тобто ПІБ і
--  телефон пацієнта. Серверний лог — не те місце, де їм бути, і додавати туди
--  новий канал для PII заради зручності діагностики не можна.
--  SQLSTATE + таблиця + операція + id рядка однозначно називають проблему і
--  дозволяють її відтворити, не виносячи вміст назовні.
--
--  ⚠️ МЕЖА, названа вголос (перший раунд ревʼю). Якщо сам `raise warning`
--  кинув би виняток, він пішов би повз обробник (внутрішній блок уже в
--  обробнику, зовнішній обробника не має) і ЗВАЛИВ БИ бізнес-запис. Аргументи
--  тут — тригерні змінні й `sqlstate`, форматування `%` по тексту; реального
--  шляху до винятку немає. Але шлях названий, бо його не існує за аргументом,
--  а не за конструкцією.
--
--  ЧОМУ ПЕРЕДРУК СТОРОЖА НА 1900+ РЯДКІВ РАДИ ОДНОГО РЯДКА
--  ------------------------------------------------------
--  Тіло `fn_audit` пінить перевірка №19 (`guard_fn_bodies`), а її список
--  живе ВСЕРЕДИНІ `invariants_check`. Змінити тіло, не змінивши пін, означає
--  лишити інваріант червоним назавжди. Канон проєкту — передрук цілком.
--  ⚠️ Передрук зібраний МЕХАНІЧНО: блок узято з 0185 (звірено — тіло в проді
--  збігається з ним байт у байт після нормалізації пробілів: довжина 111592,
--  md5 9666c0d1…) і застосовано РІВНО одну заміну піна. Жодного рядка руками.
-- ============================================================================
...
begin
  if not exists (select 1 from public.migration_ledger
    ...
    raise exception '0187 потребує 0186 (накатуйте по порядку)';
  end if;
  if exists (select 1 from public.migration_ledger
              where name = '0187_fn_audit_loud.sql') then
    raise exception '0187 вже накатана';
  end if;
end
$ledger$;

-- ============================================================================
-- 1. fn_audit: ковтаємо далі, але вже не мовчки.
-- ============================================================================
...
begin
  ...
  exception when others then
    -- с63: ковтаємо ДАЛІ (цілісність робочого запису важливіша за рядок логу),
    -- але вже НЕ МОВЧКИ. Дірка в аудит-сліді була невидимою повністю: ні в
    -- базі, ні в логах не лишалось нічого, тож «аудит тихо не пишеться» ніхто
    -- не міг помітити інакше як звіркою рядків.
    -- ⚠️ ПОВЕДІНКА НЕ МІНЯЄТЬСЯ НІ НА БІТ: `raise warning` не перериває ні
    -- транзакцію, ні тригер, і на бізнес-запис не впливає.
    -- ⚠️ SQLERRM СВІДОМО НЕ ЛОГУЄМО. Повідомлення СУБД про невдалу вставку
    -- може нести фрагмент самого рядка (before/after тут — це `to_jsonb` рядка
    -- queue_entries, тобто ПІБ і телефон пацієнта), а серверний лог — не те
    -- місце, де їм бути. SQLSTATE + таблиця + операція + id рядка однозначно
    -- називають проблему і відтворюють її, не виносячи вміст назовні.
    raise warning 'AUDIT_FAILED table=% op=% row=% sqlstate=%',
      tg_table_name, tg_op, v_row, sqlstate;
  end;
...
-- ============================================================================
-- 2. Передрук сторожа: у списку №19 пін тіла `fn_audit()` ed1ddd90e510b72a2bf001b7be9eaa37
--    → b1cd54ecfb2796b00e7b4f6c427752b2. Більше в цьому передруку НЕ ЗМІНЕНО
--    НІЧОГО: блок узято з 0185_schema_digest.sql дослівно і механічно, однією заміною.
-- ============================================================================
...
-- === ПІСЛЯ НАКАТУ ===
-- ============================================================================
-- 1. select md5(btrim(regexp_replace(p.prosrc, '\s+', ' ', 'g')))
--      from pg_proc p join pg_namespace n on n.oid = p.pronamespace
--     where n.nspname = 'public' and p.proname = 'fn_audit';
--    → b1cd54ecfb2796b00e7b4f6c427752b2 (було ed1ddd90e510b72a2bf001b7be9eaa37)
-- 2. select public.invariants_check(false);  →  ok:true, checked:23, failed:[]
--    ⚠️ Якщо тут `guard_fn_bodies` із offender `body:fn_audit()->…` — це і є
--    розхід тіла з піном: накотили секцію 1 без секції 2.
-- 3. Живий крок В ОБИДВА БОКИ (робиться зондом, не вручну):
--    • тимчасово ламаємо запис у журнал → у логах Postgres мусить зʼявитись
--      `WARNING: AUDIT_FAILED table=… sqlstate=…`, а бізнес-запис МУСИТЬ
--      пройти;
--    • після відновлення — рядок в `audit_log` пишеться як раніше.
-- 4. npm run db:gate   ← інакше рядок у ledger лишиться без md5.
-- ============================================================================
-- === ВІДКАТ ===
-- ============================================================================
-- ⚠️ Відкат повертає МОВЧАЗНЕ ковтання: дірка в аудит-сліді знову стає
--    невидимою. Поведінка запису при цьому не міняється ні до, ні після.
--
-- begin;
--   -- 1. fn_audit: редакція 0179 (обробник `null;` замість raise warning)
--   -- 2. invariants_check: редакція 0185 цілком (пін fn_audit()
--   --    → ed1ddd90e510b72a2bf001b7be9eaa37)
--   delete from public.migration_ledger where name = '0187_fn_audit_loud.sql';
-- commit;
--
