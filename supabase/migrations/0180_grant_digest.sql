-- ============================================================================
-- 0180_grant_digest.sql
-- ============================================================================
--  RF-04 (серпневий аудит): «гранти не стережуться — один GRANT мовчки
--  відкриває дірку». Точно: №15 стежив за НАЗВАНИМИ привілеями — TRUNCATE по
--  всьому каталогу (0166, гілка a), DELETE на простоях, колонковий ACL
--  `profiles` (0178). Будь-яка ІНША привілея на будь-якому іншому обʼєкті
--  лишалась поза наглядом.
--
--  ⚠️ І це НЕ теорія. Замір 07.09.2026, ще до написання цієї міграції:
--     `information_schema.role_table_grants` дає 236 рядків привілеїв
--     клієнтських ролей, а КАТАЛОГ (`pg_class.relacl`) — 284. Різниця рівно в
--     48 рядках `MAINTAIN` на 25 обʼєктах у `anon` і `authenticated`:
--       • `MAINTAIN` — привілея PostgreSQL 17 (VACUUM / ANALYZE / REINDEX /
--         CLUSTER / REFRESH MATVIEW / LOCK TABLE);
--       • у вʼюсі стандарту `information_schema` її НЕМАЄ ВЗАГАЛІ — тобто
--         будь-який аудит, побудований на information_schema (у тому числі
--         серпневий), її не бачив і не міг побачити;
--       • 0166 знімала `TRUNCATE` з клієнтських ролей і з дефолтного ACL
--         грантора `postgres` — а апгрейд на PG17 мовчки повернув на ту саму
--         поверхню `MAINTAIN` (`ALL` став ширшим), і жоден сторож не пискнув.
--     Практичного експлойта сьогодні немає: PostgREST не дає клієнтським
--     ролям виконати `LOCK TABLE`/`VACUUM` (немає поверхні довільного SQL),
--     матеріалізованих вʼюх у базі 0. Але це рівно той сценарій, проти якого
--     писався RF-04: поверхня розширилась сама, мовчки, без міграції.
--
--  ЩО РОБИТЬ ЦЯ МІГРАЦІЯ
--    1. Знімає `MAINTAIN` у `anon`/`authenticated` — з усіх обʼєктів схеми
--       public І з дефолтного ACL грантора `postgres` (щоб наступна таблиця
--       не отримала його знову). Дзеркало того, що 0166 зробила з TRUNCATE.
--       ⚠️ Дефолтний ACL грантора `supabase_admin` (arwdDxtm) лишається — у
--          нас немає членства в цій ролі; це названо ще в 0166 (зона b2).
--          Компенсація — пункт 2: новий обʼєкт червонить сторожа.
--       ⚠️ ПІД ЧАС НАКАТУ БУДЕ ~11 WARNING «no privileges could be revoked» /
--          «not all privileges could be revoked»: з 34 обʼєктів public девʼять
--          не мали MAINTAIN у жодної клієнтської ролі, ще два — лише в
--          `authenticated`. Це НОРМА для `revoke … on all tables`, не помилка;
--          доказ зняття — assert нижче, а не відсутність warning-ів.
--    2. Передрук `invariants_check` із НОВОЮ перевіркою №22 `grant_digest`:
--       увесь набір грантів клієнтських ролей ЗА КАТАЛОГОМ, чотири гілки —
--       t: таблиці/вʼюхи/foreign (48), s: секвенції (6), c: колонкові гранти
--       (4), f: definer-функції, доступні `anon` (11, дайджест «хто|власник|
--       md5 тіла»), — 69 ключів, fail-loud на new/changed/missing. Ролі НЕ
--       хардкодом: члени `authenticator` без `service_role`, плюс PUBLIC.
--       `WITH GRANT OPTION` входить у дайджест (`SELECT*`).
--       ⚠️ `checked` 21 → 22 — після накату `node scripts/bump-checked-pins.mjs 21 22`.
...
--    • НЕ звужує решту грантів. `anon` і далі має повний DML на `ceo_access`,
--      `referral_access`, `referrer_private`, `rate_limits`,
--      `clinic_deletion_requests`, `cities`, `rooms`, `services`,
--      `v_clinic_people` — заміряно 07.09: усі ці таблиці або мають RLS БЕЗ
--      жодної політики запису (rate_limits, clinic_deletion_requests —
--      deny-all), або політики лише на читання, тож гранти нічого не
--      відкривають СЬОГОДНІ. Це ратчет, а не прибирання: спершу зробити дрейф
--      видимим, звужувати — окремим пакетом із власним заміром на кожну
--      таблицю.
--    • НЕ пінить `service_role` (ключ бекенда, він і має обходити RLS) і не
--      пінить EXECUTE у `authenticated` на 44 definer-функціях (ростуть із
--      кожною фічею; постійно червоний сторож — видалений сторож, урок 0141).
--    • НЕ чіпає `pg_default_acl` грантора `supabase_admin` — недосяжно.
-- ============================================================================
...
begin
  if not exists (select 1 from public.migration_ledger
                  where name = '0179_rf09_definer_and_audit.sql') then
    raise exception '0180 потребує 0179 (накатуйте по порядку)';
  end if;
end
$ledger$;

-- ============================================================================
-- 1. RF-04 (а): MAINTAIN у клієнтських ролей — зняти
-- ============================================================================
do $maintain$
declare
  v_cnt int;
begin
  -- ⚠️ Локи: `revoke` бере на кожну таблицю RowExclusiveLock — читачів він НЕ
  --    блокує (AccessExclusiveLock у смоуку 0166 брав `create policy`, не
  --    grant/revoke; не переносьте той висновок сюди). Таймаут лишаємо як
  --    дешеву страховку: він діє до кінця ТРАНЗАКЦІЇ, тобто і на передрук
  --    сторожа нижче, і краще впасти за 4 с, ніж чекати на чужому DDL.
  set local lock_timeout = '4s';

  execute 'revoke maintain on all tables in schema public from anon, authenticated';
  -- Дефолтний ACL — ДВІЧІ: з іменем схеми і БЕЗ нього. `alter default
  -- privileges` без `in schema` лягає ОКРЕМИМ рядком із defaclnamespace = 0 і
  -- діє на public теж; 0167 уже ловив цю пастку в №15(b), і `revoke … in
  -- schema public` такого рядка НЕ знімає.
  -- ⚠️ Чесно: заміряно 07.09 — рядків із defaclnamespace = 0 у базі ЗАРАЗ
  --    нуль (усі 27 рядків іменовані), тож друга форма сьогодні НІЧОГО не
  --    знімає. Це профілактика на випадок, коли хтось видасть грант без
  --    `in schema`, а не виправлення наявного стану.
  ...
  if v_cnt <> 0 then
    raise exception '0180: MAINTAIN лишився у клієнтських ролей на % обʼєктах', v_cnt;
  end if;

  -- Дефолтний ACL грантора postgres більше не роздає MAINTAIN — перевіряємо
  -- РОЗБОРОМ acl (`aclexplode`), а не регуляркою по тексту: `anon=r*wm/postgres`
  -- (привілей із grant option) регулярка `[a-zA-Z]*m` не побачила б, бо `*`
  -- не входить у клас. Дзеркало №15(b): left join і defaclnamespace = 0.
  select count(*) into v_cnt
   ...
   where (d.defaclnamespace = 0 or n.nspname = 'public')
     ...
     and (a.grantee = 0 or a.grantee::regrole::text = any (array['anon', 'authenticated']));
  if v_cnt <> 0 then
    raise exception '0180: дефолтний ACL грантора postgres усе ще роздає MAINTAIN (% рядків)', v_cnt;
  end if;

  -- РОБОЧІ права не зачеплені (розтяжка в інший бік: revoke мав зняти рівно m).
  if not has_table_privilege('authenticated', 'public.queue_entries', 'SELECT')
     or not has_table_privilege('authenticated', 'public.queue_entries', 'INSERT')
     or not has_table_privilege('anon', 'public.clinics', 'SELECT')
     or not has_column_privilege('authenticated', 'public.profiles', 'login', 'SELECT') then
    raise exception '0180: revoke maintain зачепив робочі права — не накатувати';
  end if;
  -- І RF-09 не роз'їхалась тим самим рухом.
  if has_column_privilege('authenticated', 'public.profiles', 'invite_token', 'SELECT') then
    raise exception '0180: регрес RF-09 — invite_token знову читається клієнтом';
  end if;
end
$maintain$;

-- ============================================================================
-- 2. Передрук invariants_check: НОВА перевірка №22 grant_digest
-- ============================================================================
...
-- === ПІСЛЯ НАКАТУ ===
-- ============================================================================
-- ⚠️ ПОРЯДОК ВАЖЛИВИЙ: піни й доки — ДО стендів. `falsify-0166` перезнімає
--    частину своїх якорів із ОСТАННЬОГО передруку, тож запуск ревізії до
--    кроку 1 дає «ЯКІР НЕ ЗНАЙДЕНО» на позиціях, які насправді цілі.
--
-- 1. node scripts/bump-checked-pins.mjs 21 22
--    ⚠️ Скрипт ходить ЛИШЕ по supabase/smoke. Пін числа перевірок стоїть ще й
--       у ДОКУ, і його треба правити руками:
--         docs/ops-cron.md:38  «21 перевірок з 0177»  → «22 перевірок з 0180»,
--                              «міграція 0154–0177»   → «0154–0180»;
--         docs/ops-cron.md:97  «20 перевірок сьогодні» → «22» (бреше з 0177 —
--                              жоден канал tests/invariantsCheckedPins.test.ts
--                              цього рядка не бачить, бо в ньому немає слова
--                              «сторож»).
--    ⚠️ І провенанс, який bump НЕ чіпає:
--         supabase/smoke/room_busy_slots_scope_smoke.sql:35 «checked = 21 (0177)»
--         → номер міграції на 0180 (число bump підніме сам).
-- 2. Перезняти ДВА md5-піни тіла сторожа у supabase/smoke/gcal_pg_cron_smoke.sql
--    (нормалізований g і g2 без CR) — канон провенансу того ж смоуку: спершу
--    РОЗБОРОМ ФАЙЛА цієї міграції, ПРИЛАД звірити на 0179 (той самий розбір
--    мусить відтворити 71e552c2… / 5e468b5e…), і ТІЛЬКИ ПОТІМ підтвердити
--    живим запитом після накату:
--      select md5(replace(prosrc, chr(13), '')), length(prosrc)
--        from pg_proc where oid = 'public.invariants_check(boolean)'::regprocedure;
--    Там же оновити прозу: рядок «тіло прода != тіло файлу 0179» і абзац із
--    заміром 0179 — інакше вони брехатимуть номером (жоден канал їх не пінить).
-- 3. npm test  ← НЕ пропускати. Чотири статичні сторожі читають ОСТАННІЙ
--    передрук: invariantsCheckedPins, invariantsFailLoud (пінить маркер
--    `/* 0174 */` і рівність «обгорток = CHECKS-1»), guardFnBodiesInvariant,
--    privilegeSurface. Без цього кроку розходження видно лише в CI.
-- 4. npm run db:gate   (штампує md5 файлу в леджері)
-- 5. select public.invariants_check(false);  →  ok:true, checked:22, failed:[]
--    і окремо: select count(*) from pg_default_acl where defaclnamespace = 0;  → 0
-- 6. ПОВНА ревізія стендів: node scripts/falsify-all.mjs (передрук ламає якорі МОВЧКИ)
--
-- === ЯК ПЕРЕВІРИТИ, ЩО ВОНО ПРАЦЮЄ (зонди з відкотом) ===
-- ============================================================================
-- Зонд 1. Табличний грант (гілка t):
--   begin;
--     grant delete on public.queue_entries to authenticated;
--     select public.invariants_check(false);
--     -- очікуємо grant_digest → ['changed:t:queue_entries:authenticated:INSERT,…->DELETE,INSERT,…']
--   rollback;
--
-- Зонд 2. Колонковий грант (гілка c):
--   begin;
--     grant select (invite_token) on public.profiles to authenticated;
--     select public.invariants_check(false);
--     -- очікуємо grant_digest → ['changed:c:profiles:authenticated:SELECT:14:…->15:…']
--     -- ⚠️ і ОДНОЧАСНО priv_drift (f)/(f2) — це та сама колонка, яку пінить 0178;
--     --    два червоні сторожі на один грант, а не один.
--   rollback;
--
-- Зонд 3. Нова definer-функція, доступна anon (гілка f):
--   begin;
--     create function public.zz_probe() returns int language sql security definer
--       set search_path = public, pg_temp as 'select 1';
--     select public.invariants_check(false);
--     -- очікуємо grant_digest → ['new:f:zz_probe()->PUBLIC|postgres|<md5 тіла>']
--     --    (дефолт EXECUTE = PUBLIC, тому функція одразу доступна anon)
--     -- ⚠️ БЕЗ `set search_path` зонд червонив би ще й №2 secdef_search_path —
--     --    тоді він доводить не те, що обіцяє.
--   rollback;
--
-- Зонд 4. Зникла привілея (гілка t, бік missing/changed):
--   begin;
--     revoke select on public.cities from anon;
--     select public.invariants_check(false);
--     -- очікуємо grant_digest → ['changed:t:cities:anon:…,SELECT,…->DELETE,INSERT,REFERENCES,TRIGGER,UPDATE']
--   rollback;
--
-- Зонд 5. Повернення MAINTAIN (те, що ця міграція зняла):
--   begin;
--     grant maintain on public.queue_entries to anon;
--     select public.invariants_check(false);
--     -- очікуємо grant_digest → ['changed:t:queue_entries:anon:INSERT,…->INSERT,MAINTAIN,…']
--   rollback;
--
-- Зонд 6. НЕГАТИВНИЙ КОНТРОЛЬ (зелена база): без правок
--   select public.invariants_check(false);  →  ok:true, checked:22, failed:[]
--
-- ============================================================================
-- === ВІДКАТ ===
-- ============================================================================
-- ⚠️ Відкат №22 — це відмова від сторожа над грантами (RF-04 знову відкрита).
--    Відкат MAINTAIN — повернення привілеї, яка нікому не потрібна.
--
-- begin;
--   -- 1. Повернути MAINTAIN (якщо раптом щось на нього спиралось):
...
--   -- 2. invariants_check: редакція 0179 (передрук цілком, checked 21)
--   --    + node scripts/bump-checked-pins.mjs 22 21
--   -- delete from public.migration_ledger where name = '0180_grant_digest.sql';
