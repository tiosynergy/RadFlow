-- ============================================================================
-- 0182_rf02_invite_ttl.sql
-- ============================================================================
--  RF-02 (серпневий аудит), НАЗВАНИЙ ХВІСТ: «токен запрошення без TTL».
--  Сама RF-02 закрита ще в с32 атомарним claim-first; лишалось те, що
--  посилання-запрошення живе ВІЧНО, доки ним не скористались.
--
--  ⚠️ НАСКІЛЬКИ ГОРИТЬ — заміряно 08.09.2026, а не оцінено: живих токенів у
--     базі ЗАРАЗ **нуль** (профілів 9). Тобто сьогодні дірки немає — є
--     властивість, яка спрацює на першому ж запрошенні, що не дійшло до
--     адресата. Це ратчет, а не гасіння пожежі, і так і названо.
--
--  ЩО РОБИТЬ ЦЯ МІГРАЦІЯ
--    1. Колонка `profiles.invite_issued_at` — час ВИДАЧІ токена.
--       ⚠️ Клієнтським ролям НЕ роздається. Табличного гранта на `profiles`
--          немає з 0178 (RF-09), права видаються поколонково — тож нова
--          колонка не отримує їх автоматично. Перевірка №15(f2) вимагає, щоб
--          КОЖНА колонка, крім `invite_token`, лишалась читаною; тому виняток
--          розширено ДРУГИМ іменем — свідомо і в тій самій міграції.
--          ⚠️ Заміряно зондом ДО коду: `alter table profiles add column …` без
--             цієї правки дає рівно `profiles_column_not_granted:anon:…` і
--             `:authenticated:…`. Сторож ловить, як і мав.
--    2. Тригер `zz_invite_issued_at` + `guard_invite_issued_at()` — штамп
--       ставить БАЗА.
--       ⚠️ ЧОМУ ТРИГЕР, А НЕ ПʼЯТЬ РОУТІВ — це прямий урок пакета 41. Токен
--          пишуть щонайменше пʼять місць (`/api/staff`, `/api/staff/password`,
--          `/api/referrers/invite` двома гілками, `/api/ceo/grant`), і
--          «перелік місць завжди вужчий за дерево» спрацьовував у цьому
--          проєкті десять разів. Шостий шлях зʼявився б без штампа, і TTL
--          мовчки не діяв би саме для нього.
--       ⚠️ Імʼя на `zz_` НАВМИСНО: BEFORE-тригери спрацьовують за абеткою, і
--          `trg_guard_profile_privileges` мусить побачити рядок таким, яким
--          його прислав клієнт, ДО нашої правки.
--       ⚠️ `guard_profile_privileges` НЕ чіпаємо, і це рішення, а не забудьку-
--          ватість: штамп прибитий тим самим тригером, що його ставить
--          (гілка «токен не мінявся» повертає `old.invite_issued_at`), тож
--          клієнтська спроба зрушити термін НЕЙТРАЛІЗУЄТЬСЯ, а не лише
--          забороняється. Один механізм замість двох — канон проєкту.
--    3. Передрук `invariants_check`: №15(f2) виняток, №17 новий рядок
--       (існування і увімкненість штампа), №19 дайджест тіла (25 → 26).
...
--    • НЕ додає ліміт «за хешем токена», якого просить аудит. Токен — два
--      UUID без дефісів, 256 біт; перебір неможливий незалежно від лімітів, а
--      хто токен уже знає — тому вистачить однієї спроби. Будувати захист від
--      неможливого означало б поставити сторожа, який нічого не стереже.
--      ⚠️ Це СВІДОМА незгода з текстом аудиту, а не пропуск.
--    • НЕ гасить старі токени заднім числом: живих нуль (заміряно), гасити
--      нічого.
--    • Саме число TTL живе в КОДІ роута, а не в базі: змінити строк —
--      правка однієї константи без міграції. Названа межа: база стереже
--      наявність штампа, роут — його інтерпретацію.
-- ============================================================================
...
begin
  if not exists (select 1 from public.migration_ledger
                  where name = '0181_rf01_case_source_scope.sql') then
    raise exception '0182 потребує 0181 (накатуйте по порядку)';
  end if;
  if exists (select 1 from public.migration_ledger
              where name = '0182_rf02_invite_ttl.sql') then
    raise exception '0182 вже накатана';
  end if;
end
$ledger$;

-- ============================================================================
-- 1. Колонка часу видачі — і НЕ роздана клієнтам
-- ============================================================================
alter table public.profiles add column invite_issued_at timestamptz;

comment on column public.profiles.invite_issued_at is
  'RF-02 (0182): час видачі invite_token. Ставить тригер zz_invite_issued_at, '
  'не роути. NULL ⇔ токена немає. Клієнтським ролям НЕ роздається.';

do $col$
begin
  -- Клієнт колонку не читає — і це асерт, а не сподівання.
  if has_column_privilege('anon', 'public.profiles', 'invite_issued_at', 'SELECT')
     or has_column_privilege('authenticated', 'public.profiles', 'invite_issued_at', 'SELECT') then
    raise exception '0182: нова колонка роздана клієнтським ролям — так не задумано';
  end if;
  -- А робочі колонки цілі (дзеркало №15(f2) в ту саму мить).
  if not has_column_privilege('authenticated', 'public.profiles', 'login', 'SELECT')
     or not has_column_privilege('anon', 'public.profiles', 'role', 'SELECT') then
    raise exception '0182: поколонкові права profiles розʼїхались';
  end if;
  -- Живих токенів нуль (заміряно 08.09) — бекфіл нічого не чіпає, але
  -- ⚠️ якщо їх РАПТОМ не нуль, штамп треба поставити, інакше TTL відсіче
  --    їх усі як «без штампа». Робимо це ЯВНО, а не покладаємось на замір.
  update public.profiles
     set invite_issued_at = now()
   where invite_token is not null and invite_issued_at is null;
end
$col$;

-- ============================================================================
-- 2. Штамп ставить БАЗА: функція + тригер
-- ============================================================================
create or replace function public.guard_invite_issued_at()
returns trigger
language plpgsql
security definer
...
as $$
begin
  -- Сайд-ефекти інших тригерів не штампуємо: верхньорівнева дія вже пройшла
  -- цей самий шлях на своєму рядку.
  if pg_trigger_depth() > 1 then
    return new;
  end if;

  -- RF-02 (0182): час ВИДАЧІ запрошення ставить БАЗА, а не роут.
  -- ⚠️ ЧОМУ ТРИГЕР, А НЕ ПʼЯТЬ РОУТІВ. Токен пишуть щонайменше пʼять місць:
  --    /api/staff, /api/staff/password, /api/referrers/invite (дві гілки —
  --    створення і ПОВТОРНЕ використання наявного), /api/ceo/grant. Урок
  --    пакета 41 дослівно: перелік місць завжди вужчий за дерево, і саме
  --    тому перевірка джерела кейса переїхала з RPC у тригер. Шостий шлях
  --    зʼявиться без штампа — і TTL мовчки перестане діяти для нього.
  --    Тут же штамп не можна забути: він ставиться на ЗМІНУ КОЛОНКИ.
  -- ⚠️ DEFAULT не годиться: він спрацював би лише на INSERT, а токен
  --    перевидають UPDATE-ом на наявному профілі (див. /api/staff/password).
  if tg_op = 'INSERT' then
    if new.invite_token is not null then
      new.invite_issued_at := now();
    else
      new.invite_issued_at := null;
    end if;
    return new;
  end if;

  -- UPDATE: штамп рухається РАЗОМ із токеном і лише разом із ним.
  if new.invite_token is distinct from old.invite_token then
    if new.invite_token is null then
      -- Токен погашено (успішний set-password) — штамп теж прибираємо, щоб
      -- у базі не лишалось «коли було видано те, чого немає».
      new.invite_issued_at := null;
    else
      new.invite_issued_at := now();
    end if;
  else
    -- ⚠️ Токен НЕ мінявся → штамп НЕ ЧІПАЄМО і НЕ ДАЄМО ЧІПАТИ. Інакше
    --    оновлення будь-якого сусіднього поля (ПІБ, телефон) могло б
    --    посунути термін дії — тобто продовжити запрошення непомітно,
    --    рівно те, від чого TTL і захищає.
    new.invite_issued_at := old.invite_issued_at;
  end if;
...
/* ⚠️ REVOKE ОБОВʼЯЗКОВИЙ, І ЦЕ НЕ ПЕРЕСТРАХОВКА — ЗАМІРЯНО ЗОНДОМ.
   Postgres віддає EXECUTE на НОВУ функцію ролі PUBLIC за замовчуванням. Без
   цього рядка dry-run дав ТРИ порушення одразу:
     • №15  `definer_returns_invite_token:anon:guard_invite_issued_at()`
     • №15  те саме для `authenticated`
     • №22  `new:f:guard_invite_issued_at()->PUBLIC|postgres|…`
   Тобто штампувальний тригер став би ще однією anon-доступною SECURITY
   DEFINER функцією на таблиці, де лежать ролі. Правило `AGENTS.md` (ціна
   ратчета, заведене пакетом 40) каже дослівно: якщо обʼєкту не потрібні
   клієнтські гранти — `revoke` у ТІЙ САМІЙ міграції, і ключ у списку №22
   не зʼявляється взагалі. Саме цей шлях і обрано: тригерну функцію не
   викликають ззовні ніколи, її кличе тільки сам тригер. */
revoke all on function public.guard_invite_issued_at() from public, anon, authenticated;

drop trigger if exists zz_invite_issued_at on public.profiles;
create trigger zz_invite_issued_at
  before insert or update on public.profiles
  for each row execute function public.guard_invite_issued_at();

do $trg$
begin
  if not exists (select 1 from pg_trigger t
                  where t.tgrelid = 'public.profiles'::regclass
                    and t.tgname = 'zz_invite_issued_at' and t.tgenabled = 'O') then
    raise exception '0182: тригер штампа не створився або вимкнений';
  end if;
  -- Тіло на місці (дзеркало №19 у ту саму мить; коментарі зрізаємо — урок
  -- пакета 42: асерт на голому prosrc зеленіє від самої ПРОЗИ).
  if (select regexp_replace(prosrc, '(^|\n)\s*--[^\n]*', '\1', 'g')
        from pg_proc where oid = 'public.guard_invite_issued_at()'::regprocedure)
       not like '%new.invite_issued_at := old.invite_issued_at%' then
    raise exception '0182: у штампі немає гілки «токен не мінявся» — термін можна продовжити мовчки';
  end if;
  if (select prosecdef from pg_proc where oid = 'public.guard_invite_issued_at()'::regprocedure) is not true then
    raise exception '0182: штамп не SECURITY DEFINER';
  end if;
  -- І клієнтські ролі його НЕ кличуть (див. коментар про revoke вище).
  if has_function_privilege('anon', 'public.guard_invite_issued_at()', 'EXECUTE')
     or has_function_privilege('authenticated', 'public.guard_invite_issued_at()', 'EXECUTE') then
    raise exception '0182: штамп лишився anon-доступною definer-функцією — revoke не спрацював';
  end if;
end
$trg$;

/* ⚠️ ПОВЕДІНКОВОЇ РОЗТЯЖКИ ТУТ НЕМАЄ — І ЦЕ РІШЕННЯ, А НЕ ПРОПУСК.
   Перша редакція цієї секції вставляла справжній рядок у `profiles`, щоб
   довести штамп ділом. Власна перевірка ДО накату показала три причини цього
   не робити, і всі три заміряні запитом, а не згадані з памʼяті:
     • `profiles_id_fkey` веде на `auth.users(id)` — випадковий uuid не пройде;
     • `profiles_role_clinic_chk` вимагає центр для ролі `admin`;
     • `trg_audit_profiles` (AFTER INSERT/UPDATE/DELETE) лишив би ЧОТИРИ рядки
       про неіснуючий профіль у `audit_log` НАЗАВЖДИ — міграція комітиться.
   Тому поведінка доводиться ОКРЕМИМ зондом у транзакції з відкотом на
   СПРАВЖНЬОМУ профілі за явним id зі знімком «до» (канон власника), а не
   всередині накату. Команда — у хвості файла. */

-- ============================================================================
-- 3. Передрук invariants_check: №15(f2), №17 (новий рядок), №19 (25 → 26)
-- ============================================================================
...
values ('0182_rf02_invite_ttl.sql')
...
--    прилад звірити на 0181 (той самий розбір мусить відтворити
--    f157d9de…/19b47366… і довжину 95038), потім живим запитом.
-- 3. tests/guardFnBodiesInvariant.test.ts: PINNED +1 підпис (25 → 26):
--    `guard_invite_issued_at()`.
-- 4. supabase/types.ts: додати `invite_issued_at: string | null` у profiles
--    (файл РУКОПИСНИЙ — скрипта генерації в проєкті немає, перевірено).
-- 5. npm test  ← шість статичних сторожів читають ОСТАННІЙ передрук.
-- 6. npm run db:gate
-- 7. select public.invariants_check(false);  →  ok:true, checked:22, failed:[]
-- 8. ПОВНА ревізія стендів: node scripts/falsify-all.mjs
--
-- ============================================================================
-- === ЯК ПЕРЕВІРИТИ, ЩО ВОНО ПРАЦЮЄ (зонди з відкотом) ===
-- ============================================================================
-- Зонд 1. Штамп ставиться і рухається лише з токеном — уже В МІГРАЦІЇ (§2),
--   на справжньому рядку, в обидва боки, з прибиранням. Повторити можна так:
--   begin;
--     insert into public.profiles(id, login, full_name, role, approved,
--       password_set, invite_token)
--     values (gen_random_uuid(), 'zz1', 'зонд', 'admin', false, false, repeat('a',64));
--     -- invite_issued_at мусить бути НЕ null
--   rollback;
--
-- Зонд 2. Сторож ловить вихолощення штампа (№19):
--   begin;
--     create or replace function public.guard_invite_issued_at() returns trigger
--       language plpgsql security definer set search_path = public, pg_temp
--       as $probe$ begin return new; end $probe$;
--     select public.invariants_check(false);
--     -- очікуємо guard_fn_bodies → ['body:guard_invite_issued_at()']
--   rollback;
--
-- Зонд 3. Сторож ловить зняття тригера (№17):
--   begin;
--     drop trigger zz_invite_issued_at on public.profiles;
--     select public.invariants_check(false);
--     -- очікуємо guard_triggers → ['trigger_missing:profiles.zz_invite_issued_at']
--   rollback;
--
-- Зонд 4. Сторож ловить роздачу нової колонки клієнтам (№15):
--   begin;
--     grant select (invite_issued_at) on public.profiles to authenticated;
--     select public.invariants_check(false);
--     -- ⚠️ ОЧІКУЄМО ЗЕЛЕНЕ: №15(f2) стежить, щоб колонки НЕ ЗНИКАЛИ з роздачі,
--     --    а не щоб не додавались. Роздача НОВОЇ колонки — межа, названа тут.
--   rollback;
--
-- ============================================================================
-- === ВІДКАТ ===
-- ============================================================================
-- ⚠️ Відкат повертає ВІЧНЕ запрошення.
--
-- begin;
--   -- 1. drop trigger zz_invite_issued_at on public.profiles;
--   -- 2. drop function public.guard_invite_issued_at();
--   -- 3. alter table public.profiles drop column invite_issued_at;
--   -- 4. invariants_check: редакція 0181 (передрук цілком)
--   -- delete from public.migration_ledger where name = '0182_rf02_invite_ttl.sql';
-- commit;
--
-- ⚠️ ВІДКАТ У РЕПОЗИТОРІЇ (без цього дерево бреше про базу):
--   1. tests/guardFnBodiesInvariant.test.ts: PINNED 26 → 25.
--   2. supabase/smoke/gcal_pg_cron_smoke.sql: піни тіла редакції 0181
--      (g f157d9de…, g2 19b47366…, довжина 95038).
--   3. supabase/types.ts: прибрати `invite_issued_at`.
--   4. Роути /api/account/set-password: прибрати перевірку TTL і текст
--      «Термін дії посилання минув…».
--   5. scripts/falsify-0182.mjs прибрати, EXPECTED_STANDS повернути на 31.
