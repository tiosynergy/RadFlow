-- ---------------------------------------------------------------------------
--  0171 — гарди-тригери і server_now() під інваріантом (знахідка с55 + U-76)
--
--  ЩО. Передрук `public.invariants_check`: 16 перевірок → 18.
--    №17 `guard_triggers` — 14 названих гардів: на місці, УВІМКНЕНІ і дослівно
--        ті самі (пін на `pg_get_triggerdef`), плюс БЕЗСПИСКОВА гілка: жоден
--        не-внутрішній тригер `public` не сміє бути вимкненим.
--    №18 `server_now`     — годинник сервера: існує, викликається
--        `authenticated`, без зайвих грантів, не IMMUTABLE, тіло читає базу
--        і — головне — ЖИВИЙ ВИКЛИК віддає момент цієї ж транзакції.
--
--  ЧОМУ ОДНИМ ПЕРЕДРУКОМ. Кожен передрук `invariants_check` МОВЧКИ ламає всі
--  стенди і смоуки, прибиті до числа перевірок і до md5 тіла (с55: 19 замін у
--  девʼяти смоуках плюс пін тіла в `gcal_pg_cron_smoke`, і повна ревізія 24
--  стендів обовʼязкова). Дві перевірки одним передруком коштують ОДНІЄЇ
--  ревізії замість двох.
--
--  ⚠️ ЗАМІРЯНО ПЕРЕД ПРАВКОЮ, а не прочитано в доці:
--     • слова `tgenabled` у тілі сторожа — 0 згадок; у public 76 не-внутрішніх
--       тригерів, усі `tgenabled = 'O'` — зелена база;
--     • слова `server_now` у тілі сторожа — 0 згадок;
--     • ЗОНД ІЗ ВІДКОТОМ: тригер із `tgenabled = 'D'` віддає `exists = t` і всі
--       біти `tgtype` правильними. Тобто наявні перевірки тригерів сліпі до
--       ВИМКНЕННЯ за побудовою, а не через недогляд.
--
--  ⚠️ ХЕНДОФФ с55 СКАЗАВ ШИРШЕ, НІЖ Є, і це виправлено тут. Запис «жоден
--     guard-тригер не під інваріантом» невірний: розтяжка (e) в №15
--     `priv_drift` ВЖЕ пінить три `a01_no_client_delete` на існування і біти
--     `tgtype` (контроль: додавання `rooms` у її список дає `tripwire:rooms`).
--     Реально непокритим було інше — `tgenabled` для ВСІХ, разом із цими
--     трьома; сім PII-гардів і два referrer-гарди цілком; а
--     `trg_cleanup_orphan_clinic` стояв у каноні №8 лише на існування.
--
--  ⚠️ ЦЕ ДРУГА РЕДАКЦІЯ ПЕРЕВІРОК, і перша була слабша. Два раунди ревʼю з
--     різними лінзами знайшли в ній три дірки, КОЖНУ підтверджено запитом:
--     список колонок (`UPDATE OF …`, у шести гардів із чотирнадцяти) і `WHEN`
--     не кодуються в `tgtype`, а функція звірялась голим `proname` без схеми.
--     Тому пін тепер на `pg_get_triggerdef` цілком. Там же: ACL у №18 звіряв
--     літерал 'anon' замість членства, і жодна гілка не ВИКЛИКАЛА
--     `server_now()` — тіло `select now() + interval '2 hours'` проходило все.
--
...
begin
  if not exists (select 1 from public.migration_ledger
                  where name = '0170_policy_digest_invariant.sql') then
    raise exception '0171 потребує 0170 (накатуйте по порядку)';
  end if;
end
$ledger$;
...
values ('0171_guard_triggers_and_server_now.sql')
...
-- === ПІСЛЯ НАКАТУ ===
--
--   select public.invariants_check();   -- checked = 18, guard_triggers і
--                                       -- server_now зелені
--   npm run db:gate
...
--
-- === ЯК ПЕРЕЗНЯТИ СПИСОК ГАРДІВ (коли тригер міняють СВІДОМО) ===
--
--   select string_agg(
...
-- === ВІДКАТ ===
--
-- ⚠️ Повернути тіло 0170 і зняти рядок леджера ЛИШЕ разом із видаленням файлу
--    з чекауту — інакше гейт збірки завалить «файл є, запису немає».
--
-- begin;
-- -- передрук invariants_check у редакції 0170 (16 перевірок)
-- -- delete from public.migration_ledger
-- --  where name = '0171_guard_triggers_and_server_now.sql';
-- commit;
-- ---------------------------------------------------------------------------
