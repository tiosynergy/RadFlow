-- ============================================================================
-- 0178_rf09_invite_token_grant.sql
-- ============================================================================
--  RF-09 (High, знайдена пакетом 36; в аудиті 2026-08 її немає).
--
--  ДІРА. `profiles.invite_token` — одноразовий токен встановлення пароля —
--  читається БУДЬ-ЯКИМ співробітником клініки. Політика `profiles_select` =
--  `clinic_id = auth_clinic_id()`, грант SELECT у `authenticated` табличний,
--  отже реєстратор чи радіолог звичайним PostgREST-запитом бере токен колеги
--  у вікні між «адмін видав запрошення» і «людина задала пароль» і надсилає
--  його у `POST /api/account/set-password` — роут не вимагає нічого, крім
--  токена. Захоплення акаунта, зокрема АДМІНСЬКОГО.
--
--  ЩО САМЕ ЖИВЕ, а що ні — заміряно, а не переказано з доки:
--    • UPDATE токена клієнтом уже закритий тригером
--      `trg_guard_profile_privileges` (0064): службові поля пише лише сервер,
--      інакше 42501. Грант UPDATE на колонці існує, дірою він не є.
--    • `anon` не отримує рядків ЖОДНОЮ політикою: `auth_clinic_id()` і
--      `auth.uid()` для нього NULL. Грант знімаємо все одно — захист у
--      глибину: одна майбутня permissive-політика відкриє його назад.
--    • `profiles` НЕ входить у жодну realtime-публікацію → каналу витоку
--      через payload немає.
--    • Вʼюха `v_clinic_people` (security_invoker) читає лише дозволені
--      колонки; жодна політика ІНШОЇ таблиці не читає profiles напряму.
--    • Живих токенів на момент накату — 0 із 9 профілів. Не горить, але
--      вікно відкривається щоразу при «Скинути пароль».
--
--  ЧОМУ НЕ «просто відкликати колонку». ⚠️ ЗАМІРЯНО НА ЧЕРНЕТЦІ В ПРОДІ
--  (таблиця створена й видалена в тій самій сесії):
--
--      revoke select (secret) on <t> from authenticated;
--      → has_column_privilege(...,'secret','SELECT') = TRUE   -- нічого не сталось
--
--  Postgres НЕ віднімає колонкове право від табличного гранта. Знімає доступ
--  лише пара:
--
--      revoke select on <t> from anon, authenticated;
--      grant  select (<перелік дозволених колонок>) on <t> to anon, authenticated;
--      → select *              → 42501 permission denied
--      → select id,keep1,keep2 → ПРОХОДИТЬ
--      → select secret         → 42501 permission denied
--      → count(*)              → ПРОХОДИТЬ
--
--  ЦІНА, названа вголос: `select *` по profiles більше не працює під
--  клієнтською роллю. Перевірено зустрічно — у дереві НЕМАЄ ЖОДНОГО
--  `select("*")` по profiles: усі 58 місць перелічують колонки явно, а
--  чотири роути видачі запрошення (`/api/staff`, `/api/staff/password`,
--  `/api/referrers/invite`, `/api/ceo/grant`) працюють під service_role,
--  якого це не торкається.
--
--  ЩО РОБИТЬ ЦЯ МІГРАЦІЯ
--    1. Знімає табличний SELECT у `anon` і `authenticated` на `public.profiles`
--       і повертає його поколонково на 14 колонок із 15 — усі, крім
--       `invite_token`. UPDATE/INSERT/DELETE не чіпаються (їх стереже 0064).
--       `service_role` не чіпається взагалі.
--    2. Розширює сторожа `priv_drift` (перевірка №15) трьома гілками — щоб
--       грант більше не дрейфував МОВЧКИ. Це закриває і претензію RF-04
--       «грант не стережеться» рівно в тій точці, де вона жива.
--       ⚠️ `checked` НЕ змінюється: 21 → 21. Нових перевірок немає, розширено
--       наявну. Тобто `bump-checked-pins.mjs` запускати НЕ треба.
--    3. Передрук `invariants_check` цілком — канон 0142.
--       ⚠️ Передрук ламає стенди, прибиті до попередньої редакції, і робить це
--       МОВЧКИ. Повна ревізія стендів обовʼязкова.
-- ============================================================================
...
begin
  if not exists (select 1 from public.migration_ledger
                  where name = '0177_realtime_filter_premise.sql') then
    raise exception '0178 потребує 0177 (накатуйте по порядку)';
  end if;
end
$ledger$;

-- ============================================================================
-- 1. RF-09: поколонковий allow-list замість табличного SELECT
-- ============================================================================
-- Перелік ЯВНИЙ, а не `except invite_token` через DO-цикл: явний перелік
-- читається в дифі й у ревʼю, а згенерований — ні. Нова колонка навмисно НЕ
-- потрапляє сюди сама: сторож (f2) почервоніє і вимагатиме рішення.
revoke select on public.profiles from anon, authenticated;

grant select (
  id, clinic_id, full_name, email, phone, role, created_at, approved,
  login, note, workplace, password_set, city, contact_email
) on public.profiles to anon, authenticated;

-- PostgREST тримає привілеї в кеші схеми. Supabase перезбирає його за
-- ddl_command_end, але GRANT/REVOKE — саме той випадок, де мовчазний
-- застарілий кеш дав би 42501 живим екранам. Повідомляємо явно.
notify pgrst, 'reload schema';

-- ============================================================================
-- 2. Передрук invariants_check із гілками (f)/(f2)/(f3) у priv_drift
-- ============================================================================
...
values ('0178_rf09_invite_token_grant.sql')
...
-- ============================================================================
-- Зонд 1. Грант знято рівно з однієї колонки і рівно у клієнтських ролей.
--   select
--     has_column_privilege('authenticated','public.profiles','invite_token','SELECT') as a, -- false
--     has_column_privilege('anon','public.profiles','invite_token','SELECT')          as b, -- false
--     has_column_privilege('authenticated','public.profiles','login','SELECT')        as c, -- true
--     has_column_privilege('authenticated','public.profiles','role','SELECT')         as d, -- true
--     has_table_privilege('service_role','public.profiles','SELECT')                  as e, -- true
--     has_column_privilege('authenticated','public.profiles','invite_token','UPDATE') as f; -- true (стереже 0064)
--
-- Зонд 2. Живий запит під роллю — обидва напрями, у транзакції з відкотом:
--   begin;
--     set local role authenticated;
--     select invite_token from public.profiles limit 1;  -- очікуємо 42501
--     select id, login, role from public.profiles limit 1; -- очікуємо проходження
--   rollback;
--
-- Зонд 3. Сторож ловить повернення гранта (ВІДКОТИТИ ОДРАЗУ):
--   begin;
--     grant select on public.profiles to authenticated;
--     select public.invariants_check(false);
--     -- очікуємо priv_drift → ['invite_token_readable:authenticated']
--   rollback;
--
-- Зонд 4. Сторож ловить НЕвиданий грант на новій колонці (ВІДКОТИТИ ОДРАЗУ):
--   begin;
--     alter table public.profiles add column zz_probe text;
--     select public.invariants_check(false);
--     -- очікуємо priv_drift → ['profiles_column_not_granted:anon:zz_probe',
--     --                        'profiles_column_not_granted:authenticated:zz_probe']
--   rollback;
--
-- ============================================================================
-- === ВІДКАТ ===
-- ============================================================================
-- ⚠️ Це відкат ЗАКРИТОЇ ДІРИ БЕЗПЕКИ. Відкочувати лише разом із кодом, і лише
--    якщо екрани справді зламались — інакше токен запрошення знову читає вся
--    клініка. Спершу спробуйте точковий грант на конкретну колонку.
--
-- begin;
--   grant select on public.profiles to anon, authenticated;
--   -- і повернути редакцію invariants_check із 0177 (передрук цілком)
-- commit;
