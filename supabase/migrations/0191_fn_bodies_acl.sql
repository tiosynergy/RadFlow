...
--
-- Виконуваний фрагмент: scripts/frag/0191_rollback.sql — дзеркало накату з
-- тими самими асертами (предстан → результат 9680c291…/112207, сторож
-- зелений, другий замір із КАТАЛОГУ).
--
-- ⚠️ ВІДКАТ У РЕПОЗИТОРІЇ (без цього дерево бреше про базу):
--   1. видалити цей файл і перезапустити `npm run db:gate`;
--   2. прибрати ТРИ сигнатури зі списку PINNED у
--      tests/guardFnBodiesInvariant.test.ts — `auth_can_refer(c uuid)`,
--      `auth_referrer_visible_rooms()`, `auth_referrer_clinics()` — і пін
--      поля `;acl=`. ⚠️ ПОРЯДОК: спершу PINNED, потім видалення файла; інакше
--      `npm test` червоний у проміжку;
...
--         `/sched_override_read/` — цей рядок у списку є й до 0191;
--   4. `npm test` — упадуть саме ті тести, що читають останній передрук;
--   5. зняти згадки пінів `0a178709…` / `5f3226aa…` / `ef77618a…` з `claude/*`
