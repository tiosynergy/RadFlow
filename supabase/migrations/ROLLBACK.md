...
## Откат 0146 (приём статусов от RIS: integration_apply_status)

> Порядок обратный накату: **сначала клиент, потом БД**. Роут
> `POST /api/integrations/v1/appointments/{id}/events` — единственный
> вызыватель RPC; если дропнуть функцию раньше, чем выкатить код без
> роута, каждое событие RIS будет получать 500 (`42883`), а не честный
> 404, и мост начнёт копить ретраи. Порядок: убрать роут (деплой на
> Vercel) → предупредить интегратора → дропнуть функцию.
>
> ⚠️ `inbound_events` НЕ чистить: это журнал идемпотентности. После
> повторного наката те же `source_event_id` должны по-прежнему
> распознаваться как повторы — иначе RIS при первом же ретрае прогонит
> заново всю накопленную очередь событий и передвинет статусы задним
> числом. Таблица создана в 0144 и откату 0146 не подлежит.
>
> ⚠️ `important_events` с типом `integration.status_applied` НЕ удалять:
> журнал действий — историческая запись, а не кэш. Код после отката
> покажет такие карточки как есть (в `lib/journalText.ts` тип остаётся;
> если убирается и клиентский код — карточка отрисуется сырым типом,
> `eventTitle` на неизвестном типе не падает).
>
> Статусы, уже проставленные интеграцией, откатом НЕ возвращаются: это
> обычные переходы очереди, сделанные легально через триггеры. Обратный
> ход — только руками через интерфейс.
>
> 0146 откатывать **до** 0144 (RPC пишет в `inbound_events`) и в любом
> порядке относительно 0145 (эмиссия вебхуков от RPC не зависит; но
> учтите: пока 0145 жив, каждое применённое событие RIS порождало
> исходящий `integration.updated` — после отката 0146 этот источник
> изменений просто исчезает).
>
> Строка леджера: файл остаётся в чекауте → строку НЕ удалять (гейт:
> файл без записи = fail сборки); файл убирается → удалить обязательно.
> После отката перегенерировать/поправить `supabase/types.ts` (блок
> `Functions.integration_apply_status`) — иначе tsc обещает RPC, которой
> в БД нет.
...
-- Диагностика ПЕРЕД дропом: сколько событий канал уже применил
-- (0 = канал так и не заработал, откат безболезнен).
-- select result, count(*) from public.inbound_events group by 1 order by 2 desc;

drop function if exists public.integration_apply_status(
  uuid, uuid, uuid, text, text, timestamptz, text);

-- только если файл 0146 тоже убирается из чекаута:
...
> `db:gate`/`db:gate:check`). ⚠️ `RADFLOW_SKIP_MIGRATION_GATE=1` для этого
> НЕ годится: с с59 (RF-05, пакет 39) на Vercel эта переменная — exit 1, а не
> обход; прод-сборка с ней упадёт до `next build`.
