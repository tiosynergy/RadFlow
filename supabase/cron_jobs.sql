...
--     ⚠️ Твердження попередньої редакції («на гарди це не впливає:
--     check_not_during_incident рахує ВІКНО, а не статус») ХИБНЕ і виправлене
--     тут же (ревʼю пакета Ф4-5): гард рахує І вікно, І статус — у ньому стоїть
--     `i.status in ('active','planned')`. Тобто переведення простою в `resolved`
--     знімає блокування навіть усередині його власного вікна. Це і є задум
--     крона, але формулювати його «статус не впливає» не можна: та сама
