...
--
-- ✅ УЖЕ ВЫПОЛНЕН на проде 2026-07-27 20:29:49 UTC (аудит-лог: 27 удалений в
--    queue_entries + 1 в waitlist_entries одной транзакцией). Повторный запуск
--    БЕЗОПАСЕН: страховка ниже увидит «0 из 27» и откажется что-либо трогать.
--    Файл оставлен как история и как образец приёма. НЕ ПЕРЕИСПОЛЬЗУЙ список ID
--    для новой чистки — собери новый снимок.
--
-- ⚠️ Урок первой редакции. Она удаляла кейсы условием «нет ни одной записи» БЕЗ
--    ограничения списком — и снесла заодно посторонний осиротевший кейс, которого
--    в снимке не было (кейсов стало 3 вместо ожидаемых 4). Восстановить нечем:
--    на patient_cases нет аудит-триггера. Отсюда правило: удаляй ровно то, что
--    перечислено в снимке, а не «всё, что подходит под условие».
--
...
-- «tetsttest», «Кейс Фінал Тест» и т.п.), связанный вейтлист и кейсы, которые
-- после этого останутся без единой записи. Они портят KPI CeoDashboard: из 28
-- завершённых исследований 7 — тестовые (25%), а две записи с датой 2026-07-28
-- висят на живых досках.
--
-- ⚠️ ПОЧЕМУ ВСЁ В ОДНОМ DO-БЛОКЕ, А НЕ ЧЕРЕЗ TEMP-ТАБЛИЦУ И begin/commit.
-- Первая редакция скрипта складывала список ID во временную таблицу и
-- обрамлялась `begin; … commit;`. В Supabase SQL Editor это падало с
-- «42P01: relation "_cleanup_ids" does not exist»: редактор ходит в базу через
-- пул в режиме transaction pooling, соседние операторы одного скрипта могут
-- попасть на РАЗНЫЕ backend-соединения, а temp-таблица живёт в рамках одного.
-- По той же причине там ненадёжны явные begin/commit — редактор уже открыл
-- свою транзакцию. Один DO-блок = один оператор: разъехаться нечему, и он
-- атомарен сам по себе (исключение внутри откатывает все три удаления).
--
...
--
-- ⚠️ Кейсы удаляются ТОЛЬКО из числа привязанных к этим 27 записям. Вариант
-- «удалить все кейсы без записей» шире задачи: на 28.07 в базе уже есть один
-- осиротевший кейс, к тестовым данным отношения не имеющий, и он бы ушёл
-- заодно. Набор кейсов снимается ДО удаления очереди — после удаления записей
-- связь `queue_entries.case_id` исчезнет и восстановить набор будет неоткуда.
--
...
--
-- Dry-run на проде выполнен: waitlist 1, queue 27, cases 8;
-- queue 140 → 113, done 28 → 21, waitlist 12 → 11, cases 12 → 4.
-- ============================================================================
...
declare
  ...
  v_cases uuid[];
  v_found int; v_named int;
  v_wl int; v_q int; v_c int;
begin
  -- Страховка от протухшего снимка: если хоть одна строка успела исчезнуть или
  -- сменить имя — падаем целиком, а не удаляем «что нашлось».
  select count(*) into v_found from public.queue_entries where id = any(v_ids);
  if v_found <> 27 then
    raise exception 'СТОП: в базе % из 27 записей снимка — список протух, пересоберите', v_found;
  end if;

  select count(*) into v_named from public.queue_entries
   where id = any(v_ids) and patient_name ~* '(тест|test|keys)';
  if v_named <> 27 then
    raise exception 'СТОП: % из 27 записей уже не выглядят тестовыми — проверьте вручную', v_named;
  end if;

  -- Кейсы этих записей — снимок ДО удаления очереди.
  select array_agg(distinct case_id) into v_cases
    from public.queue_entries where id = any(v_ids) and case_id is not null;

  -- 1. Вейтлист: собственные тестовые брони + всё, что ссылается на удаляемые.
  delete from public.waitlist_entries
   where patient_name ~* '(тест|test)'
      or scheduled_entry_id = any(v_ids)
      or source_entry_id    = any(v_ids);
  ...
  -- 3. Кейсы ТОЛЬКО из снимка и только те, где не осталось ни одной записи
  --    (в тестовом кейсе мог быть и живой шаг — такой кейс не трогаем).
  delete from public.patient_cases pc
   where pc.id = any(v_cases)
     and not exists (select 1 from public.queue_entries q where q.case_id = pc.id);
  get diagnostics v_c = row_count;

  raise notice 'ГОТОВО: вейтлист %, очередь %, кейсы %', v_wl, v_q, v_c;
end $$;

-- Отчёт. Ожидаемо: queue_left=113, done_left=21, waitlist_left=11,
-- cases_left=4 (посторонний осиротевший кейс остаётся), test_left=0.
