...
const MUTATIONS = [
  {
    ...
    /* ⚠️ БУЛО `/пінів рівно стільки/` — і це називало ЛІЧИЛЬНИК, а не рядок.
       ...
       один рядок при оновленні якоря (а шістнадцять TRIG-рядків мають
       побайтово однаковий хвіст `attrs` і різняться лише іменем та md5)
       видалив би ЧУЖИЙ пін — і стенд надрукував би ✅. Тепер `expect` тримає
       ІМʼЯ: підпис приходить із назви тесту `it.each(PINNED)`. Знайдено
       ревʼю фальсифікації в с67. A2 нижче лишається на лічильнику навмисно —
       він сушить інвентар у ТЕСТІ, де цього імені вже немає. */
    expect: /sched_override_read/,
