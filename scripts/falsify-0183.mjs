...
const MUTATIONS = [
  {
    ...
    /* ⚠️ БУЛО `/пінів рівно стільки/` — і це називало ЛІЧИЛЬНИК, а не рядок.
       Той пін червоніє від зникнення БУДЬ-ЯКОГО з 33 рядків, тож промах на
       один рядок при оновленні якоря видалив би ЧУЖИЙ пін — і стенд
       надрукував би ✅. Промахнутись легко: у всіх шістнадцяти TRIG-рядків
       поле `;acl=postgres=X/postgres,service_role=X/postgres` побайтово
       однакове, а в одинадцяти з них однаковий і весь хвіст `attrs` цілком —
       різняться лише імʼя та md5. (Перша редакція цього коментаря казала
       «у всіх шістнадцяти однаковий хвіст» — це було перебільшення, заміряно
       ревʼю: хвостів там три різних.)
       Тепер `expect` тримає ІМʼЯ, і не голе: `/підпис sched_override_read/`
       прив'язане до шаблона назви `it.each(PINNED)("підпис %s пінується")`.
       Голе `/sched_override_read/` було б ширшим за названий тест — у наборі
       SPECS цього стенда є ще `schedOverrideDoor.test.ts` з `it.each`, де це
       імʼя стоїть у назві кожного екранного тесту. Знайдено двома ревʼю
       фальсифікації в с67, у два заходи.
       A2 нижче лишається на лічильнику навмисно — він сушить інвентар у
       ТЕСТІ, де цього імені вже немає. */
    expect: /підпис sched_override_read/,
  ...
  },
  {
    /* ⚠️ Той самий сторож, що в A1, але з ІНШОГО боку: там всихає список у
       ФАЙЛІ міграції, тут — інвентар у тесті. Способи обійти перевірку різні,
       і обидва мусять червоніти.
       ⚠️ `expect` тут і в A1 РІЗНІ, і це замір, а не недогляд: коли підпис
       зникає з `PINNED`, тест `it.each(PINNED)` для нього взагалі НЕ
       породжується — назвати його імʼям нічого. Єдине, що лишається
       червоніти, — лічильник. У A1, навпаки, імʼя є, і воно точніше. */
