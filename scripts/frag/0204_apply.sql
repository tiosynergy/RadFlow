...
begin
  ...
  if exists (select 1 from public.migration_ledger where name = '0204_referrer_grant_read.sql') then
    raise exception '0204: рядок уже в леджері — повторний накат заборонено';
  end if;
  ...
  if (select max(name) from public.migration_ledger) is distinct from
    ...
    raise exception '0204: останній рядок леджера % — не 0203, черга зсунулась',
  ...
  if v_body is null then
    raise exception '0204: invariants_check не знайдено';
  end if;
  v_src := replace(v_body, chr(13), '');
  if md5(v_src) is distinct from '8c8e6403db7653949e03d026320c6099' or length(v_src) <> 170446 then
    raise exception '0204: у проді не 0203 (% / %) — правка наосліп заборонена', md5(v_src), length(v_src);
  end if;
  ...
  if obj_description('public.invariants_check(boolean)'::regprocedure, 'pg_proc')
    ...
    raise exception '0204: самопін % не збігається з тілом 0203 — спершу розібратись',
  ...
  if to_regprocedure('public.tg_ref_entry_markers_prune_on_access()') is not null then
    raise exception '0204: функція tg_ref_entry_markers_prune_on_access() уже існує — чиясь чернетка? спершу розібратись';
  end if;
  if exists (select 1 from pg_trigger t
    ...
    raise exception '0204: тригер trg_zzz_ref_entry_markers_prune уже існує — спершу розібратись';
  end if;
  ...
  if not exists (
    ...
    raise exception '0204: change_marker_recipients на проді не тіло 0184 (cef6f91b5dd1dcdc35e93fd732cf7162) або атрибути інші — правка наосліп заборонена';
  end if;
  if (select count(*) from pg_proc p where p.pronamespace = 'public'::regnamespace
        and p.proname = 'change_marker_recipients') <> 1 then
    raise exception '0204: change_marker_recipients має перевантаження — create or replace не про ту функцію';
  end if;
  ...
  if v_pol is distinct from array['patient_cases.cases_select_referrer=d6b423f8c727', 'queue_entries.queue_select=ff3f89d6a1a2', 'waitlist_entries.waitlist_select=659164e8f637']::text[] then
    raise exception '0204: політики читання не у формі 0203: % — правка наосліп заборонена', v_pol;
  end if;
  ...
  if v_bad is not null then
    raise exception '0204: колонок немає або вони не uuid: %', v_bad;
  end if;
  if (select a.atttypid from pg_attribute a
    ...
    raise exception '0204: тип статусу гранту, мітка active або тип entity_type не ті';
  end if;
  if (select count(*) from pg_constraint co
    ...
    raise exception '0204: ucm_entity_type_chk не знає трьох типів записів — мітла й №14 міряли б не те';
  end if;
  ...
  if v_bad is not null then
    raise exception '0204: хелперів політик немає: %', v_bad;
  end if;
...
$fxa$;
  ...
  if v_bad is not null then
    raise exception '0204: change_marker_recipients після заміни не та: %', v_bad;
  end if;
  if not exists (
    select 1 from pg_proc p join pg_language l on l.oid = p.prolang
     where p.oid = to_regprocedure('public.change_marker_recipients(uuid, uuid, text, uuid, uuid, text, boolean)')
...
$fxb$;
  ...
  for i in 1 .. array_length(v_from, 1) loop
    ...
    if v_hits <> 1 then
      raise exception '0204: якір «%» трапляється % раз(ів), а треба 1', v_lbl[i], v_hits;
    end if;
  ...
  if md5(v_new) is distinct from 'cf1a920d2052a6debaff8b46c450aeff' or length(v_new) <> 178426 then
    raise exception '0204: підстановка дала % / %, а файл 0204 це cf1a920d2052a6debaff8b46c450aeff / 178426',
  ...
  if md5(v_src) is distinct from 'cf1a920d2052a6debaff8b46c450aeff' or length(v_src) <> 178426 then
    raise exception '0204: у БД лягло % / % замість cf1a920d2052a6debaff8b46c450aeff / 178426', md5(v_src), length(v_src);
  end if;
  ...
  if v_pin_db is distinct from 'guard_body_md5=cf1a920d2052a6debaff8b46c450aeff;len=178426' then
    raise exception '0204: пін із БД (%) розійшовся з піном із файлу (guard_body_md5=cf1a920d2052a6debaff8b46c450aeff;len=178426)', v_pin_db;
  end if;
  ...
  if obj_description('public.invariants_check(boolean)'::regprocedure, 'pg_proc') is distinct from v_pin_db then
    raise exception '0204: пін не ліг — у коментарі %',
      coalesce(obj_description('public.invariants_check(boolean)'::regprocedure, 'pg_proc'), '(NULL)');
  end if;
  ...
  if v_off16 is distinct from array['changed:patient_cases.cases_select_referrer', 'changed:queue_entries.queue_select', 'changed:waitlist_entries.waitlist_select']::text[] then
    raise exception '0204: №16 до DDL мусить назвати рівно три changed: політик читання, а назвав % — список №16 не живий?', v_off16;
  end if;
  ...
  if v_off17 is distinct from array['missing:referral_access.trg_zzz_ref_entry_markers_prune']::text[] then
    raise exception '0204: №17 до DDL мусить назвати рівно одну відсутню пару мітли, а назвав % — список №17 не живий?', v_off17;
  end if;
  ...
  if v_rows <> 1 then
    raise exception '0204: рядок леджера не ліг (% рядків)', v_rows;
  end if;
...
--   guard_md5 = cf1a920d2052a6debaff8b46c450aeff, guard_len = 178426,
--   guard_pin = guard_body_md5=cf1a920d2052a6debaff8b46c450aeff;len=178426, ledger_rows = 204, ledger_last = 0204_referrer_grant_read.sql,
