...
declare
  ...
  v_oq bigint; v_ow bigint; v_oc bigint; v_ocb jsonb;
...
begin
  ...
  if v_body is null then
    raise exception '0203-суха: invariants_check не знайдено';
  end if;
  v_src := replace(v_body, chr(13), '');
  if md5(v_src) is distinct from 'e1f1fdcfcea99906f02b0af193b814fa' or length(v_src) <> 165537 then
    raise exception '0203-суха: у проді не 0202 (% / %) — правка наосліп заборонена', md5(v_src), length(v_src);
  end if;
  ...
  if obj_description('public.invariants_check(boolean)'::regprocedure, 'pg_proc')
    ...
    raise exception '0203-суха: самопін % не збігається з тілом 0202 — спершу розібратись',
  ...
  if to_regprocedure('public.guard_record_read_keys()') is not null then
    raise exception '0203-суха: функція guard_record_read_keys() уже існує — чиясь чернетка? спершу розібратись';
  end if;
  ...
  if v_bad is not null then
    raise exception '0203-суха: тригери пакета вже існують: % — спершу розібратись', v_bad;
  end if;
  ...
  if v_bad is not null then
    raise exception '0203-суха: гард стоїть на колонках, яких немає або вони не uuid: %', v_bad;
  end if;
  ...
  if (select a.atttypid from pg_attribute a
    ...
    raise exception '0203-суха: типи або мітки ролі/статусу гранту не ті, з якими порівнює гард';
  end if;
  if exists (select 1 from pg_attribute a
    ...
    raise exception '0203-суха: у referrer_private зʼявились id/clinic_id — проза про NULL у audit_log протухла, переглянути';
  end if;
  if to_regprocedure('public.fn_audit()') is null then
    raise exception '0203-суха: fn_audit() немає — аудит-тригерам нічого кликати';
  end if;
...
$fxa$;
  ...
  if v_bad is not null then
    raise exception '0203-суха: гард після створення не той: %', v_bad;
  end if;
  if (select md5(replace(p.prosrc, chr(13), '')) from pg_proc p where p.oid = 'public.guard_record_read_keys()'::regprocedure)
    ...
    raise exception '0203-суха: сирий md5 тіла гарда після створення не 55f111f5d83e52b1c7970f06746d7ff6';
  end if;
  ...
  if has_function_privilege('anon', 'public.guard_record_read_keys()', 'EXECUTE')
    ...
    raise exception '0203-суха: guard_record_read_keys() виконують anon/authenticated/PUBLIC — ACL не звужено';
  end if;
  if not has_function_privilege('service_role', 'public.guard_record_read_keys()', 'EXECUTE') then
    raise exception '0203-суха: service_role втратив EXECUTE на guard_record_read_keys()';
  end if;
  ...
  if v_acl is distinct from 'postgres=X/postgres,service_role=X/postgres' then
    raise exception '0203-суха: ACL guard_record_read_keys() = % замість postgres=X/postgres,service_role=X/postgres', v_acl;
  end if;
  ...
  for i in 1 .. array_length(v_from, 1) loop
    ...
    if v_hits <> 1 then
      raise exception '0203-суха: якір «%» трапляється % раз(ів), а треба 1', v_lbl[i], v_hits;
    end if;
  ...
  if md5(v_new) is distinct from '8c8e6403db7653949e03d026320c6099' or length(v_new) <> 170446 then
    raise exception '0203-суха: підстановка дала % / %, а файл 0203 це 8c8e6403db7653949e03d026320c6099 / 170446',
  ...
  if md5(v_src) is distinct from '8c8e6403db7653949e03d026320c6099' or length(v_src) <> 170446 then
    raise exception '0203-суха: у БД лягло % / % замість 8c8e6403db7653949e03d026320c6099 / 170446', md5(v_src), length(v_src);
  end if;
  ...
  if v_pin_db is distinct from 'guard_body_md5=8c8e6403db7653949e03d026320c6099;len=170446' then
    raise exception '0203-суха: пін із БД (%) розійшовся з піном із файлу (guard_body_md5=8c8e6403db7653949e03d026320c6099;len=170446)', v_pin_db;
  end if;
  ...
  if obj_description('public.invariants_check(boolean)'::regprocedure, 'pg_proc') is distinct from v_pin_db then
    raise exception '0203-суха: пін не ліг — у коментарі %',
      coalesce(obj_description('public.invariants_check(boolean)'::regprocedure, 'pg_proc'), '(NULL)');
  end if;
  ...
  if (v_res->>'checked')::int <> 26 then
    raise exception '0203-суха: сторож перевірив % замість 26', v_res->>'checked';
  end if;
  ...
  if v_failed is not null then
    raise exception '0203-суха: до DDL сторож червоний не від пакета: % — %', v_failed, v_res->'failed';
  end if;
  ...
  if v_off17 is distinct from array['missing:doctors.trg_audit_doctors', 'missing:patient_cases.trg_audit_patient_cases', 'missing:patient_cases.zz_guard_read_keys', 'missing:queue_entries.zz_guard_read_keys', 'missing:referrer_private.trg_audit_referrer_private', 'missing:services.trg_audit_services', 'missing:waitlist_entries.zz_guard_read_keys']::text[] then
    raise exception '0203-суха: №17 до DDL мусить назвати рівно сім відсутніх пар, а назвав % — список №17 не живий?', v_off17;
  end if;
  ...
  select count(*) filter (where x.t = 'q'), count(*) filter (where x.t = 'w'), count(*) filter (where x.t = 'c')
    into v_oq, v_ow, v_oc
    from (
      select 'q' as t, q.referrer_id as k, q.clinic_id as c from public.queue_entries q where q.referrer_id is not null
      union all
      select 'w', w.referrer_id, w.clinic_id from public.waitlist_entries w where w.referrer_id is not null
      union all
      select 'c', pc.referrer_id, pc.clinic_id from public.patient_cases pc where pc.referrer_id is not null
    ) x
      ...
      select y.t || ':' || y.cat as bucket, count(*) as n
        from (
          select x.t,
            ...
            from (
          select 'q' as t, q.created_by as k, q.clinic_id as c from public.queue_entries q where q.created_by is not null
          union all
          select 'w', w.created_by, w.clinic_id from public.waitlist_entries w where w.created_by is not null
          union all
          select 'c', pc.created_by, pc.clinic_id from public.patient_cases pc where pc.created_by is not null
            ) x
  ...
  if v_bad is not null then
    raise exception '0203-суха: останній BEFORE-тригер рядка — не zz_guard_read_keys: %', v_bad;
  end if;
  ...
  if v_tmp is not null then
    raise exception '0203-суха: №17 після DDL червоний: %', v_tmp;
  end if;
  ...
  if v_rows <> 1 then
    raise exception '0203-суха: рядок леджера не ліг (% рядків)', v_rows;
  end if;
