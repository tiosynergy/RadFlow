...
$p$
  ...
  v_lbl  constant text[] := array[
    $p$№19: queue_set_status_rpc md5 ef04f36d… → a49a4c2e…$p$,
    $p$№19: emergency_stop_rpc md5 ac62900b… → 4fe9671d…$p$,
    $p$№19: submit_incident_rpc md5 3534d77c… → 02e0e9eb…$p$,
    $p$№19: room_busy_slots md5 83ddb89d… → ad9e1dfd…$p$,
    $p$№23: дайджест k:clinics (CHECK звужено)$p$,
    $p$проза №19: історія росту$p$,
    $p$проза №19: абзац 0202$p$,
    $p$проза №23: ремарка k:clinics$p$
  ];
begin
  ...
  if current_user <> 'postgres' then
    raise exception '0202-суха: мусить іти від ролі postgres, а йде від %', current_user;
  end if;
  if exists (select 1 from public.migration_ledger where name = '0202_tz_kyiv_no_catalog_scan.sql') then
  ...
  if (select max(name) from public.migration_ledger) is distinct from
     '0201_pin_gated_rpcs_role_surface.sql' then
  ...
  if v_body is null then
    raise exception '0202-суха: invariants_check не знайдено';
  end if;
  v_src := replace(v_body, chr(13), '');
  if md5(v_src) is distinct from 'f0134c6203dacab659fd85648c5e9aa9' or length(v_src) <> 164374 then
    raise exception '0202-суха: у проді не 0201 (% / %) — правка наосліп заборонена', md5(v_src), length(v_src);
  end if;
  ...
  if obj_description('public.invariants_check(boolean)'::regprocedure, 'pg_proc')
     is distinct from 'guard_body_md5=f0134c6203dacab659fd85648c5e9aa9;len=164374' then
    raise exception '0202-суха: самопін % не збігається з тілом 0201 — спершу розібратись',
  ...
  if v_qid is not null then
    ...
    raise notice '0202-суха: до — майбутнього scheduled-запису немає, замір пропущено (-3)';
  end if;
  ...
  if (now() at time zone 'Europe/Kyiv') is distinct from (now() at time zone 'Europe/Kiev')
    ...
    raise exception '0202-суха: зона Europe/Kyiv на цьому сервері не дає той самий час, що Europe/Kiev — стоп';
  end if;
  ...
  if v_ids is distinct from array['c79588d6-c379-4949-9c23-a22c227a12e1']::uuid[] then
    raise exception '0202-суха: рядки на Europe/Kiev = % — не заміряний до-образ {c79588d6-c379-4949-9c23-a22c227a12e1}; стоп', v_ids;
  end if;
  if exists (select 1 from public.clinics where timezone not in ('Europe/Kiev', 'Europe/Kyiv', 'UTC')) then
    raise exception '0202-суха: у clinics є зона поза списком 0192 — CHECK не тримає?';
  end if;
  ...
  if v_rows <> 1 then
    raise exception '0202-суха: оновлено % рядків clinics замість 1', v_rows;
  end if;
  if exists (select 1 from public.clinics where timezone = 'Europe/Kiev') then
    raise exception '0202-суха: після оновлення ще є Europe/Kiev';
  end if;
  ...
  if v_con is distinct from 'CHECK ((timezone = ANY (ARRAY[''Europe/Kyiv''::text, ''Europe/Kiev''::text, ''UTC''::text])))' then
    raise exception '0202-суха: рендер clinics_timezone_chk до правки = % — не форма 0192', coalesce(v_con, '(немає)');
  end if;
  with kon as (
  ...
  if v_dig is distinct from '5:588baa1ac5d2' then
    raise exception '0202-суха: дайджест k:clinics до правки % ≠ 5:588baa1ac5d2', v_dig;
  end if;
  ...
  if v_con is distinct from 'CHECK ((timezone = ANY (ARRAY[''Europe/Kyiv''::text, ''UTC''::text])))' then
    raise exception '0202-суха: рендер clinics_timezone_chk після правки = % — не очікувана форма', v_con;
  end if;
  with kon as (
    select 'k:' || rel.relname::text as key,
           co.conname::text || ':' || co.contype::text || ':'
  ...
  if v_dig is distinct from '5:2d77f97c5c03' then
    raise exception '0202-суха: дайджест k:clinics після правки % ≠ 5:2d77f97c5c03 — пін №23 розійшовся б', v_dig;
  end if;

  -- ── Живі рядки шести функцій ДО (паритет із продом, замір 16.09) — виразом `cur` №19, вирізаним з тіла ──
  ...
  if v_bad is not null then
    raise exception '0202-суха: заголовок функції ДО (паритет із продом, замір 16.09) не той, що на проді 16.09: %', v_bad;
  end if;
  select count(*) into v_hits from pg_proc p
   where p.pronamespace = 'public'::regnamespace
     and p.proname in ('check_no_overlap', 'check_not_in_past', 'queue_set_status_rpc', 'emergency_stop_rpc', 'submit_incident_rpc', 'room_busy_slots');
  if v_hits <> 6 then
    raise exception '0202-суха: обʼєктів із шістьма іменами % замість 6 — перевантаження', v_hits;
  end if;
  ...
  if v_bad is not null then
    raise exception '0202-суха: живі рядки шести функцій ДО (паритет із продом, замір 16.09) не збіглися: %', v_bad;
  end if;
...
$fxf$;
  ...
  -- ── Живі рядки шести функцій ПІСЛЯ (md5 нових текстів із генератора) — виразом `cur` №19, вирізаним з тіла ──
  -- ⚠️ плюс ПОВНИЙ заголовок (аргументи з DEFAULT-ами, тип результату): identity-
  --    аргументи дефолтів не несуть, а `create or replace` міняє їх мовчки.
  --    NULL-безпечно (`not exists … is not distinct from`), без фільтра prokind:
  --    процедура з таким імʼям теж мусить стати порушником, а не випасти з NOT IN.
      ...
      from pg_proc p where p.pronamespace = 'public'::regnamespace
       ...
       and not exists (select 1 from (values
        ...
        where h.fn = p.proname and p.prokind = 'f'
          ...
          and h.res is not distinct from pg_get_function_result(p.oid))) x;
  if v_bad is not null then
    raise exception '0202-суха: заголовок функції ПІСЛЯ (md5 нових текстів із генератора) не той, що на проді 16.09: %', v_bad;
  end if;
  select count(*) into v_hits from pg_proc p
   where p.pronamespace = 'public'::regnamespace
     and p.proname in ('check_no_overlap', 'check_not_in_past', 'queue_set_status_rpc', 'emergency_stop_rpc', 'submit_incident_rpc', 'room_busy_slots');
  ...
  if v_bad is not null then
    raise exception '0202-суха: живі рядки шести функцій ПІСЛЯ (md5 нових текстів із генератора) не збіглися: %', v_bad;
  end if;
  ...
  if v_qid is not null then
    ...
    v_ms := v_ms || array[-3, -3]::numeric[]; v_trg := v_trg || array[-3, -3]::numeric[];
    raise notice '0202-суха: після — майбутнього scheduled-запису немає, замір пропущено (-3)';
  end if;
  ...
  for i in 1 .. array_length(v_from, 1) loop
    ...
    if v_hits <> 1 then
      raise exception '0202-суха: якір «%» трапляється % раз(ів), а треба 1', v_lbl[i], v_hits;
    end if;
  ...
  if md5(v_new) is distinct from 'e1f1fdcfcea99906f02b0af193b814fa' or length(v_new) <> 165537 then
    raise exception '0202-суха: підстановка дала % / %, а файл 0202 це e1f1fdcfcea99906f02b0af193b814fa / 165537',
  ...
  if md5(v_src) is distinct from 'e1f1fdcfcea99906f02b0af193b814fa' or length(v_src) <> 165537 then
    raise exception '0202-суха: у БД лягло % / % замість e1f1fdcfcea99906f02b0af193b814fa / 165537', md5(v_src), length(v_src);
  end if;
  ...
  if v_pin_db is distinct from 'guard_body_md5=e1f1fdcfcea99906f02b0af193b814fa;len=165537' then
    raise exception '0202-суха: пін із БД (%) розійшовся з піном із файлу (guard_body_md5=e1f1fdcfcea99906f02b0af193b814fa;len=165537)', v_pin_db;
  end if;
  ...
  if obj_description('public.invariants_check(boolean)'::regprocedure, 'pg_proc') is distinct from v_pin_db then
    raise exception '0202-суха: пін не ліг — у коментарі %',
      coalesce(obj_description('public.invariants_check(boolean)'::regprocedure, 'pg_proc'), '(NULL)');
  end if;
  ...
  if (v_res->>'checked')::int <> 26 then
    raise exception '0202-суха: сторож перевірив % замість 26', v_res->>'checked';
  end if;
  if exists (select 1 from jsonb_array_elements(v_res->'failed') e
              where e.value->>'check' in ('guard_fn_bodies', 'guard_self_pin', 'schema_digest', 'secdef_search_path', 'guard_triggers', 'room_busy_service_role')) then
