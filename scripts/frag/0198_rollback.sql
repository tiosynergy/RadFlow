...
begin
  ...
  if md5(v_new) is distinct from 'f7bcdb2accee718e381f8c805a522abb' or length(v_new) <> 132608 then
    raise exception '0198-відкат: зворотна пара дала % / %, а 0197 це f7bcdb2accee718e381f8c805a522abb / 132608',
  ...
  if md5(v_src) is distinct from 'f7bcdb2accee718e381f8c805a522abb' then
    raise exception '0198-відкат: у БД лягло % замість f7bcdb2accee718e381f8c805a522abb', md5(v_src);
  end if;
...
--        from pg_proc p join pg_namespace n on n.oid = p.pronamespace
--       where n.nspname='public' and p.proname='invariants_check';
--      -- очікування: body = f7bcdb2accee718e381f8c805a522abb, pin = (NULL)
