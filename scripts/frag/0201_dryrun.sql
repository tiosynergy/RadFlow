...
$rcc$;
  ...
  -- `checked = 26` і зелені №19, №25, №26 ДО того, як чіпати прод.
  v_res := public.invariants_check(false);
  if (v_res->>'checked')::int <> 26 then
    raise exception '0201-суха: сторож перевірив % замість 26', v_res->>'checked';
  end if;
  ...
  if exists (select 1 from jsonb_array_elements(v_res->'failed') e
              where e.value->>'check' in ('guard_fn_bodies', 'guard_self_pin', 'role_surface')) then
