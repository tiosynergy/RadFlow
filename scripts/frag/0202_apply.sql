...
begin
  ...
  if current_user <> 'postgres' then
    raise exception '0202: мусить іти від ролі postgres, а йде від %', current_user;
  end if;
  ...
  if v_body is null then
    raise exception '0202: invariants_check не знайдено';
  end if;
  v_src := replace(v_body, chr(13), '');
  if md5(v_src) is distinct from 'f0134c6203dacab659fd85648c5e9aa9' or length(v_src) <> 164374 then
    raise exception '0202: у проді не 0201 (% / %) — правка наосліп заборонена', md5(v_src), length(v_src);
  end if;
  ...
  if obj_description('public.invariants_check(boolean)'::regprocedure, 'pg_proc')
    ...
    raise exception '0202: самопін % не збігається з тілом 0201 — спершу розібратись',
  ...
  if v_con is distinct from 'CHECK ((timezone = ANY (ARRAY[''Europe/Kyiv''::text, ''Europe/Kiev''::text, ''UTC''::text])))' then
    raise exception '0202: рендер clinics_timezone_chk до правки = % — не форма 0192', coalesce(v_con, '(немає)');
  end if;
  with kon as (
  ...
  if v_dig is distinct from '5:588baa1ac5d2' then
    raise exception '0202: дайджест k:clinics до правки % ≠ 5:588baa1ac5d2', v_dig;
  end if;
  ...
  if v_con is distinct from 'CHECK ((timezone = ANY (ARRAY[''Europe/Kyiv''::text, ''UTC''::text])))' then
    raise exception '0202: рендер clinics_timezone_chk після правки = % — не очікувана форма', v_con;
  end if;
  with kon as (
    select 'k:' || rel.relname::text as key,
           co.conname::text || ':' || co.contype::text || ':'
  ...
  if v_dig is distinct from '5:2d77f97c5c03' then
    raise exception '0202: дайджест k:clinics після правки % ≠ 5:2d77f97c5c03 — пін №23 розійшовся б', v_dig;
  end if;
  ...
  if v_bad is not null then
    raise exception '0202: заголовок функції ДО (паритет із продом, замір 16.09) не той, що на проді 16.09: %', v_bad;
  end if;
  select count(*) into v_hits from pg_proc p
   where p.pronamespace = 'public'::regnamespace
     and p.proname in ('check_no_overlap', 'check_not_in_past', 'queue_set_status_rpc', 'emergency_stop_rpc', 'submit_incident_rpc', 'room_busy_slots');
  if v_hits <> 6 then
    raise exception '0202: обʼєктів із шістьма іменами % замість 6 — перевантаження', v_hits;
  end if;
  ...
  if v_bad is not null then
    raise exception '0202: живі рядки шести функцій ДО (паритет із продом, замір 16.09) не збіглися: %', v_bad;
  end if;
...
$fxf$;
      ...
      from pg_proc p where p.pronamespace = 'public'::regnamespace
       ...
       and not exists (select 1 from (values
        ...
        where h.fn = p.proname and p.prokind = 'f'
          ...
          and h.res is not distinct from pg_get_function_result(p.oid))) x;
  if v_bad is not null then
    raise exception '0202: заголовок функції ПІСЛЯ (md5 нових текстів із генератора) не той, що на проді 16.09: %', v_bad;
  end if;
  select count(*) into v_hits from pg_proc p
   where p.pronamespace = 'public'::regnamespace
     and p.proname in ('check_no_overlap', 'check_not_in_past', 'queue_set_status_rpc', 'emergency_stop_rpc', 'submit_incident_rpc', 'room_busy_slots');
  ...
  if v_bad is not null then
    raise exception '0202: живі рядки шести функцій ПІСЛЯ (md5 нових текстів із генератора) не збіглися: %', v_bad;
  end if;
  ...
  for i in 1 .. array_length(v_from, 1) loop
    ...
    if v_hits <> 1 then
      raise exception '0202: якір «%» трапляється % раз(ів), а треба 1', v_lbl[i], v_hits;
    end if;
  ...
  if md5(v_new) is distinct from 'e1f1fdcfcea99906f02b0af193b814fa' or length(v_new) <> 165537 then
    raise exception '0202: підстановка дала % / %, а файл 0202 це e1f1fdcfcea99906f02b0af193b814fa / 165537',
  ...
  if md5(v_src) is distinct from 'e1f1fdcfcea99906f02b0af193b814fa' or length(v_src) <> 165537 then
    raise exception '0202: у БД лягло % / % замість e1f1fdcfcea99906f02b0af193b814fa / 165537', md5(v_src), length(v_src);
  end if;
  ...
  if v_pin_db is distinct from 'guard_body_md5=e1f1fdcfcea99906f02b0af193b814fa;len=165537' then
    raise exception '0202: пін із БД (%) розійшовся з піном із файлу (guard_body_md5=e1f1fdcfcea99906f02b0af193b814fa;len=165537)', v_pin_db;
  end if;
  ...
  if obj_description('public.invariants_check(boolean)'::regprocedure, 'pg_proc') is distinct from v_pin_db then
    raise exception '0202: пін не ліг — у коментарі %',
      coalesce(obj_description('public.invariants_check(boolean)'::regprocedure, 'pg_proc'), '(NULL)');
  end if;
...
--   clinic_tz = Europe/Kyiv, con_def = CHECK ((timezone = ANY (ARRAY['Europe/Kyiv'::text, 'UTC'::text]))), fns_with_scan = 0
