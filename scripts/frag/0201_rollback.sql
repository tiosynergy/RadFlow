...
$p$
  ...
  v_lbl  constant text[] := array[
    $p$назад: вставка перевірки №26 перед збіркою v_res$p$,
    $p$назад: проза №25: «єдина річ» — не єдина$p$,
    $p$назад: абзац 0200: «окреме рішення» — вирішено в 0201$p$,
    $p$назад: проза №2: CREATE на public тепер пасе №26$p$,
    $p$назад: межа №22 «лише public» — дефолти й USAGE тепер у №26$p$,
    $p$назад: абзац 0201 у прозі №19 перед кроком лічильника$p$,
    $p$назад: межа 0191 «не входять нікуди більше» — тепер №26$p$,
    $p$назад: історія росту списку в прозі №19: + 0201 → 59$p$,
    $p$назад: лічильник у заголовку прози №19: 43 -> 59$p$,
    $p$назад: 16 рядків у список №19 після set_waitlist_status_rpc$p$
  ];
begin
  ...
  if v_body is null then
    raise exception '0201-відкат: invariants_check не знайдено';
  end if;
  v_src := replace(v_body, chr(13), '');
  if md5(v_src) is distinct from 'f0134c6203dacab659fd85648c5e9aa9' or length(v_src) <> 164374 then
    raise exception '0201-відкат: у проді не 0201 (% / %) — правка наосліп заборонена', md5(v_src), length(v_src);
  end if;
  ...
  if obj_description('public.invariants_check(boolean)'::regprocedure, 'pg_proc')
    ...
    raise exception '0201-відкат: самопін % не збігається з тілом 0201 — спершу розібратись',
  ...
  for i in 1 .. array_length(v_from, 1) loop
    ...
    if v_hits <> 1 then
      raise exception '0201-відкат: якір «%» трапляється % раз(ів), а треба 1', v_lbl[i], v_hits;
    end if;
  ...
  if md5(v_new) is distinct from '00146b182c9a366094678ccdb10f35aa' or length(v_new) <> 140268 then
    raise exception '0201-відкат: підстановка дала % / %, а 0200 це 00146b182c9a366094678ccdb10f35aa / 140268',
  ...
  if md5(v_src) is distinct from '00146b182c9a366094678ccdb10f35aa' or length(v_src) <> 140268 then
    raise exception '0201-відкат: у БД лягло % / % замість 00146b182c9a366094678ccdb10f35aa / 140268', md5(v_src), length(v_src);
  end if;
  ...
  if v_pin_db is distinct from 'guard_body_md5=00146b182c9a366094678ccdb10f35aa;len=140268' then
    raise exception '0201-відкат: пін із БД (%) розійшовся з піном із файлу (guard_body_md5=00146b182c9a366094678ccdb10f35aa;len=140268)', v_pin_db;
  end if;
  ...
  if obj_description('public.invariants_check(boolean)'::regprocedure, 'pg_proc') is distinct from v_pin_db then
    raise exception '0201-відкат: пін не ліг — у коментарі %',
      coalesce(obj_description('public.invariants_check(boolean)'::regprocedure, 'pg_proc'), '(NULL)');
  end if;

  delete from public.migration_ledger where name = '0201_pin_gated_rpcs_role_surface.sql';
...
--        from pg_proc p join pg_namespace n on n.oid = p.pronamespace
--       where n.nspname = 'public' and p.proname = 'invariants_check';
--      -- очікування: body = 00146b182c9a366094678ccdb10f35aa, len = 140268, pin = guard_body_md5=00146b182c9a366094678ccdb10f35aa;len=140268
