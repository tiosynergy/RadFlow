...
begin
  ...
  -- 7. ПʼЯТЬ ЧЕРВОНИХ БАЗИСІВ, у цій же транзакції: ДВА напрямки прав (а, а2)
  --    і ТРИ тіла (б, в, г). Без них «зелено» означає лише «пін збігся», а не
  --    «пін ловить».
  --    ⚠️ Довго стояло «ТРИ»: заголовок писався до того, як ревʼю Н9 додало
  --    базис (а2), і не був оновлений разом із кодом нижче. Число, яке відстає
  --    від власного коду, — це те саме, що число, яке бреше.
  declare
    v_bad   jsonb;
    v_rdef  text;
    v_rhead text;
  ...
  raise notice '0191 НАКАТАНО: md5 % len % checked 23; пʼять червоних базисів спрацювали (2 права + 3 тіла)', v_md5, v_len;
end
$apply$;
