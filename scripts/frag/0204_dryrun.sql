...
begin
  ...
  if v_unreach <> '{}'::jsonb then
    raise exception '0204-суха: уже є недосяжні позначки записів % — до накату ручна зачистка за явним списком id (AGENTS.md, видалення даних проду)', v_unreach;
  end if;
...
$fxa$;
  ...
  if not exists (
    ...
    raise exception '0204-суха: change_marker_recipients після заміни не та (атрибути або сирий md5 тіла a9e7002f2ead3f05b130255129e6cc50)';
  end if;
...
$fxb$;
  ...
  if not exists (
    ...
    raise exception '0204-суха: tg_ref_entry_markers_prune_on_access() після створення не та (атрибути або сирий md5 тіла 25b92931235f5c0d846e082509c105ea)';
  end if;
  -- ── ACL: пастка 0122 (дефолтний ACL схеми public роздає EXECUTE і anon,
  --    і PUBLIC) — асерт у ТІЙ САМІЙ транзакції ──
  ...
  if md5(v_new) is distinct from '843d4a74b4b6b88127989ac017f0281a' or length(v_new) <> 177302 then
    raise exception '0204-суха: підстановка дала % / %, а файл 0204 це 843d4a74b4b6b88127989ac017f0281a / 177302',
  ...
  if md5(v_src) is distinct from '843d4a74b4b6b88127989ac017f0281a' or length(v_src) <> 177302 then
    raise exception '0204-суха: у БД лягло % / % замість 843d4a74b4b6b88127989ac017f0281a / 177302', md5(v_src), length(v_src);
  end if;
  ...
  if v_pin_db is distinct from 'guard_body_md5=843d4a74b4b6b88127989ac017f0281a;len=177302' then
    raise exception '0204-суха: пін із БД (%) розійшовся з піном із файлу (guard_body_md5=843d4a74b4b6b88127989ac017f0281a;len=177302)', v_pin_db;
  end if;
