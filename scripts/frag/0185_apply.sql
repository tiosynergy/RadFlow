...
declare
  ...
  v_anchor constant text := $anchor$  /* 0174 */     'check', 'grant_digest', 'offenders',
  /* 0174 */     to_jsonb(array['raised:' || sqlstate || ':' || left(sqlerrm, 120)])));
  /* 0174 */ end;$anchor$;
  v_branch constant text := $branch$  -- 23. ФОРМА СХЕМИ — останній непокритий шматок schema-contract manifest,
...
begin
  ...
  if md5(replace(v_body, chr(13), '')) is distinct from '11a297318da068f53b113c3d9120e6b8' then
  ...
  if v_md5 <> '8871cad05dbbee6c4cef4f4f9a3fb515' or v_len <> 111592 then
    raise exception '0185: зібране тіло % / % — очікували 8871cad05dbbee6c4cef4f4f9a3fb515 / 111592', v_md5, v_len;
  end if;
