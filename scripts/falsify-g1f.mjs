...
/** Создать новую запись`,
  ...
  /* ---------- пакет 63 (с63): ВІДМОВА МУСИТЬ БУТИ ВИДИМОЮ ----------
     Заплачено живим прогоном 11.09: гард відпрацював, а оператор не побачив
     нічого — тост малювався під оверлеєм модалки. Позиції нижче стережуть
     обидві половини лікування, бо кожна поодинці безсила. */
  {
    /* Модалка перестає ПОКАЗУВАТИ: стан є, вузла немає. Найтихіша з трьох —
       код компілюється, тести на проводку гарда лишаються зеленими. */
    id: "M30", file: "wm",
    expect: /показує помилку сервера У СЕБЕ/,
    what: "вузол помилки прибрано з розмітки модалки — відмова знову невидима",
    from: `        {saveErr && (\n          <div className="dlg-err" role="alert">⚠ {saveErr}</div>\n        )}`,
    to: `        {false && saveErr && (\n          <div className="dlg-err" role="alert">⚠ {saveErr}</div>\n        )}`,
  },
  {
    /* Текст приходить, але нікуди не кладеться — модалка мовчить так само. */
    id: "M31", file: "wm",
    expect: /показує помилку сервера У СЕБЕ/,
    what: "результат onSave більше не потрапляє в стан — текст губиться мовчки",
    from: `      if (err) setSaveErr(err);`,
    to: `      void err;`,
  },
  {
    /* Друга половина: батько знову топить текст у тості. Модалка при цьому
       ідеально готова його показати — і показувати нічого. */
    id: "M32", file: "wb",
    expect: /ВІДДАЮТЬ текст, а не топлять/,
    what: "дошка листа повернулась до notify — відмова знову тоне під оверлеєм",
    from: `    if (!res.ok) return res.error;\n    setAddOpen(false);`,
    to: `    if (!res.ok) { notify("Помилка: " + res.error, "error"); return null; }\n    setAddOpen(false);`,
  },
];
...
const EXPECTED_RED = 44;   // +14 у пакеті 22 (три шляхи створення), +M30..M32 у пакеті 63 (видимість відмови)
