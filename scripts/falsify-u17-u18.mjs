...
/* ⚠️ ПРОМІЖНИЙ ЗВІТ ПРИБИРАЄМО ЗА СОБОЮ (знахідка с63: після повної ревізії
   в корені лишався `.falsify-u17-u18.json` на 61 КБ). Решта стендів роблять
   це у `finally`, цей — не робив узагалі: `rmSync` стояв тільки на ВХОДІ в
   `runSpec`. У коміт файл не доїхав би (він у .gitignore), але саме на такі
   «нешкідливі» сміттєві файли в корені проєкт уже наступав — `.mergemsg`
   (с50) і звіти `falsify-*.md` (с47). */
rmSync(REPORT, { force: true });
