...
/** Недосяжні позначки ЗАПИСІВ — предикат гілки №14 `unreachable:` (агрегат).
...
const PREMISES = (tag, { unreachStop = true } = {}) => [
  ...
  ...(unreachStop ? [
    ...
    `    raise exception '${tag}: уже є недосяжні позначки записів % — до накату ручна зачистка за явним списком id (AGENTS.md, видалення даних проду)', v_unreach;`,
    "  end if;",
  ] : []),
].join("\n");
...
const DRYRUN = [
  ...
  "--    Будь-який інший текст — провал, прочитати і розібратись. `уже є недосяжні",
  "--    позначки записів` — ручна зачистка за явним списком id ДО накату.",
  "-- ⚠️ `radius_*` і `unreachable` — ЗАМІР, а не умова зупинки (крім недосяжних:",
  "--    їх повний сторож назве червоним): інше число = переглянути абзац ціни в",
  "--    PR-доці, не стоп.",
...
const fakeMark = (ref, clinic, etype, eid, scope = "record") => [
...
const PROBES = [
  ...
  stateProbe("P-revoke-update", `${entryMarks("v_ref", "v_clinic")} = 0`, `'позначок записів після відкликання ' || ${entryMarks("v_ref", "v_clinic")}`),
  ...
  ...["pending_referrer", "pending_clinic", "declined"].flatMap((s) => [setGrant("v_ref", "v_clinic", s), readProbe(`R-${s}`, "v_ref", "0/0/0")]),
...
const MIG_PRE = [
  ...
  "  if v_unreach <> '{}'::jsonb then",
  "    raise exception '0204: уже є недосяжні позначки записів % — до накату ручна зачистка за явним списком id', v_unreach;",
  "  end if;",
  "end",
  "$pre$;",
].join("\n");
...
const MIG_HEAD = [
  ...
  "--     • гонка «емісія позначки ‖ відкликання гранту» може лишити позначку —",
  ...
  "--      missing:). unreachable і radius_* — ЗАМІР, а не умова зупинки: інше число",
  "--      — переглянути абзац ціни, не стоп.",
...
const MIG_TAIL = [
  ...
  "--     (направник з активним грантом), рядки 0204 у `docs/UNREAD_CHANGES.md` і три абзаци в",
  "--     `AGENTS.md` («Роли и авторизация», «Контекстные красные точки», «Миграции",
  "--     и БД»). Нового стенда пакет НЕ заводить — фальсифікація разова, протокол",
  "--     у `docs/audit/PR-0204-referrer-grant-read.md`.",
  "-- ============================================================================",
].join("\n");
