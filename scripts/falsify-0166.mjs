...
/* с57: смоук, який сторожа НЕ читає — на ньому перевіряється, що канал прози
   звужений РЯДКОМ, а не файлом (борг с56, розділ §2.2 плану). */
...
const E = {
  ...
  /* ⚠️ с57/0177: тест перейменовано разом із появою ТРЕТЬОГО власного
     обробника (оракул усередині №21). Відношення тепер «обробників =
     обгорткам + власним», а не «+2» — якір мусив переїхати за назвою. */
  handlers20: /обробників = обгорткам \+ власним/,
...
/* ── с57: канал прози звужений РЯДКОМ, а не ФАЙЛОМ (борг с56, §2.2 плану) ───
   Обидві позиції — на смоуці, який сторожа НЕ читає взагалі. Пара показує, що
   канал розрізняє МОВУ рядка, а не його місце: «сторож рахує N перевірок»
   ловиться навіть там, де слова `checked` немає (стара, файлова редакція цього
   НЕ бачила зовсім — це нове покриття, яке дав переїзд із файла на рядок), а
   «усі N перевірок пройдено» про власні кроки смоука не ловиться і поготів.
   ⚠️ Саме на цьому файлі стара редакція дала б ХИБНОГО порушника в той день,
   коли тут зʼявиться будь-який зонд на `'checked'` (замір: 1 проти 0). */
M.push(
  ["N46 брехлива проза про сторожа у смоуці, який сторожа не читає", CPV, E.proseLine,
   "  raise notice 'СМОУК 0147: OK (усі 7 перевірок пройдено)';",
   "  -- сторож рахує 18 перевірок\n  raise notice 'СМОУК 0147: OK (усі 7 перевірок пройдено)';"],
  ["N47 [позитивний контроль] проза про ВЛАСНІ кроки смоука з чужим числом — має лишитись зеленою",
   CPV, GREEN,
   "  raise notice 'СМОУК 0147: OK (усі 7 перевірок пройдено)';",
   "  -- усі 8 перевірок пройдено\n  raise notice 'СМОУК 0147: OK (усі 7 перевірок пройдено)';"],
  ["N48 інструмент бампу звузив прозу інакше, ніж сторож — бамп «успішний», рядок не переписаний",
   BUMP, E.proseTool,
