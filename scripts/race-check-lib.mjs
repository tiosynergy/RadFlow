...
/** Вікно, у якому гард шукає СЛІД n8n-гілки в `event_outbox`. Рівно горизонт
    `event_outbox_retention(p_delivered_days default 30)` з 0159 — і це не
    збіг, а МЕЖА ПРИЛАДУ. Доставлені рядки, старші за горизонт, прибирає крон,
    тож ширше вікно не додало б ані одного доказу живості; додало б воно лише
    недоставлених (їх ретенція не чіпає ніколи) — тобто тихо посунуло б
    вердикт у бік дозволу. Перша редакція цього гарда брала 90 днів «щоб не
    вакуумило», і це було послабленням, замаскованим під обережність
    (знахідка ревʼю А, с65). Значення припнуте тестом, який читає 0159. */
export const N8N_EVIDENCE_WINDOW_DAYS = 30;

/** Префікс подій, які йдуть ВЕБХУКУ КЛІНІКИ. Дзеркало
    `INTEGRATION_EVENT_PREFIX` (lib/integrationContract.ts): усе, що НЕ має
    цього префікса, воркер шле в n8n. Розійдеться — гард рахуватиме не ту
    гілку й мовчки подобрішає, тому значення припнуте тестом. */
export const INTEGRATION_PREFIX_MIRROR = "integration.";

/** Помітка, якою воркер відкладає n8n-рядки, коли гілка НЕ сконфігурована або
    зламана (`deferRows(deferredN8nIds, …, "n8n_deferred")` у lib/outbox.ts).
    Це ЄДИНИЙ позитивний доказ мовчання: саме по собі «не доставлено» доказом
    не є — рядок міг щойно народитись і ще не пройти повз воркера, або падати
    з HTTP-помилкою, тобто при ЖИВОМУ URL (знахідка ревʼю А, с65). */
export const N8N_DEFER_NOTE = "n8n_deferred";

/** Сценарії харнеса, які НЕ можуть породити подію поза префіксом
    «integration.», тобто не зачіпають n8n-гілку взагалі.

    ⚠️ ЗАМІРЯНО ПО БД 12.09.2026, а не виведено з назв: серед функцій, що
    пишуть у `event_outbox`, подію поза префіксом створює РІВНО одна —
    `emergency_stop_rpc`; `integration_outbox_enqueue` (0145) пише лише
    `integration.*`. Отже n8n-гілку годує тільки сценарій `stop`.
...
/** ГАРД ДОСТАВКИ ПЕРЕД ПЕРШИМ ЗАПИСОМ У ПРОД — чистий вердикт, без БД.
    ...
    ⚠️ ЗНАЙДЕНО ЗАМІРОМ, А НЕ ОГЛЯДОМ. Попередник цього вердикту
    (`assertNoLiveWebhook`, с38) дивився РІВНО в `integration_webhooks` і
    рапортував «слати нема куди». Але доставка має ДВА призначення
    (lib/outbox.ts, «ДВА ПРИЗНАЧЕННЯ»): `integration.*` → вебхук клініки, УСЕ
    ІНШЕ → n8n (`N8N_WEBHOOK_URL`/`N8N_WEBHOOK_SECRET`). Сценарій `stop`
    емітить саме `emergency_stop`, тобто ДРУГУ гілку, якої гард не бачив.
    Заміряно в проді 12.09.2026: у `event_outbox` пʼять подій `emergency_stop`
    і всі пʼять із `delivered_at`; чотири з них (id 60–63) народжені прогонами
    харнеса 10–11.09. Гард казав «нікуди не піде» рівно тоді, коли воно ЙШЛО.

    ⚠️ ЧОМУ ВЕРДИКТ ЗА НАСЛІДКОМ, А НЕ ЗА КОНФІГУРАЦІЄЮ. `N8N_WEBHOOK_URL` —
    змінна оточення ДЕПЛОЮ на Vercel: ні харнес, ні БД її не бачать, і
    «перевірка конфігурації» тут була б вигадкою. Зате наслідок гілки видно в
    самій БД: рядок не-`integration.*` із `delivered_at` означає, що
    зовнішній приймач відповів 2xx (транспорт ЖИВИЙ), а рядок, відкладений
    воркером із поміткою `n8n_deferred`, — що гілка не сконфігурована.

    ⚠️ НЕВАКУУМНІСТЬ ВБУДОВАНА. Якщо у вікні немає ЖОДНОЇ події n8n-гілки,
    вердикт не «чисто», а «не знаю» — і це ВІДМОВА. Урок с63: перевірка, яка
    не вміє сказати, що не перевірила нічого, гірша за відсутню.

    ⚠️ Прапорець `--allow-n8n` знімає САМЕ n8n-гілку і нічого більше: вебхук
    клініки лишається закритим за будь-яких обставин — це чужий партнер, а не
    наш приймач.

    ⚠️ Тип фактів навмисно ДОПУСКАЄ дірки: `count` у PostgREST може бути
    `null`, а виклик — пропустити поле. Саме тому гілка `n8n_malformed` існує
    і саме тому тип не бреше, ніби числа завжди числа (інакше тест на зіпсуті
    факти не компілювався б — і це спіймав `tsc`, а не vitest).

    @param {{enabledHooks: number,
             n8n: {windowDays?: number | null, rows?: number | null, delivered?: number | null,
                   deferred?: number | null, lastDeliveredAt?: string | null} | null,
             allowN8n?: boolean, scenarioEmitsN8n?: boolean}} x
    @returns {{ok: boolean, code: string, message: string}} */
export function deliveryGuardVerdict({ enabledHooks, n8n, allowN8n = false, scenarioEmitsN8n = true }) {
  if (!Number.isFinite(enabledHooks)) {
    return {
      ok: false, code: "hooks_unknown",
      message: "не читаються вебхуки клініки — гард закритий: невідомо, куди пішли б фікстури.",
    };
  }
  if (enabledHooks > 0) {
    return {
      ok: false, code: "webhook_live",
      message: "у клініки УВІМКНЕНО вебхук інтеграції — фікстури харнеса пішли б партнеру.\n"
        + "  Вимкніть вебхук на час прогону або оберіть іншу клініку (--room <uuid> іншого центру).",
    };
  }
  /* Сценарій, який фізично не породжує подій поза префіксом, під n8n-гард не
     підпадає: міряти там нічого, а відмова «за вакуумом» вимкнула б чотири
     робочі сценарії заради гілки, якої вони не торкаються (знахідка ревʼю Б,
     с65). Вебхук клініки для них уже перевірено вище. */
  if (!scenarioEmitsN8n) {
    return {
      ok: true, code: "n8n_not_in_play",
      message: `сценарій не емітить подій поза префіксом «${INTEGRATION_PREFIX_MIRROR}» — n8n-гілка не задіяна; перевірено вебхук клініки.`,
    };
  }
  if (!n8n) {
    return {
      ok: false, code: "n8n_unknown",
      message: "не читається `event_outbox` — гард закритий: слід n8n-гілки перевірити не було на чому.",
    };
  }
  const windowDays = Number(n8n.windowDays);
  const rows = Number(n8n.rows);
  const delivered = Number(n8n.delivered);
  const deferred = Number(n8n.deferred);
  /* ⚠️ NaN не більший за нуль і не дорівнює нулю — без цієї перевірки зіпсуті
     факти (`{}`, недорахований `count`, відʼємне число, `delivered > rows`)
     провалилися б повз УСІ гілки у фінальний дозвіл. Рівно той fail-open, що
     `<>` проти NULL: мовчазний «не знаю», прочитаний як «чисто». */
  if (![windowDays, rows, delivered, deferred].every((x) => Number.isFinite(x) && x >= 0)
      || delivered > rows || deferred > rows - delivered) {
    return {
      ok: false, code: "n8n_malformed",
      message: `факти про n8n-гілку не сходяться (вікно ${n8n.windowDays}, рядків ${n8n.rows},`
        + ` доставлено ${n8n.delivered}, відкладено ${n8n.deferred}) — гард закритий: рахувати нема чого.`,
    };
  }
  if (delivered > 0) {
    if (allowN8n) {
      return {
        ok: true, code: "n8n_allowed",
        message: `--allow-n8n: n8n-гілка ЖИВА (доставлених подій у вікні: ${delivered}), перевірку знято СВІДОМО.`,
      };
    }
    return {
      ok: false, code: "n8n_live",
      message: `n8n-гілка ЖИВА: у вікні ${windowDays} дн. доставлено ${delivered} под. поза префіксом «${INTEGRATION_PREFIX_MIRROR}»`
        + `${n8n.lastDeliveredAt ? `, остання — ${n8n.lastDeliveredAt}` : ""} — зовнішній приймач відповідав 2xx.\n`
        + "  Аварійні події несуть ПІБ і телефони (lib/outbox.ts), і прогін відправив би туди фікстури.\n"
        /* ⚠️ Перша редакція радила «зніміть N8N_WEBHOOK_URL і запустіть знову».
           Порада НЕ ПРАЦЮВАЛА (знахідка ревʼю Б, с65): вердикт читає історію,
           і доставлений рядок лишається у вікні до кінця горизонту ретенції —
           оператор зняв би змінну, отримав той самий текст і вирішив, що гард
           зламаний. Кажемо це прямо. */
        + "  ⚠️ Зняти N8N_WEBHOOK_URL у деплої цей вердикт НЕ змінить: він читає ІСТОРІЮ доставок,\n"
        + "     а доставлений рядок живе у вікні до кінця горизонту ретенції.\n"
        + "  Якщо гілку вже вимкнено — переконайтесь у цьому самі й запускайте свідомо: --allow-n8n.",
    };
  }
  if (rows === 0) {
    if (allowN8n) {
      return {
        ok: true, code: "n8n_allowed",
        message: "--allow-n8n: сліду n8n-гілки у вікні немає, перевірку знято СВІДОМО.",
      };
    }
    return {
      ok: false, code: "n8n_vacuous",
      message: `не доведено, що n8n-гілка мовчить: у вікні ${windowDays} дн. немає ЖОДНОЇ події поза префіксом «${INTEGRATION_PREFIX_MIRROR}».\n`
        + "  Це «не знаю», а не «чисто» — перевіряти не було на чому, тому гард закритий.\n"
        + "  ⚠️ Порожнє вікно — ШТАТНИЙ стан спокійного центру: цю гілку годують лише аварійні зупинки.\n"
        + "  Свідомий прогін: --allow-n8n.",
    };
  }
  /* ПОЗИТИВНИЙ доказ мовчання, а не відсутність доказу протилежного: КОЖЕН
     недоставлений рядок вікна має помітку воркера «гілка не сконфігурована».
     Рядок без помітки — це або щойно емітований (воркер ще не дійшов), або
     такий, що падає з помилкою транспорту, тобто при ЖИВОМУ URL. */
  if (deferred === rows - delivered) {
    return {
      ok: true, code: "n8n_silent",
      message: `n8n-гілка мовчить: усі ${rows} под. поза префіксом «${INTEGRATION_PREFIX_MIRROR}» у вікні ${windowDays} дн.`
        + ` відкладені воркером із поміткою «${N8N_DEFER_NOTE}» — гілка не сконфігурована.`,
    };
  }
  if (allowN8n) {
    return {
      ok: true, code: "n8n_allowed",
      message: `--allow-n8n: стан n8n-гілки незʼясований (${rows - delivered - deferred} недоставлених без помітки`
        + ` «${N8N_DEFER_NOTE}»), перевірку знято СВІДОМО.`,
    };
  }
  return {
    ok: false, code: "n8n_unclear",
    message: `стан n8n-гілки незʼясований: із ${rows - delivered} недоставлених под. у вікні ${windowDays} дн.`
      + ` помітку «${N8N_DEFER_NOTE}» мають лише ${deferred}.\n`
      + "  Решта або щойно емітовані (воркер ще не дійшов), або падають помилкою транспорту — а це означає,\n"
      + "  що URL налаштований. Зачекайте такт воркера і повторіть, або запускайте свідомо: --allow-n8n.",
  };
}
...
/** Вердикт СЕРІЇ прогонів гонки за кейс — і саме він знімає блокування с62.
    ...
    ⚠️ ВИРІШАЛЬНЕ УПОРЯДКУВАННЯ САМЕ ДІЛИТЬСЯ НАВПІЛ — замір 11.09.2026
    (пакет 62, с63), знято `pg_get_functiondef` із прода. Тіло
    `add_case_step_rpc` робить `select * into v_case … for update` і ЛИШЕ
    ПОТІМ `if v_case.status <> 'open'`. Звідси дві різні історії, що дають
    ОДНАКОВУ відповідь `22023 «кейс не активний»`:

      (б) `add` дійшов до `for update`, поки скасування ТРИМАЛО лок → став,
          дочекався коміту, перечитав рядок і побачив не-`open`. Зламаний
          гард (перевірка статусу ДО лока) прочитав би `open` і вставив крок.
          Ось це — доказ;
      (а) скасування ВСТИГЛО закомітити, перш ніж транзакція `add` узагалі
          дійшла до вибірки → `add` нікого не чекав, одразу прочитав
          `cancelled`. Зламаний гард прочитав би те саме і теж відмовив.
          Доказу немає — це вакуум усередині «вирішального».

    ⚠️ РОЗРІЗНИТИ (а) і (б) З КЛІЄНТА НЕМОЖЛИВО, і кандидат «за тривалістю
    `add`» НЕ ПРАЦЮЄ. У вирішальному упорядкуванні `add` нічого не вставляє,
    тож власної роботи в нього мало в обох історіях; уся різниця — у
    ЧЕКАННІ. А клієнтські мітки міряють HTTP-виклик, а не транзакцію: момент,
    коли транзакція `add` почалась, назовні не видно. Затримка на локу і
    затримка на отриманні зʼєднання дають ту саму форму за годинником.

    Що РОЗРІЗНИЛО б: серверне спостереження очікування
    (`pg_stat_activity.wait_event_type = 'Lock'` для бекенда `add` під час
    гонки). Через PostgREST це не читається, а заводити заради тесту нову
    SECURITY DEFINER-поверхню в проді проєкт уже свідомо відмовився (та сама
    причина, що в `DEADLOCK_SQLSTATE`). Тому межа НАЗВАНА, а не закрита.

    ⚠️ ЧОМУ PASS усе одно чогось вартий. Історія (а) вимагає, щоб транзакція
    `add` почалась на ~200 мс пізніше за власний HTTP-запит, випущений у ту
    саму мілісекунду, що й скасування. Це рідкість, і що більше вирішальних
    прогонів, то менш імовірно, що ВСІ вони були (а). Тому їх кількість
    друкується в причині — це не прикраса, а сила твердження. Один
    вирішальний прогін такої сили не дає, і вердикт про це каже.
...
export function verdictCaseRounds(rounds) {
  ...
  /* ⚠️ СИЛА ТВЕРДЖЕННЯ ЗАЛЕЖИТЬ ВІД КІЛЬКОСТІ ВИРІШАЛЬНИХ ПРОГОНІВ, і це не
     статистичне кокетство. Кожен вирішальний прогін міг бути історією (а) —
     «скасування встигло закомітити до старту транзакції add», де гарант не
     знадобився. Розібрати (а) і (б) з клієнта неможливо (розбір у шапці), тож
     єдине, чим PASS підкріплюється, — що ВСІ k прогонів були (а) тим менш
     імовірно, чим більше k. При k = 1 цього підкріплення немає взагалі, і
     мовчазний PASS тут читався б як доведений гарант. */
  const weak = decisive.length === 1;
  return {
    verdict: "PASS", spread,
    reason: `${rounds.length} прогонів: ${decisive.length} у вирішальному упорядкуванні `
      ...
      + "⚠️ Вирішальне упорядкування саме ділиться навпіл: «add чекав на локу» (доказ) і "
      + "«скасування встигло закомітити до старту add» (вакуум) дають ОДНАКОВУ відмову, і "
      + "з клієнта вони не розрізняються — див. шапку verdictCaseRounds. "
      + (weak
        ? "⚠️ ВИРІШАЛЬНИЙ ПРОГІН РІВНО ОДИН: він міг бути вакуумним, і спертись немає на що. "
          + "Для твердження про гарант потрібно більше — збільште --rounds"
        : `Опора тут одна: щоб усі ${decisive.length} були вакуумними, транзакція add мусила б `
          + "щоразу стартувати на ~200 мс пізніше за власний запит"),
  };
}
