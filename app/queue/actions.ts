...
/* ===== Запис ПОЗА ГРАФІКОМ кабінету =====
   ...
   ⚠️ ВИПРАВЛЕНО в с63 — тут стояла НЕПРАВДА, і вона встигла розійтись по
   чотирьох місцях. Було: «Графік (години/дні) у БД НЕ enforce'иться взагалі —
   ні до 0077, ні після. Для випадку „після кінця дня“ цей серверний гард —
   ЄДИНИЙ рубіж». Це було правдою рівно до **0084** (`check_room_schedule`),
   яка зробила графік ІНВАРІАНТОМ БД: закритий день, час до відкриття і стеля
   +120 хв відхиляються тригером ЗАВЖДИ, а робота після закриття — лише з
   `off_schedule = true`. Тобто рубежів ДВА, як і для перерви, і саме тому
   прямий PostgREST-INSERT повз сервер більше не проходить.
   Заміряно живим зондом 11.09: вставка на 23:50 у кабінет 08:00–18:00 падає
   `23514 ROOM_CLOSED` із `check_room_schedule()`.
   Що це НЕ скасовує: гард тут лишається потрібним і першим — він дає
   користувачеві діалог підтвердження замість сирого SQLSTATE, а при збої
   читання графіка падає fail-closed (H-6 нижче). Але називати його ЄДИНИМ
   рубежем не можна: на цьому твердженні будувалась оцінка ризику U-13. */
...
async function scheduleGate(
  ...
  /* ⚠️ U-13 (с49): гілка вище закривала лише ПОМИЛКУ, а `room?.schedule ?? null`
     ...
     чого коментар вище обіцяв не допускати.
     ⚠️ ОЦІНКА ВАГИ ВИПРАВЛЕНА в с63. Тут стояло «а графік у БД не enforce'иться
     взагалі, тож для „після кінця дня“ цей гард — ЄДИНИЙ рубіж»: неправда з
     0084 (`check_room_schedule`), див. розбір у шапці вище. Правка від цього
     потрібною бути не перестає — гард, що рахує за вигаданим 08:00–18:00,
     бреше користувачеві і сам, — але вона не «останній рубіж», а перший із
     двох: 23:50 у кабінет 08:00–18:00 тригер відхилить і без нас.
